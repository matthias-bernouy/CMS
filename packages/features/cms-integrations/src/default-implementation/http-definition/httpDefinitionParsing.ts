...
export function parseVersions(value: unknown): IntegrationDefinitionVersion[] {
    ...
    const versions = value.map((entry) => {
        ...
        const verificationDigest = digest(entry.verificationDigest);
        return {
...
function digest(value: unknown): string | undefined {
    ...
    if (typeof value !== "string" || !/^[a-f0-9]{64}$/u.test(value)) {
        throw new Error("integration repository response verificationDigest must be lowercase SHA-256 when present");
    }
    return value;
}
