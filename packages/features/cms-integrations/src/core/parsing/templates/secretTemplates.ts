...
import type { DeclarativeGeneratedSecretTemplate, DeclarativeSecretTemplate } from "../../../interfaces/Integration";
...
export function parseSecretTemplates(value: unknown, secretInputs: ReadonlySet<string>): DeclarativeSecretTemplate[] {
    ...
    if (!Array.isArray(value)) {
        throw new IntegrationInputError("definition.secrets", "must be an array");
    }
...
export function parseGeneratedSecretTemplates(value: unknown): DeclarativeGeneratedSecretTemplate[] {
    ...
    if (!Array.isArray(value)) {
        throw new IntegrationInputError("definition.generatedSecrets", "must be an array");
    }
...
export function assertUniqueSecretBindingNames(
    ...
    for (const secret of secrets) {
        if (seen.has(secret.input)) {
            throw new IntegrationInputError(`definition.secrets.${secret.input}`, "duplicate secret binding name");
        }
    ...
    for (const secret of generatedSecrets) {
        if (seen.has(secret.name)) {
            throw new IntegrationInputError(
                `definition.generatedSecrets.${secret.name}`,
                "duplicate secret binding name",
            );
        }
...
export function validateGeneratedSecretDefinition(secret: DeclarativeGeneratedSecretTemplate): void {
    if (!secret.name) {
        throw new IntegrationInputError("definition.generatedSecrets.name", "is required");
    }
    if (!secret.key) {
        throw new IntegrationInputError(`definition.generatedSecrets.${secret.name}.key`, "is required");
    }
    ...
    if (secret.bytes !== undefined && (!Number.isInteger(secret.bytes) || secret.bytes < 16 || secret.bytes > 64)) {
        throw new IntegrationInputError(
            `definition.generatedSecrets.${secret.name}.bytes`,
            "must be an integer between 16 and 64",
        );
    }
}

function parseSecretTemplate(
    value: unknown,
    name: string,
    ...
    if (!input) {
        throw new MissingIntegrationParam(`${name}.input`);
    }
    if (!secretInputs.has(input)) {
        throw new IntegrationInputError(`${name}.input`, "must reference a secret input");
    }
