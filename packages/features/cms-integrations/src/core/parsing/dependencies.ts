...
export function parseDependencies(value: unknown, definitionKind: string): IntegrationDependency[] {
    ...
    if (!Array.isArray(value)) throw new IntegrationInputError("definition.dependencies", "must be an array");
...
function parseDependency(value: unknown, name: string): IntegrationDependency {
    ...
    if (!dependencyName) throw new MissingIntegrationParam(`${name}.name`);
    if (!SIMPLE_ID.test(dependencyName)) throw new IntegrationInputError(`${name}.name`, "must be a simple id");
    const kind = text(value.kind);
    if (!kind) throw new MissingIntegrationParam(`${name}.kind`);
    return {
        name: dependencyName,
        kind,
...
function validateDependencyList(dependencies: readonly IntegrationDependency[], definitionKind: string): void {
    ...
    for (const dependency of dependencies) {
        ...
        if (dependency.kind === definitionKind) {
            throw new IntegrationInputError(`definition.dependencies.${dependency.name}.kind`, "must not reference the integration itself");
        }
