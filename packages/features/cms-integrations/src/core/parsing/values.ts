...
export function parseAnswersBody(value: unknown): Record<string, unknown> | undefined {
    ...
    if (!isRecord(parsed)) throw new IntegrationInputError("answers", "must be an object");
    return parsed;
}
...
export function parseOptions(value: unknown): IntegrationImportOptions {
    ...
    if (!isRecord(parsed)) throw new IntegrationInputError("options", "must be an object");
...
export function validateUrl(name: string, value: string): void {
    ...
    if (!verdict.ok) throw new IntegrationInputError(`answers.${name}`, verdict.reason);
}
...
export function isInputType(value: string | undefined): value is IntegrationInput["type"] {
    return value === "text" || value === "url" || value === "password"
        || value === "select" || value === "boolean" || value === "json";
}
