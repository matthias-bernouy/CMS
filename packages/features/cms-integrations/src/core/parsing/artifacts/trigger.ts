...
export function parseTriggerTemplate(value: Record<string, unknown>, name: string): TriggerDto {
    ...
    if (!isRecord(value.event)) throw new IntegrationInputError(`${name}.event`, "must be an object");
    ...
    if (eventKind !== "endpoint") throw new IntegrationInputError(`${name}.event.kind`, "must be endpoint");
    const phase = text(value.event.phase);
    if (phase !== "request" && phase !== "response") throw new IntegrationInputError(`${name}.event.phase`, "must be request or response");
    const functionId = text(value.function.id);
    if (!functionId) throw new MissingIntegrationParam(`${name}.function.id`);
    return {
        id,
        ...
        ...(value.failureMode !== undefined ? { failureMode: text(value.failureMode) as TriggerDto["failureMode"] } : {}),
        ...(value.condition !== undefined ? { condition: value.condition as TriggerDto["condition"] } : {}),
        function: {
            id: functionId,
            ...(isRecord(value.function.params) ? { params: value.function.params as NonNullable<TriggerDto["function"]["params"]> } : {}),
            ...(value.function.body !== undefined ? { body: value.function.body as TriggerDto["function"]["body"] } : {}),
        },
    };
}
