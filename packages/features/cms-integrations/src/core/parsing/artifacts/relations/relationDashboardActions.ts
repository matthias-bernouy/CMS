    ...
    return value.map((entry, index) => {
        ...
        return {
            ...
            ...(text(entry.placement)
                ? { placement: text(entry.placement)! as RelationDashboardAction["placement"] }
                : {}),
            ...(entry.endpoint !== undefined
                ? { endpoint: parseRelationDashboardActionEndpoint(entry.endpoint, `${name}.${index}.endpoint`) }
                : {}),
        };
    });
}

function parseRelationDashboardActionEndpoint(
    value: unknown,
    name: string,
): NonNullable<RelationDashboardAction["endpoint"]> {
    if (!isRecord(value)) {
