...
function parseRule(value: unknown, name: string, depth: number, budget: { nodes: number }): DashboardVisibilityRule {
    if (depth >= DASHBOARD_VISIBILITY_MAX_DEPTH) throw new IntegrationInputError(name, "exceeds the maximum visibility depth");
    if (++budget.nodes > DASHBOARD_VISIBILITY_MAX_NODES) throw new IntegrationInputError(name, "exceeds the maximum visibility rule count");
    ...
    if (hasAll || hasAny) {
        if (hasAll && hasAny) throw new IntegrationInputError(name, "cannot declare both all and any");
        if (hasCondition) throw new IntegrationInputError(name, "cannot combine a group with a value condition");
        if (Object.keys(value).length !== 1) throw new IntegrationInputError(name, "contains unsupported visibility properties");
    ...
    if (hasEquals === hasNotEquals) throw new IntegrationInputError(name, "must declare exactly one of equals or notEquals");
    if (Object.keys(value).length !== 2) throw new IntegrationInputError(name, "contains unsupported visibility properties");
    return hasEquals
