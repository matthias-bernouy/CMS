...
function resolveExpression(expression: string, context: TemplateContext): string | boolean {
    ...
    if (expression.startsWith("answers.")) {
        ...
        if (context.secretInputs?.has(key)) {
            throw new IntegrationInputError(
                "template",
                `secret answer "${key}" must be referenced through secrets.${key}`,
            );
        }
    ...
    if (expression.startsWith("secrets.")) {
        ...
        if (!value) {
            throw new IntegrationInputError("template", `unknown secret "${key}"`);
        }
        return value;
    }
    if (expression.startsWith("generated.")) {
        ...
        if (!value) {
            throw new IntegrationInputError("template", `unknown generated value "${key}"`);
        }
        return value;
    }
    if (expression.startsWith("connectorSecrets.")) {
        ...
        if (!value) {
            throw new IntegrationInputError("template", `unknown connector secret "${key}"`);
        }
        return value;
    }
...
function resolveDependencyExpression(expression: string, context: TemplateContext): string | boolean {
    ...
    if (key === "answers" && rest.length === 1) {
        ...
        if (typeof value !== "string" && typeof value !== "boolean") {
            throw new IntegrationInputError(
                "template",
                `dependency answer "${name}.${answerKey}" cannot be interpolated as text`,
            );
        }
        return value;
    }
