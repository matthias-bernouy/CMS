    ...
    try {
        for (const write of writes) {
            if (write.previous) {
                ...
                if (!updated) {
                    throw new IntegrationRuntimeError(`trigger disappeared during import: ${write.trigger.id}`, 409);
                }
...
async function rollbackTriggers(
    ...
    for (const write of completed.reverse()) {
        try {
            if (write.previous) {
                await triggerRepository.updateTrigger(write.previous);
            } else {
                await triggerRepository.deleteTrigger(write.trigger.id);
            }
        } catch {
