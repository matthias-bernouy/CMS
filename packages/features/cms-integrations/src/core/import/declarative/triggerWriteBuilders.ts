...
export async function buildTriggerWrites(
    ...
    if (!triggerArtifacts.length) return [];
    if (!deps.triggers) throw new IntegrationRuntimeError("trigger repository not configured");

    const triggerWrites: IntegrationTriggerWrite[] = [];
    const seen = new Set<string>();
    for (const trigger of triggerArtifacts) {
        if (seen.has(trigger.id)) throw new DuplicateTriggerError(trigger.id);
        seen.add(trigger.id);
        ...
        if (!options.force && previous) throw new DuplicateTriggerError(trigger.id);
