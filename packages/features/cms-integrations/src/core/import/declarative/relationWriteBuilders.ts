...
export async function buildRelationWrites(
    ...
    for (const relation of relations) {
        if (seen.has(relation.id)) throw new DuplicateRelationError(relation.id);
        ...
        if (!options.force && previous) throw new DuplicateRelationError(relation.id);
        relationWrites.push({ relation, previous });
    }
    return relationWrites;
}

function relationSourceResolver(
    deps: IntegrationImportDeps,
    sourceArtifacts: Source[],
): RelationEndpointResolver {
    ...
    return async (sourceId, endpointId) => {
        ...
        if (source) return source.endpoints.find(endpoint => endpoint.urn === endpointUrn) ?? null;
