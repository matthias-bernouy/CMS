...
async function validateProjectionTarget(
    ...
    const relation =
        relationById.get(projection.relationId) ?? (await deps.relations!.getRelation(projection.relationId));
    if (!relation) {
        throw new IntegrationInputError(
            "artifacts",
            `dashboard relation projection "${id}" references unknown relation "${projection.relationId}"`,
        );
    }
    ...
    if (!dashboard) {
        throw new IntegrationInputError(
            "artifacts",
            `dashboard relation projection "${id}" references unknown dashboard "${projection.dashboardId}"`,
        );
    }
    ...
    if (!view) {
        throw new IntegrationInputError(
            "artifacts",
            `dashboard relation projection "${id}" references unknown dashboard view "${projection.viewId}"`,
        );
    }
    if (view.widget !== "w-detail") {
        throw new IntegrationInputError(
            "artifacts",
            `dashboard relation projection "${id}" must target a w-detail view`,
        );
    }
}
