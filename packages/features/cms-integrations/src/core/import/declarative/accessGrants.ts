import { functionEndpointUrn, type CmsFunction } from "@bernouy/cms-functions";
import {
    PUBLIC_ROLE,
    USER_ROLE,
    type Grant,
...
import { sourceEndpointAccessMode, type Source, type SourceEndpointAccessMode } from "@bernouy/cms-sources";
...
export function buildIntegrationAccessGrants(sources: Source[], functions: CmsFunction[]): IntegrationAccessGrant[] {
    const grants: IntegrationAccessGrant[] = [];
    for (const source of sources) {
        for (const endpoint of source.endpoints) {
            const roleId = roleForAccessMode(sourceEndpointAccessMode(endpoint));
            if (roleId) {
                grants.push({ roleId, permission: endpoint.urn });
            }
        }
    }
    for (const fn of functions) {
        ...
        if (roleId) {
            grants.push({ roleId, permission: functionEndpointUrn(fn.id) });
        }
    }
    return grants;
}
...
export async function applyIntegrationAccessGrants(
    ...
    if (!roles) {
        throw new IntegrationRuntimeError("roles repository not configured");
    }
...
function grantsByRole(grants: IntegrationAccessGrant[]): Map<IntegrationAccessGrant["roleId"], Set<string>> {
    const byRole = new Map<IntegrationAccessGrant["roleId"], Set<string>>();
    for (const grant of grants) {
        if (!byRole.has(grant.roleId)) {
            byRole.set(grant.roleId, new Set());
        }
...
function roleForAccessMode(mode: SourceEndpointAccessMode): typeof PUBLIC_ROLE | typeof USER_ROLE | null {
    if (mode === "public") {
        return PUBLIC_ROLE;
    }
    if (mode === "auth") {
...
function defaultBuiltInRole(roleId: IntegrationAccessGrant["roleId"]): RoleDefinition {
    if (roleId === PUBLIC_ROLE) {
        return { id: PUBLIC_ROLE, label: "Public", builtin: true, grants: [] };
    }
