...
import { deleteArtifact, restoreArtifacts, type ArtifactRestorer } from "./repositoryArtifacts";
 ...
 */
export async function withObsoleteArtifactCleanup<T>(request: ObsoleteArtifactCleanupRequest<T>): Promise<T> {
...
async function findUnclaimedObsoleteArtifacts(
    ...
    for (const installation of await request.installations.list()) {
        ...
        for (const artifact of installation.artifacts) {
            claimedElsewhere.add(artifactKey(artifact));
        }
    }
    ...
    return request.previousArtifacts
        .filter((artifact) => {
            ...
            if (seen.has(key) || retained.has(key) || claimedElsewhere.has(key)) {
                return false;
            }
            seen.add(key);
            return true;
