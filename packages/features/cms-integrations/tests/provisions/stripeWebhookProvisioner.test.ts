...
import { capturedRequest, json, stripeEndpoint, stripeWebhookDeployment } from "./stripeWebhookFixtures";
...
describe("StripeWebhookProvisioner", () => {
    ...
    test("fails closed when Stripe owns a destination whose secret is not stored", async () => {
        ...
        await expect(
            provisioner.provision(stripeWebhookDeployment(), { existingOutputs: {}, env: {} }),
        ).rejects.toThrow(/already exists but its signing secret is missing/);
    });
