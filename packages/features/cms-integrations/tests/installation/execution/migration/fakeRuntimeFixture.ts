...
class FakeMigrationRuntime implements IntegrationMigrationRuntime {
    ...
    readonly compensations = new Map<IntegrationMigrationPhase, number>();
        ...
        const result: IntegrationMigrationStepResult = {
            ...
            ...(context.phase === "switch-cms-binding"
                ? {
                      importResult: {
                          artifacts:
                              context.connectors.some((connector) => connector.plan.cmsMediated) &&
                              context.targetDefinition.artifacts?.some((artifact) => artifact.type === "source")
                                  ? [{ type: "source" as const, id: "urn:commerce-api", action: "updated" as const }]
                                  : [],
                          connectors: [
                              {
                                  provider: "supabase",
    ...
    async compensateStep(context: IntegrationMigrationStepContext) {
        this.compensations.set(context.phase, (this.compensations.get(context.phase) ?? 0) + 1);
        return { compensated: true, externalOperationId: `compensated:${context.idempotencyKey}` };
    }
