...
describe("SupabaseConnectorDeployer", () => {
    test("applies schemas, sets function secrets, and deploys functions through the Management API", async () => {
        ...
        const fetchImpl: typeof fetch = async (input, init) => {
            ...
            if (String(input).endsWith("/postgrest") && init?.method === "GET") {
                return Response.json({ db_schema: "public,storage", max_rows: 1000, db_extra_search_path: "public", db_pool: null, db_pool_acquisition_timeout: null });
            }
            if (String(input).includes("/functions/deploy")) {
                return Response.json({ id: "fn_1", slug: "cms-user-account", name: "CMS User Account", status: "ACTIVE", version: 1 }, { status: 201 });
            }
        ...
        expect(JSON.parse(String(requests[0]?.init.body))).toEqual({ query: "create schema if not exists user_account;\n" });
        ...
        expect(requests.every(request => new Headers(request.init.headers).get("authorization") === "Bearer sbp_test")).toBe(true);
    });
    ...
    test("reloads the PostgREST schema cache when a Data API schema is already exposed", async () => {
        ...
        const result = await deployer.deploy({ ...userAccountDeployment(), functions: [] }, {
            answers: {},
            generated: {},
            secrets: {},
            env: {},
    ...
    test("redacts secret values from Supabase API errors", async () => {
        ...
        const deployer = new SupabaseConnectorDeployer({
            ...
            fetch: async (input, init) => {
                ...
                if (String(input).endsWith("/postgrest") && init?.method === "GET") {
                    return Response.json({ db_schema: "public,user_account", max_rows: 1000, db_extra_search_path: "public", db_pool: null, db_pool_acquisition_timeout: null });
                }
                return new Response(null, { status: 201 });
            },
        });
...
async function createSupabaseConnectorFixture(): Promise<string> {
    ...
    await writeFile(join(connectorRoot, "functions", "cms-user-account", "index.ts"), "Deno.serve(() => new Response(\"ok\"));\n");
    return root;
}
