...
export type VerificationJobResultV1 = Readonly<{
    ...
    bindings: Readonly<{
        ...
        /** Missing only when the corresponding legacy admission has no runtime release plan. */
        releaseVerificationPlanDigest?: string;
        /** Missing only when the corresponding legacy admission has no runtime release plan. */
        upgradeBaselineDigests?: readonly string[];
    }>;
