...
type SystemGatewayEndpoint = {
    urn:       string;
    targetUrl: string;
};

export async function executeAuthSystemGatewayEndpoint<Role extends string>(
    cfg: PublicAuthRoutesConfig<Role>,
    endpoint: SystemGatewayEndpoint,
    req: Request,
): Promise<Response> {
...
function parseSystemAuthTarget(endpoint: SystemGatewayEndpoint): string {
    let url: URL;
    try {
