...
describe("typed dashboard detail fields", () => {
    test("accepts every typed field and the exact technical limits", () => {
        const tableColumns = [
            ...
            { id: "combo", label: "Combo", path: "combo", editable: true, type: "combobox",
        ...
        const fields = [
            ...
            { id: "matrix", label: "Matrix", path: "matrix", type: "table", editable: true,
                addLabel: "Add row", columns: tableColumns },
            { id: "variantAxes", label: "Axes", path: "variantAxes", type: "reorderable-list",
                itemKey: "id", fields: itemFields },
            { id: "definition", label: "Definition", path: "definition", type: "schema",
                schema: { endpoint: "schema" }, exclude: { from: "$field.variantAxes", valuePath: "fieldKey" } },
            { id: "choice", label: "Choice", path: "choice", type: "combobox", lookup: {
                ...embeddedLookup(), create: { mode: "modal", endpoint: "create", valuePath: "id", labelPath: "name",
    ...
    test("rejects values above every technical limit", () => {
        const fields = [
            { id: "status", label: "Status", path: "status", type: "select", options: options(DASHBOARD_MAX_OPTIONS + 1) },
            { id: "matrix", label: "Matrix", path: "matrix", type: "table", columns: columns(DASHBOARD_MAX_NESTED_FIELDS + 1) },
        ...
        expect(validateDashboard(detail(fields))).toEqual(expect.arrayContaining([
    ...
    test("rejects unsafe or legacy schema exclusions", () => {
        const valid = { id: "schema", label: "Schema", path: "schema", type: "schema",
            schema: { endpoint: "schema" }, exclude: { from: "$field.axis", valuePath: "fieldKey" } };
        const cases: Array<[Record<string, unknown>, string]> = [
            [{ ...valid, exclude: { from: "$resource.axis", valuePath: "fieldKey" } }, "exclude.from must be a $field expression"],
            [{ ...valid, exclude: { from: "$field", valuePath: "fieldKey" } }, "exclude.from must be a $field expression"],
            [{ ...valid, exclude: { from: "$field.__proto__.axis", valuePath: "fieldKey" } }, "exclude.from must be a $field expression"],
            [{ ...valid, exclude: { from: "$field.missing", valuePath: "fieldKey" } }, "references unknown field \"missing\""],
            [{ ...valid, exclude: { from: "$field.axis", valuePath: "prototype.key" } }, "valuePath must be a safe dotted data path"],
            [{ ...valid, exclude: { from: "$field.axis", valuePath: "fieldKey", extra: true } }, "unsupported properties"],
        ...
        for (const [candidate, message] of cases) {
            expect(validateDashboard(detail([
                { id: "axis", label: "Axis", path: "axis", type: "text" },
                candidate as DashboardField,
            ])).join("\n")).toContain(message);
        }
    });

    test("resolves schema exclusions across main and aside sections", () => {
        const dashboard = detail([{ id: "schema", label: "Schema", path: "schema", type: "schema",
            schema: { endpoint: "schema" }, exclude: { from: "$field.axis", valuePath: "fieldKey" } }]);
        const widget = dashboard.views[0];
        if (widget?.widget !== "w-detail") throw new Error("invalid fixture");
        widget.aside = [{ id: "aside", title: "Aside",
            fields: [{ id: "axis", label: "Axis", path: "axis", type: "text" }] }];
    ...
    test("rejects incoherent nested editors and duplicate ids", () => {
        const fields = [
            { id: "readonly", label: "Readonly", path: "readonly", type: "table", addLabel: "Add", columns: [
            ...
            { id: "items", label: "Items", path: "items", type: "reorderable-list", itemKey: "id", fields: [
                ...
                { id: "combo", label: "Combo", path: "combo", type: "combobox",
                    lookup: { ...embeddedLookup(), create: null } },
            ] },
            { id: "modal", label: "Modal", path: "modal", type: "combobox", lookup: {
                ...embeddedLookup(), create: { mode: "modal", endpoint: "create", valuePath: "id", labelPath: "name",
                    fields: [{ id: "same", label: "A", path: "a", type: "text" },
    ...
    test("validates number constraints and accepts editable false as readonly", () => {
        const fields = [
            ...
            { id: "table", label: "Table", path: "table", type: "table",
...
function detail(fields: DashboardField[]): Dashboard {
    return { id: "typed", source: "typed", views: [{ widget: "w-detail", id: "detail",
        source: { endpoint: "resource" }, main: [{ id: "main", title: "Main", fields }] }] };
}

const options = (count: number) => Array.from({ length: count }, (_, index) => ({ value: `v${index}`, label: `V${index}` }));
const columns = (count: number, offset = 0) => Array.from({ length: count }, (_, index) => ({
    id: `column${index + offset}`, label: `Column ${index + offset}`, path: `column${index + offset}`,
}));
const nestedFields = (count: number, offset = 0) => Array.from({ length: count }, (_, index) => ({
    id: `field${index + offset}`, label: `Field ${index + offset}`, path: `field${index + offset}`, type: "text" as const,
}));
