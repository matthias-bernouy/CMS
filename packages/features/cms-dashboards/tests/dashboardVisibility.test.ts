...
describe("dashboard visibility", () => {
    ...
    test("bounds recursive depth and total rule count", () => {
        ...
        expect(validateDashboard(detailDashboard(tooDeep))).toContainEqual(expect.stringContaining("maximum visibility depth"));
        const countErrors = validateDashboard(detailDashboard(tooWide))
            .filter(error => error.includes("maximum visibility rule count"));
    ...
    test("evaluates expressions strictly and fails closed", () => {
        ...
        expect(evaluateDashboardVisibility({
            all: [
                { value: "$resource.status", equals: "draft" },
        ...
        expect(evaluateDashboardVisibility({ field: "mode", equals: "advanced" } as unknown as DashboardVisibilityRule, resolve)).toBe(false);
        expect(evaluateDashboardVisibility({
            value: "$field.mode",
            equals: "advanced",
            notEquals: "simple",
        } as DashboardVisibilityRule, resolve)).toBe(false);
        expect(evaluateDashboardVisibility({
            any: [
                { value: "$field.mode", equals: "advanced" },
        ...
        expect(evaluateDashboardVisibility({
            ...
            equals: "advanced",
            unexpected: true,
        } as unknown as DashboardVisibilityRule, resolve)).toBe(false);
    });
});
...
function wrap(rule: DashboardVisibilityRule, count: number): DashboardVisibilityRule {
    let result = rule;
    for (let index = 0; index < count; index++) result = { all: [result] };
    return result;
}
