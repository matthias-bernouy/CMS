...
export function applyDetailSourceOverlay(widget: DetailWidget, overlay: SourceOverlay, dashboardId: string): DetailWidget {
    ...
    next = {
        ...
        main: addOverlayDetailFields(next.main, overlay),
...
function addOverlayDetailFields(sections: DetailSections, overlay: SourceOverlay): DetailSections {
    const fields = overlay.fields.filter(field => field.adminEditable !== false);
    if (!fields.length) return sections;

    let next = sections;
    for (const section of groupedDashboardFields(overlay, fields)) {
...
function groupedDashboardFields(overlay: SourceOverlay, fields: readonly SourceOverlayField[]): DetailSections {
    ...
    for (const field of fields) {
        ...
        groups.get(sectionId)!.push(dashboardField(field));
    }
...
function addOverlayActionBodies(actions: DashboardAction[] | undefined, overlay: SourceOverlay): DashboardAction[] | undefined {
    ...
    return actions.map(action => {
        ...
        for (const field of fields) body[sourceOverlayFieldPath(field)] = `$field.${overlayFieldId(field)}`;
