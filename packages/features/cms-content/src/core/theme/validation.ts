...
export function validateThemeSettings(settings: ThemeSettings): ThemeSettings {
    ...
    for (const item of allTokens(settings)) {
        ...
        if (tokenIds.has(item.id)) {
            throw new ContentValidationError("theme", `duplicate token id: ${item.id}`);
        }
        if (variables.has(item.variable)) {
            throw new ContentValidationError("theme", `duplicate CSS variable: ${item.variable}`);
        }
    ...
    for (const theme of settings.themes) {
        ...
        if (themeIds.has(theme.id)) {
            throw new ContentValidationError("theme", `duplicate theme id: ${theme.id}`);
        }
