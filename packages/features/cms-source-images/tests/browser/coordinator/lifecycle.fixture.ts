...
describe("bound image runtime lifecycle in an isolated DOM fixture", () => {
    ...
    test("activates a complete picture group only after every dynamic value resolves", async () => {
        ...
        const order: string[] = [];
        recordAttributeWrites(source, order);
        recordAttributeWrites(fallback, order);
...
function recordAttributeWrites(element: Element, writes: string[]): void {
    const setAttribute = element.setAttribute.bind(element);
    element.setAttribute = (name, value) => {
        writes.push(`${element.localName}:${name}`);
