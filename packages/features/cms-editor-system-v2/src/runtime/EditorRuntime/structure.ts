...
export function runtimeElements(root: HTMLElement): HTMLElement[] {
    return [root, ...Array.from(root.querySelectorAll<HTMLElement>("*"))]
        .filter(element => !hasCompositionAncestor(element));
}
...
export function findRichTextOwner(
    context: EditorRuntimeStructureContext,
    target: HTMLElement,
): Editor | undefined {
...
function structureChildren(
    ...
    for (const editor of context.editors) {
        if (!context.document.contentRoot.contains(editor.target)) continue;
        if (editor.target === parent) continue;
        if (findRichTextOwner(context, editor.target)) continue;
        if (!parent.contains(editor.target)) continue;
        if (closestStructureParent(context, editor.target, parent) !== parent) continue;
        ...
        children.push({
            ...
            children: editor.getStructureMode() === "opaque"
                ? []
                : structureChildren(context, editor.target),
        });
    }
...
function closestStructureParent(
    ...
    while (current && current !== stopAt) {
        if (context.document.contentRoot.contains(current)) {
            ...
            if (editor && !findRichTextOwner(context, editor.target)) return current;
        }
        current = current.parentElement;
    }
