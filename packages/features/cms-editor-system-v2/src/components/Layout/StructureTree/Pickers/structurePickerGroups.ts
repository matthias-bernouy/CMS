...
export function replaceGroups(context: StructurePickerGroupContext, node: EditorStructureNode): BlockPickerSlotGroup[] {
    ...
    if (!parent) return rootGroups(context);
...
export function defaultTemplateItems(context: StructurePickerGroupContext): BlockPickerItem[] {
    const templates = context.insertItems.filter((item): item is Extract<BlockPickerItem, { kind: "template" }> => item.kind === "template");
...
export function isSlotFull(context: StructurePickerGroupContext, parent: EditorStructureNode, slot: ContentSlot): boolean {
