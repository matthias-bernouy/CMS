...
export function visibleStructureNodes(
    ...
    return nodes.flatMap((node) => {
        ...
        return [...current, ...visibleStructureNodes(node.children, isCollapsed, depth + 1)];
    });
}
...
export function flattenStructureNodes(nodes: StructureNode[]): StructureNode[] {
    return nodes.flatMap((node) => [node, ...flattenStructureNodes(node.children)]);
}

export function editorChildrenOf(parent: EditorStructureNode): EditorStructureNode[] {
    return parent.children;
}

export function isDescendantStructureNode(candidate: EditorStructureNode, parent: EditorStructureNode): boolean {
