...
export function repeatArrayOptions(scopes: DataScope[]): RepeatOption[] {
    ...
    for (const option of scopes.flatMap((scope) =>
        repeatArrayFields(scope.fields, scope.name, scope.label ?? scope.name),
    )) {
...
export function repeatArrayFields(
    fields: DataField[],
    scopeName: string,
    scopeLabel: string,
    prefix = "",
    ...
    return fields.flatMap((field) => {
        const relativePath =
            prefix && field.path !== "." ? `${prefix}.${field.path}` : field.path === "." ? prefix : field.path;
...
export function visibleRepeatOptions(options: RepeatOption[], query: string): RepeatOption[] {
    ...
    return options.filter((option) =>
        [option.path, option.label, option.scopeLabel].some((value) => value.toLowerCase().includes(normalizedQuery)),
    );
}
