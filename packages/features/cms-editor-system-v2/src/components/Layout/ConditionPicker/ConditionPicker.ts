...
import type { ConditionFieldOption, ConditionPickerApplyDetail, ConditionPickerCondition, ConditionPickerMode, ConditionPickerSource, FieldConditionDraft } from "./types";

export type { ConditionFieldOption, ConditionPickerApplyDetail, ConditionPickerCondition, ConditionPickerSource } from "./types";
...
export class ConditionPicker extends HTMLElement {
    ...
    connectedCallback(): void { this.ownerDocument.addEventListener("keydown", this.onKeydown); }
    disconnectedCallback(): void { this.ownerDocument.removeEventListener("keydown", this.onKeydown); }
    ...
    private renderModes(): HTMLElement {
        ...
        group.append(this.modeButton("source", "Source state"), this.modeButton("field", "Data field"), this.modeButton("advanced", "Advanced"));
        return group;
    }
    ...
    private modeButton(mode: ConditionPickerMode, label: string): HTMLButtonElement {
        ...
        button.addEventListener("click", () => { this._mode = mode; this.render(); });
        return button;
    }

    private renderPanel(): HTMLElement {
        if (this._mode === "field") return renderFieldMode(this._fields, this._fieldDraft, render => render ? this.render() : this.syncSummary());
        if (this._mode === "advanced") return renderAdvancedMode(this._advancedExpression, value => { this._advancedExpression = value; this.syncSummary(); });
    ...
    private summaryLabel(): HTMLElement { const label = document.createElement("span"); label.textContent = "Expression"; return label; }

    private summaryCode(): HTMLElement { const code = document.createElement("code"); code.textContent = this.currentExpression() || "No condition selected."; return code; }
    ...
    private syncActions(): void { this.applyButton.disabled = !this.canApply(); this.removeButton.disabled = !this._canRemove; }

    private currentExpression(): string {
        if (this._mode === "advanced") return this._advancedExpression.trim();
        if (this._mode === "field" && this._fieldDraft.path) return fieldExpression(this._fieldDraft);
    ...
    private canApply(): boolean {
        if (this._mode === "source") return this._selected.size > 0;
        return this.currentExpression().trim().length > 0;
    }

    private readonly apply = (): void => {
        if (!this.canApply()) return;
        const detail = this._mode === "source"
            ? { conditions: selectedSourceConditions(this._sources, this._selected) }
            : { conditions: [], expression: this.currentExpression().trim() };
        this.dispatchEvent(new CustomEvent<ConditionPickerApplyDetail>(CONDITION_PICKER_APPLY_EVENT, { bubbles: true, composed: true, detail }));
        this.close();
    };
    ...
    private readonly onBackdropClick = (event: Event): void => { if (event.target === this.backdrop) this.close(); };
    private readonly onKeydown = (event: KeyboardEvent): void => { if (!this.backdrop.hidden && event.key === "Escape") this.close(); };

    private get backdrop(): HTMLElement { return this.shadowRoot!.querySelector(".backdrop")!; }
    private get closeButton(): HTMLButtonElement { return this.shadowRoot!.querySelector(".close")!; }
    private get subtitle(): HTMLElement { return this.shadowRoot!.querySelector(".subtitle")!; }
    private get body(): HTMLElement { return this.shadowRoot!.querySelector(".body")!; }
    private get summary(): HTMLElement { return this.shadowRoot!.querySelector(".summary")!; }
    private get applyButton(): HTMLButtonElement { return this.shadowRoot!.querySelector(".apply")!; }
    private get removeButton(): HTMLButtonElement { return this.shadowRoot!.querySelector(".remove")!; }
}
