...
import inlineRichTextCss from "../Styles/inlineRichText.css" with { type: "text" };
import templateHtml from "../template.html" with { type: "text" };
import componentCss from "../style.css" with { type: "text" };

export function createShellTemplate(): HTMLTemplateElement {
    const template = document.createElement("template");
    template.innerHTML = `<style>${[componentCss, inlineRichTextCss, pageSettingsCss, pageSettingsTagsCss]
        .map((css) => String(css))
