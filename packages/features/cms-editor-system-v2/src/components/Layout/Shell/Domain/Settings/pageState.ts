...
export function applyPageStateSetting(
    ...
    if (setting.attribute === PAGE_STATE_ENABLE_SETTING) {
        ...
        if (isValidValueKey(next)) target.setAttribute(CMS_BINDING_ATTRIBUTES.pageState, next);
        return true;
    }
...
export function pageStateSettings(editor: Editor): SettingSection | null {
    ...
    if (isEnabled && hasFieldName) settings.push({
        type: "toggle",
    ...
    if (isEnabled && !usesFieldName) settings.push({
        type: "text",
        label: "State key",
