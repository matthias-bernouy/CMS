...
export function dashboardRelationProjectionId(
    projection: Pick<DashboardRelationProjection, "dashboardId" | "viewId" | "relationId">,
): string {
...
export function validateDashboardRelationProjection(projection: DashboardRelationProjection): string[] {
    ...
    if (!isRecord(projection)) {
        return ["dashboardRelationProjection must be an object"];
    }
    if (projection.type !== "dashboardRelation") {
        errors.push("dashboardRelationProjection.type must be dashboardRelation");
    }
    ...
    if (projection.placement !== undefined) {
        validateEnum("dashboardRelationProjection.placement", projection.placement, PLACEMENTS, errors);
    }
    ...
    if (
        projection.pageSize !== undefined &&
        (!Number.isInteger(projection.pageSize) || projection.pageSize < 1 || projection.pageSize > MAX_RELATION_LIMIT)
    ) {
...
function validateProjectionColumns(columns: RelationDashboardColumn[] | undefined, errors: string[]): void {
    ...
    if (!Array.isArray(columns)) {
        return void errors.push("dashboardRelationProjection.columns must be an array");
    }
    ...
    for (const [index, column] of columns.entries()) {
        ...
        if (column.width !== undefined && (typeof column.width !== "string" || !column.width.trim())) {
            errors.push(`${path}.width must be non-empty when provided`);
        }
        if (column.format !== undefined) {
            validateEnum(`${path}.format`, column.format, FORMATS, errors);
        }
    }
}
...
function validateProjectionActions(actions: RelationDashboardAction[] | undefined, errors: string[]): void {
    ...
    if (!Array.isArray(actions)) {
        return void errors.push("dashboardRelationProjection.actions must be an array");
    }
    const seen = new Set<string>();
    for (const [index, action] of actions.entries()) {
        ...
        if (action.tone !== undefined) {
            validateEnum(`${path}.tone`, action.tone, ACTION_TONES, errors);
        }
        if (action.placement !== undefined) {
            validateEnum(`${path}.placement`, action.placement, ACTION_PLACEMENTS, errors);
        }
...
function validateProjectionActionEndpoint(action: RelationDashboardAction, path: string, errors: string[]): void {
    ...
    if (action.endpoint.params !== undefined) {
        validateExpressionMap(action.endpoint.params, `${path}.endpoint.params`, errors, RELATION_ACTION_EXPR);
    }
    if (action.endpoint.body !== undefined) {
        validateExpressionMap(action.endpoint.body, `${path}.endpoint.body`, errors, RELATION_ACTION_EXPR);
    }
}
