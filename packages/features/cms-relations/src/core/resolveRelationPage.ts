...
export async function resolveRelationPage(
    ...
    if (!relation.page) throw new RelationResolutionError(`relation "${relation.id}" does not declare pagination`, 500);
    ...
    const endpoint = await deps.sources.getEndpoint(makeEndpointUrn(relation.binding.endpoint.sourceId, relation.binding.endpoint.endpointId));
    if (!endpoint) {
        throw new RelationResolutionError(`relation endpoint not found: ${relation.binding.endpoint.sourceId}.${relation.binding.endpoint.endpointId}`, 404);
    }
    if (deps.callerAccessMode && !sourceEndpointAccessAllows(sourceEndpointAccessMode(endpoint), deps.callerAccessMode)) {
    ...
    if (!response.ok) {
        throw new RelationResolutionError(await response.text() || `relation endpoint failed (${response.status})`, response.status);
    }
    ...
    const nextCursor = relation.page.nextCursorPath ? textAt(data, relation.page.nextCursorPath) || undefined : undefined;
    const result: RelationPageResult = {
        items,
        limit: page.limit,
    };
    if (total !== undefined) result.total = total;
    if (page.offsetApplied && page.offset !== undefined) result.offset = page.offset;
