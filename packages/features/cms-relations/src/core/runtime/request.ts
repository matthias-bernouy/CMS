...
export function relationRequestParams(
    ...
    if (page?.limitParam) params[page.limitParam] = String(pageRequest.limit);
    if (page?.offsetParam && pageRequest.offset !== undefined) params[page.offsetParam] = String(pageRequest.offset);
    if (page?.cursorParam && pageRequest.cursor) params[page.cursorParam] = pageRequest.cursor;
    return params;
}
...
export function normalizedPageRequest(
    ...
    return {
        ...
        offsetApplied: offset !== undefined && (
            Boolean(relation.page?.offsetParam)
            || Object.values(binding.params).includes("$page.offset")
        ),
    };
}
...
function resolveRelationExpression(expression: string, fromItem: unknown, pageRequest: NormalizedPageRequest): unknown {
    ...
    if (expression === "$from") return fromItem;
    if (expression.startsWith("$from.")) return valueAt(fromItem, expression.slice("$from.".length));
    if (expression === "$page.limit") return pageRequest.limit;
    if (expression === "$page.offset") return pageRequest.offset;
    if (expression === "$page.cursor") return pageRequest.cursor;
    return undefined;
}
