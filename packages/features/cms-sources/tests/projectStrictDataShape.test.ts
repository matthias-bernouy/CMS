...
describe("projectStrictDataShape", () => {
    test("accepts root null only when the shape explicitly declares it nullable", () => {
        ...
        expect(() => projectStrictDataShape(null, { type: "string" }))
            .toThrow(DataShapeProjectionError);
        expect(() => projectStrictDataShape(null, { type: "string", nullable: false }))
            .toThrow(DataShapeProjectionError);
    });
    ...
    test("rejects present null values unless their nested shape is nullable", () => {
        expect(() => projectStrictDataShape({ value: null }, {
            type: "object",
            properties: { value: { type: "string" } },
        })).toThrow("body.value must be a string");

        expect(() => projectStrictDataShape([1, null], {
            type: "array",
