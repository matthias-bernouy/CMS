...
describe("validateSource", () => {
    test("validates source and endpoint identity", () => {
        ...
        expect(
            validateSource(source({ urn: "urn:system-custom", endpoints: [ep("urn:system-custom:x")] })).some((e) =>
                e.includes("reserved"),
            ),
        ).toBe(true);
        expect(
            validateSource(source({ endpoints: [ep("urn:other:x")] })).some((e) => e.includes("does not belong")),
        ).toBe(true);
        expect(
            validateSource(source({ endpoints: [ep("urn:shop")] })).some((e) => e.includes("invalid endpoint urn")),
        ).toBe(true);
        expect(
            validateSource(source({ endpoints: [ep("urn:shop:a"), ep("urn:shop:a")] })).some((e) =>
    ...
    test("validates endpoint method and access mode", () => {
        expect(
            validateSource(source({ endpoints: [{ ...ep("urn:shop:x"), method: "FETCH" as any }] })).some((e) =>
                e.includes("invalid method"),
            ),
        ).toBe(true);
        ...
        expect(
            validateSource(source({ endpoints: [{ ...ep("urn:shop:x"), access: { mode: "visitor" as any } }] })).some(
                (e) => e.includes("invalid access mode"),
            ),
        ).toBe(true);
    });
                    ...
                    endpoints: [
                        {
                            ...ep("urn:shop:x"),
                            input: {
                                params: [{ name: "q", in: "cookie" as any, schema: { type: "string" as const } }],
                            },
                        },
                    ],
                }),
            ).some((e) => e.includes("invalid param location")),
        ).toBe(true);
        expect(
            validateSource(
                source({ endpoints: [{ ...ep("urn:shop:x"), input: { params: [duplicate, { ...duplicate }] } }] }),
            ).some((e) => e.includes("duplicate param")),
        ).toBe(true);
        expect(
            validateSource(
                source({
                    endpoints: [
                        {
                            ...ep("urn:shop:x"),
                            input: {
                                params: [{ name: "Host", in: "header" as const, schema: { type: "string" as const } }],
                            },
                        },
                    ],
                }),
            ).some((e) => e.includes("forbidden or invalid header param")),
        ).toBe(true);
    });
            ...
            input: {
                params: [
                    ...
                    },
                    {
                        name: "X-User-Role",
                        in: "header" as const,
                        source: { from: "computed" as const, ref: "userRole" as const },
                        schema: { type: "string" as const },
                    },
                ],
            },
    ...
    test("validates duplicate headers, secret prefixes, and responses", () => {
        ...
        expect(
            ...
            ).some((e) => e.includes("missing response contract")),
        ).toBe(true);
    });
    ...
    test("accepts file endpoints and fully-furnished endpoints", () => {
        expect(
            validateSource(
                source({ endpoints: [{ ...ep("urn:shop:file"), responseKind: "file", mediaType: "image/*" }] }),
            ),
        ).toEqual([]);
        const endpoint = {
            ...ep("urn:shop:x"),
