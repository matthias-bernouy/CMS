...
describe("dynamic source overlay contracts", () => {
    ...
    test.each([
        ...
        ["a network failure", async () => { throw new Error("network unavailable"); }],
        ["an invalid JSON response", async () => new Response("not-json", {
    ...
    test("materializes every concurrent lookup result without fixing the current request count", async () => {
        ...
        const endpoints = await Promise.all(Array.from(
            { length: 4 },
            () => repository.getEndpoint(targetEndpoint.urn),
        ));
