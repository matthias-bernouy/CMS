...
describe("executeEndpoint proxy", () => {
    test("forwards to the built upstream URL with only declared query params", async () => {
        ...
        const endpoint = ep({ input: { params: [{ name: "lat", in: "query", required: true, schema: { type: "number" } }] } });
    ...
    test("upstream body and failures are proxied", async () => {
        ...
        expect((await executeEndpoint(ep(), new Request("http://local/x"), { fetchImpl: errorFetch })).status).toBe(502);
    });
