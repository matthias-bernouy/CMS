...
export async function buildForwardHeaders(
    ...
    if (!paramHeaders.ok) return paramHeaders;
...
function applyHeaderParams(headers: Headers, builtHeaders: Record<string, string>): HeaderResult {
    for (const [name, value] of Object.entries(builtHeaders)) {
        ...
        catch { return { ok: false, response: new Response(`invalid header param: "${name}"`, { status: 400 }) }; }
    }
...
async function applyConfiguredHeaders(
    ...
    for (const { name, source } of endpoint.headers ?? []) {
        ...
        if (source.from === "secret") {
            ...
            try { headers.set(name, result.value); }
            catch { return { ok: false, response: new Response(`invalid header: "${name}"`, { status: 400 }) }; }
            continue;
        }
        const value = source.from === "computed" ? computedValue(source.ref, computed) : source.value;
        if (value == null) return { ok: false, response: new Response(`computed header unavailable: "${name}"`, { status: 401 }) };
        try { headers.set(name, value); }
        catch { return { ok: false, response: new Response(`invalid header: "${name}"`, { status: 400 }) }; }
    }
    return { ok: true, headers };
...
async function secretHeaderValue(name: string, ref: string, prefix: string | undefined, deps: HeaderDeps | undefined) {
    if (!deps?.resolveSecret) {
        return { ok: false as const, response: new Response(`secret header requires a configured secret store (not wired yet): ${name}`, { status: 500 }) };
    }
    const value = await deps.resolveSecret(ref);
    if (value == null) return { ok: false as const, response: new Response(`secret not found: ${ref}`, { status: 500 }) };
