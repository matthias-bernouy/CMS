...
export function validateTriggerResponse(endpoint: SourceEndpoint, response: EndpointResponse, errors: string[]): void {
    ...
    if (!isStructuredObject(response.body)) {
        errors.push(
            `public response body must be a structured non-null object when trigger fields exist for "${endpoint.urn}": "${response.status}"`,
        );
    }
    if (!isStructuredTriggerBody(triggerBody)) {
        errors.push(
            `trigger response body must contain only structured object fields for "${endpoint.urn}": "${response.status}"`,
        );
        return;
    }
