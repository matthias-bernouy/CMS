...
export async function bindResponseIdentities(
    ...
    if (!bindings.length || !response.ok) {
        return null;
    }
    if (!identities) {
    ...
    if (!subjectId) {
        return new Response("identity binding requires an authenticated user", { status: 401 });
    }
    const parsed = await readIdentityBindingPayload(response);
    if (!parsed.ok) {
        return new Response("identity binding response is invalid", { status: 502 });
    }
    ...
    try {
        ...
        for (const binding of bindings) {
            ...
            if (!authority) {
                return new Response(`identity binding path is not a qualified user-id: ${binding.responsePath}`, {
                    status: 500,
                });
            }
...
function responseShape(endpoint: SourceEndpoint, status: number) {
    return (
        endpoint.output?.find((output) => output.status === String(status))?.body ??
...
async function readIdentityBindingPayload(response: Response): Promise<{ ok: true; payload: unknown } | { ok: false }> {
    try {
