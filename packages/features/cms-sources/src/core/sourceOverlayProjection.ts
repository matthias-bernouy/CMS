...
export function applySourceOverlays(source: Source, overlays: readonly SourceOverlay[]): Source {
    ...
    if (!relevant.length) return structuredClone(source);

    const next = structuredClone(source);
    for (const overlay of relevant) {
        for (const endpoint of next.endpoints) applyEndpointOverlay(endpoint, overlay);
    }
    return next;
}
...
function applyEndpointOverlay(endpoint: SourceEndpoint, overlay: SourceOverlay): void {
    ...
    for (const target of overlay.output ?? []) {
        ...
        for (const response of endpoint.output ?? []) {
            if (response.body) response.body = withExtraFieldsShape(response.body, target, fields);
        }
    }
}
...
function withExtraFieldsShape(
    ...
    if (objectShape) attachExtraFields(objectShape, fields);
    return next;
}

function objectShapeAt(shape: DataShape, path: string): DataShape | null {
    let current: DataShape | undefined = shape;
    for (const part of path.split(".").map(part => part.trim()).filter(Boolean)) {
...
function attachExtraFields(shape: DataShape, fields: readonly SourceOverlayField[]): void {
    ...
    for (const field of fields) attachExtraField(shape, sourceOverlayFieldPath(field), field);
}

function attachExtraField(shape: DataShape, path: string, field: SourceOverlayField): void {
    const parts = path.split(".").map(part => part.trim()).filter(Boolean);
    if (!parts.length || shape.type !== "object") return;

    let current = shape;
    for (const part of parts.slice(0, -1)) {
        current.properties ??= {};
        const existing = current.properties[part];
        if (!existing || existing.type !== "object") current.properties[part] = { type: "object", properties: {} };
        current = current.properties[part]!;
    }
    ...
    if (field.required) required.add(leaf);
    else required.delete(leaf);
    current.required = [...required];
    if (!current.required.length) delete current.required;
}

function editableFields(fields: readonly SourceOverlayField[], editable: SourceOverlayEditableScope | undefined): SourceOverlayField[] {
