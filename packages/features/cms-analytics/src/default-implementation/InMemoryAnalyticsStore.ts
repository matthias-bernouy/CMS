...
export class InMemoryAnalyticsStore implements AnalyticsStore {
    ...
    async record(event: AnalyticsEvent): Promise<void> {
        ...
        for (const w of eventToWrites(event)) {
            this._merge(w);
        }
    ...
    private _rows(metric: string, dim: string, from: Date, to: Date): RollupUpsert[] {
        return [...this._rollups.values()].filter(
            (r) => r.metric === metric && r.dim === dim && r.bucket >= from && r.bucket < to,
        );
    }
    ...
    async summary(from: Date, to: Date): Promise<AnalyticsSummary> {
        ...
        const errors = this._rows("pv", "status", from, to)
            .filter((r) => r.key >= "400" && r.key < "600")
            .reduce((n, r) => n + r.count, 0);
        return {
            views,
            uniqueVisitors,
            avgMs: views ? Math.round(msSum / views) : 0,
            errorRate: views ? errors / views : 0,
        };
    }
    ...
    async timeseries(q: RangeQuery): Promise<TimeBucket[]> {
        ...
        return [...acc.entries()]
            ...
            .map(([t, a]) => ({
                bucket: new Date(t),
                count: a.count,
                avgMs: a.count ? Math.round(a.msSum / a.count) : 0,
                maxMs: a.maxMs,
            }));
    }
    ...
    private async _top(dim: string, from: Date, to: Date, limit: number): Promise<KeyCount[]> {
        ...
        for (const r of this._rows("pv", dim, from, to)) {
            byKey.set(r.key, (byKey.get(r.key) ?? 0) + r.count);
        }
