...
const BOT_RE = /bot|crawl|spider|slurp|mediapartners|facebookexternalhit|embedly|bingpreview|whatsapp|telegram|ia_archiver|lighthouse|headless|pingdom|uptimerobot/i;
...
export function classifyUserAgent(ua: string | undefined): UserAgentClass {
    ...
    if (!s) return { device: 'other', browser: 'other' };
    if (BOT_RE.test(s)) return { device: 'bot', browser: detectBrowser(s) };
