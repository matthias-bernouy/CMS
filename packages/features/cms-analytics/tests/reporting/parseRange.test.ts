...
import { parseRange } from "cms-analytics/core/reporting/parseRange";
...
describe("parseRange", () => {
    test("24h → hourly buckets, 24h window", () => {
        ...
        expect(r.to.toISOString()).toBe("2026-06-04T12:00:00.000Z");
    ...
    test("7d → daily buckets, 7-day window", () => {
        ...
        expect(r.to.toISOString()).toBe("2026-06-04T00:00:00.000Z");
        expect(r.from.toISOString()).toBe("2026-05-28T00:00:00.000Z");
    });

    test("30d → daily buckets, 30-day window", () => {
        const r = parseRange("30d", NOW);
        expect(r.interval).toBe("day");
        expect(r.from.toISOString()).toBe("2026-05-05T00:00:00.000Z");
    });
