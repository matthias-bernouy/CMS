...
describe("parseRange", () => {
    test("24h → hourly buckets, 24h window", () => {
        ...
        expect(r.from.toISOString()).toBe("2026-06-03T13:00:00.000Z");
    });

    test("7d → daily buckets, 7-day window", () => {
        const r = parseRange("7d", NOW);
        expect(r.interval).toBe("day");
        expect(r.from.toISOString()).toBe("2026-05-29T00:00:00.000Z");
    });

    test("30d → daily buckets, 30-day window", () => {
        const r = parseRange("30d", NOW);
        expect(r.interval).toBe("day");
        expect(r.from.toISOString()).toBe("2026-05-06T00:00:00.000Z");
    });
