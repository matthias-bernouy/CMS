...
export type GatewayTargetUrlValidationOptions = {
...
export function validateGatewayTargetUrl(value: string, opts: GatewayTargetUrlValidationOptions = {}): TargetUrlVerdict {
    let url: URL;
    try {
        url = new URL(value);
    } catch {
...
export function isAllowedGatewayTargetUrl(value: string, opts?: GatewayTargetUrlValidationOptions): boolean {
    return validateGatewayTargetUrl(value, opts).ok;
}
...
export function validateProvider(provider: Provider): string[] {
    ...
    for (const ep of provider.endpoints) {
        ...
        const target = validateGatewayTargetUrl(ep.targetUrl);
        if (!target.ok) {
