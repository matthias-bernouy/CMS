import type { Endpoint, EndpointParam, EndpointResponse, HTTPMethod, ParamIn, Provider } from "../../interfaces/Gateway";
import { makeEndpointUrn, makeProviderUrn } from "../urn";
import { isValidResponseStatus, validateProvider } from "../validateProvider";
...
export type OpenApiProviderImportOptions = {
    providerId: string;
    baseUrl?: string;
    includeOperation?: (operation: OpenApiImportOperation) => boolean;
};

export function openApiSpecToProvider(raw: string, options: OpenApiProviderImportOptions): Provider {
    const providerId = options.providerId.trim();
    if (!providerId) throw new SpecParseError("providerId is required.");
    ...
    const endpoints = selected
        .map(({ slim, op }) => endpointFromOperation(providerId, uniqueId(op?.operationId || slim.id, usedIds), slim, op, resolver, baseUrl))
        .filter((e): e is Endpoint => e !== null);
    const provider: Provider = {
        urn: makeProviderUrn(providerId),
        meta: {
            name: spec.info.title || providerId,
            ...(spec.info.version ? { description: `Version ${spec.info.version}` } : {}),
        },
        endpoints,
    };
    const errors = validateProvider(provider);
    if (errors.length) throw new SpecParseError(`Imported provider is invalid:\n  - ${errors.join("\n  - ")}`);
    return provider;
}
...
function endpointFromOperation(
    ...
    const endpoint: Endpoint = {
        urn: makeEndpointUrn(providerId, endpointId),
