...
export class MongoCmsFilesMetadata implements CmsFilesMetadataRepository {
    ...
    private get col(): Collection<ItemDoc> {
        return this.db.collection<ItemDoc>(this._prefix + "filesMeta");
    }
    ...
    async updateItem(id: string, patch: ItemPatch): Promise<FilesItem | null> {
        ...
        if (patch.parentId !== undefined && patch.parentId !== cur.parentId) {
            ...
            if (cur.type === "folder" && patch.parentId !== null) {
                await this._assertNotIntoOwnSubtree(id, patch.parentId);
            }
        }
    ...
    private async _insert(doc: ItemDoc): Promise<void> {
        try {
            await this.col.insertOne(doc as OptionalUnlessRequiredId<ItemDoc>);
        } catch (e) {
            throw clashOr(e);
        }
    }
