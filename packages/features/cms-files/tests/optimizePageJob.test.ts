...
describe("optimizePageImages", () => {
    test("generates variants + manifest for raster images; skips SVG, hashless files, and unknown ids", async () => {
        ...
        const svg = await metadata.createFile({
            name: "b.svg",
            parentId: null,
            size: 1,
        ...
        const hashless = await metadata.createFile({ name: "c.png", parentId: null, size: 1, mimeType: "image/png" });

        await optimizePageImages(
            { metadata, sourceBlob, variantStore },
            [img.id, svg.id, hashless.id, "ghost"],
            [200, 999],
        );
