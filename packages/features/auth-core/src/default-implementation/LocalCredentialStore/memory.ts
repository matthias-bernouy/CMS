...
import type { LocalCredentialStore, LocalCredential, NewCredential } from "auth-core/interfaces/LocalCredentialStore";
import { dummyPasswordVerify } from "auth-core/auth/passwordTiming";
...
export class InMemoryLocalCredentialStore implements LocalCredentialStore {
    ...
    async verify(email: string, password: string): Promise<Identity | null> {
        ...
        return (await Bun.password.verify(password, rec.hash))
...
const strip = (r: Record): LocalCredential => ({ sub: r.sub, email: r.email, createdAt: r.createdAt, updatedAt: r.updatedAt });
