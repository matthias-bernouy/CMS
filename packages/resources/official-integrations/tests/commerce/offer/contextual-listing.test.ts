...
describe("commerce contextual offer listing", () => {
    test("validates contextual filters in PostgreSQL before enriching the page", async () => {
        setRestResponder(request => {
            ...
            if (table === "custom_field_definitions") return jsonResponse([]);
            if (table === "sellers") return jsonResponse([{ id: 7, display_name: "Seller" }]);
            if (table === "products") return jsonResponse([{ id: 42, brand_id: 4, title: "Blade", metadata: { weight: 305 } }]);
            if (table === "brands") return jsonResponse([{ id: 4, slug: "wilson", name: "Wilson" }]);
            if (table === "product_categories") return jsonResponse([{
                product_id: 42, category_id: 8, is_primary: true,
        ...
        const response = await requestCommerce(`/offers?category=rackets/tennis&brand=wilson&filters=${filters}&limit=12`);
