...
describe("commerce protected C2C claims and refunds", () => {
    ...
    test("keeps business key and financial allocation inside Commerce", async () => {
        ...
        expect(expectSingleRpc("request_order_refund").body).toEqual({
            p_order_id: 42,
            p_reason: "support_resolution",
            p_requested_amount: 8_000,
            p_actor_kind: "support",
            p_actor_id: "finance-7",
        });
    });

    test("requires optimistic concurrency when finance reviews a refund", async () => {
    ...
    test("fails closed when a finance action lacks the finance role or actor", async () => {
        const body = {
            refundRequestId: 19,
            decision: "approved",
            reason: "reviewed",
        ...
        for (const userRole of [undefined, "admin", "support"]) {
            const response = await requestCommerce("/admin/refund/review", {
                userId: "actor-7",
                userRole,
                body,
    ...
    test("does not let support impersonate a system release actor", async () => {
