...
export function expectedC2cSourceResponse(response: Row): Row {
    ...
    return {
        ...
        activePolicy: {
            id: active.id, policyKey: active.policyKey, version: active.version,
            name: active.name, status: active.status, currency: active.currency,
            fee: active.fee,
            ...
            subsidy: active.subsidy === null
                ? null
                : sourceSubsidy(active.subsidy as Row, true),
        },
        feePolicy: sourceFields(response.feePolicy as Row, [
            "id", "status", "costEstimatesConfigured", "subsidyOverride",
        ]),
        protectionPolicy: sourceFields(response.protectionPolicy as Row, [
            "id", "status", "paymentWindowMinutes", "claimWindowHours",
            "financeReviewThresholdAmount",
        ]),
        sellerRiskPolicy: sourceFields(response.sellerRiskPolicy as Row, [
            "id", "status", "payoutDelayDays", "reserveLiabilityDays",
        ]),
...
function feeOutput(row: Row): Row {
    return {
        id: row.id, policyKey: row.policy_key, version: row.version,
        name: row.name, status: row.status, currency: row.currency,
        ...
        subsidyOverride: row.subsidy_override, subsidyReason: row.subsidy_reason,
        publishedAt: row.published_at, createdBy: row.created_by, createdAt: row.created_at,
    };
}
...
function protectionOutput(row: Row): Row {
    return {
        ...
        sellerHandoffHours: row.seller_handoff_hours, scanGraceHours: row.scan_grace_hours,
        claimWindowHours: row.claim_window_hours, sellerResponseHours: row.seller_response_hours,
...
function riskOutput(row: Row): Row {
    return {
        id: row.id, policyKey: row.policy_key, version: row.version,
        name: row.name, status: row.status, currency: row.currency,
        reserveRateBps: row.reserve_rate_bps, payoutDelayDays: row.payout_delay_days,
...
function componentOutput(row: Row): Row {
    return {
        id: row.id, feePolicyId: row.fee_policy_id, componentKey: row.component_key,
        payer: row.payer, basis: row.basis, rateBps: row.rate_bps,
        fixedAmount: row.fixed_amount, minimumAmount: row.minimum_amount,
        maximumAmount: row.maximum_amount, roundingMode: row.rounding_mode,
        refundPolicy: row.refund_policy, position: row.position, createdAt: row.created_at,
    };
}

function subsidyOutput(row: Row): Row {
    return {
        id: row.id, feePolicyId: row.fee_policy_id,
        maximumDeficitAmount: row.maximum_deficit_amount,
        reason: row.reason, approvedBy: row.approved_by,
...
function sourceComponent(row: Row): Row {
    return sourceFields(row, [
        "id", "componentKey", "payer", "basis", "rateBps", "fixedAmount",
        "minimumAmount", "maximumAmount", "roundingMode", "refundPolicy",
    ]);
}

function sourceSubsidy(row: Row, includeApprover = false): Row {
    return sourceFields(row, [
        "id", "maximumDeficitAmount", "reason",
