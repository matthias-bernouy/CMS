...
export type ReturnAuthorizationResource =
    | "claim"
    | "order"
    | "seller"
    | "financialTerms";
...
export function useReturnAuthorizationResponder(
    options: ReturnAuthorizationOptions = {},
): void {
    setRestResponder(request => {
        const path = new URL(request.url).pathname;
        if (path.endsWith("/rest/v1/rpc/get_claim_return_authorization_context")) {
            if (options.failure) return upstreamFailure(options);
            return jsonResponse(returnAuthorizationEnvelope(options));
        }

        const resource = resourceForPath(path);
        if (!resource) throw new Error(`unexpected return-authorization request ${request.url}`);
        if (options.failure === resource) return upstreamFailure(options);
        return jsonResponse([rowForResource(resource, options)].filter(value => value !== null));
    });
}

export function returnAuthorizationEnvelope(
    options: ReturnAuthorizationOptions = {},
): JsonRecord {
...
function rowForResource(
    resource: ReturnAuthorizationResource,
    options: ReturnAuthorizationOptions,
): JsonRecord | null {
    if (resource === "claim") return option(options.claim, claimRow);
    if (resource === "order") return option(options.order, orderRow);
    if (resource === "seller") return option(options.seller, sellerRow);
...
function resourceForPath(path: string): ReturnAuthorizationResource | null {
    if (path.endsWith("/rest/v1/marketplace_claims")) return "claim";
    if (path.endsWith("/rest/v1/orders")) return "order";
    if (path.endsWith("/rest/v1/sellers")) return "seller";
    if (path.endsWith("/rest/v1/order_financial_terms")) return "financialTerms";
    return null;
}
