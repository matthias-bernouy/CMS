...
import {
    ...
    executeClaimTracking,
    loadClaimTrackingFunction,
} from "./harness";
...
describe("Commerce Mondial Relay claim return tracking boundaries", () => {
    test("keeps authenticated access and validates selectors before source work", async () => {
        ...
        const missing = await executeClaimTracking(
            successfulResponder(),
            {
                request: new Request(
                    "https://cms.test/functions/getClaimReturnForMe",
                ),
            },
        );
        const invalid = await executeClaimTracking(
            successfulResponder(),
            { request: claimTrackingRequest("invalid") },
        );
        ...
        const decimal = await executeClaimTracking(
            successfulResponder(),
            { request: claimTrackingRequest(7.5) },
        );
        await expectGenericFailure(decimal.response);
        expect(decimal.calls.map(call => call.url.pathname)).toEqual([
