...
describe("commerce response contracts", () => {
    test("declares usable fields for every structured JSON response", async () => {
        ...
        expect(rootFields(endpoints, "product")).toEqual(expect.arrayContaining(["id", "media", "variantAxes", "variantMatrix"]));
        expect(itemFields(endpoints, "offers")).toEqual(expect.arrayContaining(["id", "productId", "acceptedPriceAmount"]));
        expect(itemFields(endpoints, "offers")).not.toContain("sellerId");
        expect(rootFields(endpoints, "offer")).toEqual(expect.arrayContaining(["product", "priceRule", "priceProposals"]));
        ...
        expect(rootFields(endpoints, "myOrder")).toEqual(expect.arrayContaining([
            "shippingAmount", "deliveryQuotedAt", "financialTerms",
        ]));
        expect(nestedFields(endpoints, "myOrder", "financialTerms")).toEqual(expect.arrayContaining([
        ...
        expect(itemFields(endpoints, "mySales")).toEqual(expect.arrayContaining(["id", "shippingAmount", "totalAmount"]));
        expect(rootFields(endpoints, "mySale")).toEqual(expect.arrayContaining(["id", "lines", "events", "financialTerms"]));
        expect(nestedFields(endpoints, "mySale", "financialTerms")).toEqual(expect.arrayContaining([
            "orderId", "merchandiseSubtotalAmount", "shippingAmount", "sellerCommissionAmount",
            "platformShippingShareAmount", "sellerShippingShareAmount", "sellerProceedsAmount", "sellerTransferReleaseAmount",
            "sellerReserveLiabilityAmount", "currency", "financialRevision",
        ]));
        expect(rootFields(endpoints, "mySale")).not.toEqual(expect.arrayContaining([
            "sellerId", "buyerCmsUserId", "shippingAddress", "billingAddress", "idempotencyKey",
        ]));
    });
    ...
    test("declares structured metadata entries on every buyer and seller order response", async () => {
        ...
        expect(nestedArrayItemFields(endpoints, "checkoutMyCart", "orders")).toEqual(expect.arrayContaining([
            "metadata", "metadataEntries",
        ]));
        expect(nestedItemMetadataEntryFields(endpoints, "checkoutMyCart", "orders")).toEqual(expect.arrayContaining([
            "key", "label", "type", "value", "unit",
        ]));
        for (const endpointId of ["myOrders", "mySales"]) {
            expect(itemMetadataEntryFields(endpoints, endpointId)).toEqual(expect.arrayContaining([
                "key", "label", "type", "value", "unit",
            ]));
        }
        for (const endpointId of ["myOrder", "mySale", "createOrder"]) {
            expect(metadataEntryFields(endpoints, endpointId)).toEqual(expect.arrayContaining([
                "key", "label", "type", "value", "unit",
            ]));
        }
    });
    ...
    test("preserves the exact buyer price breakdown through strict source projection", async () => {
        ...
        }, responseBody(endpoints, "myOrder"), "response", { enforceRequired: false });
        ...
        const lockedTerms = projectStrictDataShape({
            ...
            financialRevision: 1,
        }, responseBody(endpoints, "lockOrderFinancialTerms"), "response", { enforceRequired: false });
    ...
    test("projects the immutable seller proceeds snapshot without the buyer total", async () => {
        ...
        }, responseBody(endpoints, "mySale"), "response", { enforceRequired: false });
    ...
    test("returns the paging values declared by every paginated list", async () => {
        ...
        const routes = [
            "/products", "/offers", "/me/offers", "/me/orders", "/me/sales",
            "/admin/products", "/admin/product/variants?productId=1",
            "/admin/sellers", "/admin/offers", "/admin/orders",
        ];
...
function responseBody(endpoints: Endpoint[], endpointId: string): Shape {
    ...
    if (!body) throw new Error(`Missing output body for ${endpointId}`);
    return body;
}
