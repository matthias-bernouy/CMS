...
describe("commerce custom-field overlays", () => {
    test("keeps Product admin metadata contextual while projecting public metadata", async () => {
        ...
        const byEntity = Object.fromEntries(dynamicOverlays.map(overlay => [
            overlay.fieldSource?.params?.entityType ?? "",
            overlay,
        ]));
        ...
        expect(byEntity.product.output).toEqual(expect.arrayContaining([
            { endpointId: "product" },
            { endpointId: "products", path: "items[]" },
            { endpointId: "offer", path: "product" },
            { endpointId: "manageOffer", path: "product" },
        ]));
        expect(byEntity.product.output).not.toEqual(expect.arrayContaining([
            { endpointId: "manageProduct" },
            { endpointId: "upsertProduct" },
        ]));
        expect(byEntity.offer.input).toEqual(expect.arrayContaining([
            { endpointId: "updateMyOffer", editable: "self" },
            { endpointId: "upsertOffer", editable: "admin" },
        ]));
        expect(byEntity.seller.input).toEqual(expect.arrayContaining([
            { endpointId: "registerMySeller", editable: "self" },
            { endpointId: "updateMySeller", editable: "self" },
        ]));
        expect(byEntity.order.input).toEqual(expect.arrayContaining([
            { endpointId: "checkoutMyCart", editable: "self" },
            { endpointId: "createOrder", editable: "self" },
        ]));
        expect(byEntity.order.output).toEqual(expect.arrayContaining([
            { endpointId: "myOrder" },
