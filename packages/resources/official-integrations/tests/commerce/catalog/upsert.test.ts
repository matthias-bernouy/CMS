...
describe("commerce product writes", () => {
    test("derives the variant matrix from axes and rejects arbitrary variants", async () => {
        ...
        expect(payload.variantAxes).toEqual(expect.arrayContaining([
            expect.objectContaining({ key: "color", fieldKey: "color", label: "color" }),
        ]));
        ...
        expect(matrix).not.toEqual(expect.arrayContaining([
            expect.objectContaining({ key: "arbitrary:matrix" }),
        ]));
        expect(matrix.every(row => row.status === "active"
            && Array.isArray(row.choices)
            && row.choices.length === 2)).toBeTrue();
    });
});
