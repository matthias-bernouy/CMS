...
describe("commerce resource-backed lookup selections", () => {
    test("uses the six embedded relations and keeps direct endpoints available", async () => {
        ...
        expect(endpoints(definition).map(endpoint => endpoint.endpointId)).toEqual(expect.arrayContaining([
            "manageBrand",
            "manageCategory",
            "seller",
            "manageProduct",
    ...
    test("preserves relation labels and ids through strict source projection", async () => {
        ...
        const product = projectStrictDataShape({
            ...
            privateNote: "hidden",
        }, responseBody(sourceEndpoints, "manageProduct"), "response", { enforceRequired: false });
        const category = projectStrictDataShape({
            ...
            privateNote: "hidden",
        }, responseBody(sourceEndpoints, "manageCategory"), "response", { enforceRequired: false });
...
function lookupSelections(value: unknown, selections: Record<string, string> = {}): Record<string, string> {
    if (Array.isArray(value)) {
        for (const item of value) lookupSelections(item, selections);
        return selections;
    }
    ...
    if (typeof entry.id === "string" && lookup && typeof lookup === "object") {
        ...
        if (typeof selected === "string") selections[entry.id] = selected;
    }
    for (const child of Object.values(entry)) lookupSelections(child, selections);
    return selections;
}
