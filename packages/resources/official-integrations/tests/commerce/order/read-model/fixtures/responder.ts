import { capturedFetches, jsonResponse, setRestResponder, type CapturedFetch } from "../../../harness";
import {
    adminEventRows,
    buyerEventRows,
    lineRows,
...
export function useCompleteOrderResponder(): void {
    setRestResponder(async (request) => {
        ...
        if (resource === "get_order_fulfillment_authorization") {
...
export function completeDetailEnvelope(scope: "buyer" | "seller" | "admin"): Record<string, unknown> {
    if (scope === "seller") {
...
function orderResponse(url: URL): Response {
    ...
    if (sellerFilter && sellerFilter !== "eq.17") {
        return jsonResponse([]);
    }
