...
import { orderPublicId, sellerId, sellerTrackingResponse } from "../shared/fixtures";
...
describe("seller shipment read contract", () => {
    ...
    test("keeps an absent shipment as a successful empty array", async () => {
        const { response, calls } = await executeRead(sellerResponder({ shipments: { items: [] } }));
    ...
    test("preserves nulls while omitting absent optional shipment fields", async () => {
        ...
        const { response } = await executeRead(sellerResponder({ shipments: { items: [changed] } }));
...
function executeRead(responder = sellerResponder()) {
    return executeSellerFunction(functionId, sellerGetRequest(functionId, "42"), responder);
}
