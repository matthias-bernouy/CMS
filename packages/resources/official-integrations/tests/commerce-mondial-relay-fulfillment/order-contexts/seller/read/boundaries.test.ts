...
describe("seller shipment read boundaries", () => {
    test("fails before source work when the authenticated subject is missing", async () => {
        const { response, calls } = await execute(sellerGetRequest(functionId, "42"), sellerResponder(), null);
    ...
    test("forwards optional string selectors and stops on Commerce refusal", async () => {
        for (const orderId of [undefined, "", "invalid", "7.5", "-1", "0", "9007199254740992"]) {
    ...
    test("fails closed on malformed Commerce and Delivery responses", async () => {
        const malformedSale = await executeRead(
            sellerResponder({
                sale: Response.json([]),
            }),
        );
        const malformedItems = await executeRead(
            sellerResponder({
...
function execute(request: Request, responder: (request: Request) => Response, user?: null) {
