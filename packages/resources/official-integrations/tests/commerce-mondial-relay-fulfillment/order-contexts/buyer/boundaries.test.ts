...
describe("buyer shipment context boundaries", () => {
    ...
    test("preserves incomplete Commerce context behavior", async () => {
        ...
        const missingId = await executeBuyerTracking(
            successfulBuyerResponder({ order: { id: undefined } }),
        );
    ...
    test("forwards optional string selectors and stops on Commerce refusal", async () => {
        for (const suffix of [
            ...
            const request = new Request(
                `https://cms.test/functions/getShipmentForOrder${suffix}`,
            );
            const { response, calls } = await executeBuyerTracking(
                successfulBuyerResponder(),
                { request },
    ...
    test("redacts Commerce and Delivery failures at their exact stop point", async () => {
        const commerce = await executeBuyerTracking(
            successfulBuyerResponder({ failAt: "commerce" }),
        );
        const delivery = await executeBuyerTracking(
            successfulBuyerResponder({ failAt: "delivery" }),
        );
    ...
    test("fails closed on malformed context and shipment collections", async () => {
        ...
        const nonArray = await executeBuyerTracking(
            successfulBuyerResponder({ items: { id: "shipment-42" } }),
        );
