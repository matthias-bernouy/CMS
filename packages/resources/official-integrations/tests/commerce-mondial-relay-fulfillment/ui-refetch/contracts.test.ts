...
describe("seller fulfillment UI mutation contracts", () => {
    test("renders the shipment returned after label creation without changing the seller projection", async () => {
        ...
        const { bloc, calls } = await createBloc(call => {
            ...
            if (call.path === readPath) return { ...order, shipments: [created] };
        ...
        expect(snapshot(bloc)).toEqual({
            ...
            status: "Bordereau prêt",
            expeditionNumber: shipment.expeditionNumber,
            latest: "Le bordereau d’expédition est disponible.",
            message: "",
            contentHidden: false,
            createHidden: true,
            handoffHidden: false,
    ...
    test("renders a seller handoff and emits the existing fulfillment event", async () => {
        ...
        const { bloc, calls } = await createBloc(call => {
            ...
            if (call.path === readPath) return { ...order, shipments: [handedOff] };
            throw new Error(`unexpected request ${call.method} ${call.path}`);
        });
        ...
        expect(snapshot(bloc)).toEqual({
            orderNumber: order.orderNumber,
            status: "Dépôt déclaré",
            expeditionNumber: shipment.expeditionNumber,
            latest: "En attente du premier scan Mondial Relay.",
            message: "",
            contentHidden: false,
            createHidden: true,
            handoffHidden: true,
