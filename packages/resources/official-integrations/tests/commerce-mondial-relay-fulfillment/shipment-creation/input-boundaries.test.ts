...
describe("seller shipment creation input boundaries", () => {
    test("rejects malformed bodies before source work", async () => {
        ...
        const cases: Array<[Request, string]> = [
            ...
            [shipmentCreationRequest({ orderId: null }), "body.orderId must be a string"],
            [shipmentCreationRequest({ orderId: 42 }), "body.orderId must be a string"],
            [shipmentCreationRequest({ orderId: "42", extra: true }), "body.extra is not allowed"],
        ];

        for (const [request, error] of cases) {
            const { response, calls } = await executeShipmentCreation(creationResponder(), { request });
    ...
    test("fails without an execution identity before outgoing fetch", async () => {
        const { response, calls } = await executeShipmentCreation(creationResponder(), { user: null });
