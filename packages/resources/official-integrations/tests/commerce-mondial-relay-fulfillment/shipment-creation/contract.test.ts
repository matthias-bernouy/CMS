...
describe("seller shipment creation contract", () => {
    test("preserves the exact success response and six causal boundaries", async () => {
        ...
        expect(Object.keys(body)).toEqual([
            "orderId", "orderPublicId", "shipment", "fulfillment",
        ]);
        expect(Object.keys(body.shipment)).toEqual([
            "ok", "id", "expeditionNumber", "trackingUrl", "status",
            "createdAt",
        ]);
        expect(Object.keys(body.fulfillment)).toEqual([
            "id", "orderId", "businessKey", "deliveryQuoteId",
            "financialTermsHash", "status", "attempts",
            "providerReference", "providerShipmentId", "lastError",
            "createdAt", "updatedAt", "idempotentReplay", "fulfillment",
        ]);
...
function expectedCalls() {
    return [{
        ...
        method: "POST", path: "/resolveDeliveryQuote", params: {},
        body: expectedQuoteRequest(), userId: null,
    }, {
        method: "POST", path: "/createShipment", params: {},
        body: expectedShipmentRequest(), userId: sellerId,
    }, {
        method: "POST", path: "/completeShipmentCreation", params: {},
        body: expectedCompletionRequest(), userId: null,
    }];
}
