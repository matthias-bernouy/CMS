...
import { creationResponder, privateFailure, type CreationReplies } from "./responders";
...
describe("seller shipment creation dependency boundaries", () => {
    test("preserves setup seller and allowed refusals", async () => {
        const cases: Array<[unknown, number, string]> = [
            [
                ...
                403,
                "Fulfillment authorization belongs to another seller",
            ],
            [
                {
                    ...sellerSetup,
                ...
                409,
                "Commerce has not authorized shipment creation",
            ],
        ];

        for (const [reply, status, error] of cases) {
            const { response, calls } = await executeShipmentCreation(creationResponder({ setup: reply }));
            expect(response.status).toBe(status);
            expect(await response.json()).toEqual({ error });
            expect(calls.map((call) => call.url.pathname)).toEqual(["/shipmentCreationSellerContext"]);
        }
    });
    ...
    test("preserves the reservation seller refusal", async () => {
        ...
        expect(calls.map((call) => call.url.pathname)).toEqual(expectedPaths(2));
    });

    test("preserves the financial-terms projection failure", async () => {
        const { response, calls } = await executeShipmentCreation(
            creationResponder({
                setup: privateFailure(502, "invalid fulfillment authorization"),
            }),
        );
    ...
    test("redacts failures and stops at each causal boundary", async () => {
        ...
        for (const [replies, count] of cases) {
            const { response, calls } = await executeShipmentCreation(creationResponder(replies));
            await expectGenericFailure(response);
            expect(calls.map((call) => call.url.pathname)).toEqual(expectedPaths(count));
        }
    });
