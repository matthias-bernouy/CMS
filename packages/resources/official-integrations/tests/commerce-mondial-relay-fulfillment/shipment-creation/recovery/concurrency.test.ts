    ...
    test("preserves the in-progress conflict while one shipment is creating", async () => {
        ...
        const responder = creationResponder({
            shipment: async () => {
                ...
                return privateFailure(409, "shipment creation is already in progress");
            },
            fulfillment,
        });

        const results = await Promise.all([executeShipmentCreation(responder), executeShipmentCreation(responder)]);
        ...
        expect(failed.calls.map((call) => call.url.pathname)).toEqual(paths().slice(0, 4));
        ...
        expect(
            results
                ...
                .filter((call) => call.url.pathname === "/completeShipmentCreation"),
        ).toHaveLength(1);
    });
});
