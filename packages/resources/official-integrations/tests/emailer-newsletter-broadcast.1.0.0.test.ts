...
async function seedNewsletter(deps: { sources: InMemorySourceRepository; installations: InMemoryIntegrationInstallationRepository }): Promise<void> {
...
function newsletterOutputShape() {
    return { type: "object" as const, properties: { subscriptions: { type: "array" as const, items: { type: "object" as const, properties: { email: { type: "string" as const } } } }, total: { type: "number" as const } } };
}

function subscribers() { return [{ email: "ada@example.test", subscribed: true }, { email: "bea@example.test", subscribed: true }]; }
