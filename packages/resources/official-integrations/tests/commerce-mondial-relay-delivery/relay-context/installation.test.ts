...
describe("Commerce Mondial Relay delivery installation", () => {
    test("imports, validates, and grants both relay functions", async () => {
        ...
        for (const id of ["commerce", "mondial-relay", "user-account"]) {
            const sourceId = id === "mondial-relay"
                ? "delivery"
                : id === "user-account" ? "accounts" : id;
        ...
        if (!definition) throw new Error("delivery integration not found");
        ...
        for (const id of [
            ...
            if (!fn) throw new Error(`${id} was not imported`);
        ...
        expect(
            (await roles.get(USER_ROLE))?.grants.map(grant => grant.permission),
