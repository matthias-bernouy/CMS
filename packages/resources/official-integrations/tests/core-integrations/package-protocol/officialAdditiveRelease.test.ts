...
describe("official Photo Albums additive release", () => {
    ...
    test("binds executable install, migration, repeatable, and constraint assets", async () => {
        ...
        const target = packageVersion(packages, "1.1.0");
        const functionPrefix = "connectors/supabase/functions/cms-photo-albums/";
        expect(packageFiles(target, functionPrefix)).toEqual(packageFiles(source, functionPrefix));
        expect(packageText(target, `${functionPrefix}routes/photos.ts`)).toContain(
...
function packageFiles(
    ...
    return Object.fromEntries(
        Object.entries(integrationPackage.package.envelope.files).filter(([path]) => path.startsWith(prefix)),
    );
}
...
function packageText(
    ...
    const file = integrationPackage.package.envelope.files[path];
    if (!file) {
        throw new Error(`Official package file is missing: ${path}`);
    }
    return new TextDecoder().decode(decodeIntegrationPackageFile(file));
}
