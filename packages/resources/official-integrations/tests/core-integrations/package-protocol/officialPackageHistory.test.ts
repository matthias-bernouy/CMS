...
import { appendFile, chmod, cp, mkdtemp, rm } from "node:fs/promises";
...
describe("official package history", () => {
    ...
    test("rejects a non-canonical historical package object", async () => {
        const root = await temporaryOfficialRoot();
        const objectPath = join(root, officialPackageObjectRelativePath(PHOTO_ALBUMS_1_0_0_DIGEST), "package.json");
        await chmod(objectPath, 0o600);
        await appendFile(objectPath, "\n");
