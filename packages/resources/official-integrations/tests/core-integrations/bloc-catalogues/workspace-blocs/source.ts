...
export async function loadDefinition(): Promise<IntegrationDefinition> {
    const definition = await new FsIntegrationDefinitionRepository(OFFICIAL_INTEGRATIONS_ROOT).get("workspace-blocs");
    if (!definition) {
        throw new Error('Integration "workspace-blocs" definition not found');
    }
    return definition;
}
