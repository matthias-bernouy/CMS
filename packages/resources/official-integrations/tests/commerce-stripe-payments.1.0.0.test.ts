...
import { InMemorySourceRepository, makeEndpointUrn } from "@bernouy/cms-sources";
...
import {
    ...
    seedInstallation,
    stripeSource,
} from "./commerce-stripe-payments/integration-contract/sources/index";
...
describe("commerce-stripe-payments 1.0.0", () => {
    test("imports and creates a payment from trusted order data with the canonical CMS seller identity", async () => {
        ...
        const sectionFieldPaths = (detail: any, sectionId: string) =>
            ...
            [];
        const protectedPaymentDetail = paymentsTab?.children?.find(
            (child: any) => child.id === "protectedPaymentDetail",
        );
        ...
        expect(
            protectedPaymentDetail?.main?.find((section: any) => section.id === "immutableFinancialSnapshot")
                ?.description,
