import { negotiationContext, offer, policy, proposal, seller } from "./expected";
...
export function failingResponder(point: FailurePoint) {
    return (request: Request): Response => {
        const path = new URL(request.url).pathname;
        if (point === "offer" && (path === "/admin/offer" || path === "/system/offer/negotiation-context")) {
            return Response.json({ error: "offer not found" }, { status: 404 });
        }
        if (point === "seller" && path === "/admin/seller") {
            return Response.json({ error: "seller not found" }, { status: 404 });
        }
        if (point === "seller" && path === "/system/offer/negotiation-context") {
            return Response.json({ error: "seller not found" }, { status: 404 });
        }
        if (point === "negotiation" && (path === "/policy" || path === "/proposals")) {
            return Response.json({ error: "proposal policy unavailable", detail: "internal-row-7" }, { status: 409 });
        }
