...
export async function workflowSources(): Promise<InMemorySourceRepository> {
    ...
    await sources.createSource(source("commerce", [
        ...
        get(
            "getOfferNegotiationContext",
            "https://commerce.test/system/offer/negotiation-context",
            ["offerId"],
            { mode: "system" },
        ),
    ]));
    await sources.createSource(source("commerce-negotiation", [
        {
            ...get("getProposalPolicy", "https://negotiation.test/policy", [
            ...
            input: {
                body: {
                    ...
                    properties: Object.fromEntries([
                        "offerId", "amount", "message", "offerSlug", "offerTitle",
                        "sellerCmsUserId", "sellerDisplayName", "referenceAmount",
                        "currency", "publicationStatus", "availability",
                    ].map(name => [name, {
...
function get(
    ...
    return {
        urn: makeEndpointUrn(
            targetUrl.includes("negotiation.test") ? "commerce-negotiation" : "commerce",
            id,
        ),
        method: "GET",
        access,
        ...
        input: {
            params: params.map(name => ({
                ...
                schema: {
                    type: ["offerId", "referenceAmount"].includes(name)
                        ? "number"
                        : "string",
                },
            })),
