...
import { buyerId, buyerTrackingResponse, orderPublicId, shipment } from "../shared/fixtures";
import { executeBuyerTracking, loadBuyerTrackingFunction } from "../shared/harness";
...
describe("buyer shipment context contract", () => {
    test("keeps the exact null-aware public projection and call budget", async () => {
        ...
        expect(Object.keys(body)).toEqual(["orderId", "orderPublicId", "shipments"]);
        ...
        expect(JSON.stringify(body)).not.toContain(shipment.sellerHandoffDeclaredAt);
        ...
        expect(
            ...
            ["GET", "/system/order/payment-context", { orderId: "42" }, buyerId],
            ["GET", "/shipmentForExternalOrder", { externalOrderId: orderPublicId }, null],
        ]);
        expect(calls.every((call) => call.body === undefined)).toBe(true);
    });

    test("keeps empty tracking as a successful empty array", async () => {
        const { response, calls } = await executeBuyerTracking(successfulBuyerResponder({ items: [] }));
