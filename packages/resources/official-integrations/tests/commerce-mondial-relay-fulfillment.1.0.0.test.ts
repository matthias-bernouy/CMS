    ...
    test("gets buyer and seller shipment projections with one Delivery call and preserves an empty result", async () => {
        ...
        for (const testCase of cases) {
            ...
            for (const items of [[shipment], []]) {
                    ...
                    new Request(`https://cms.test/functions/${testCase.functionId}?orderId=42`),
                    {
                        ...
                        deps: {
                            fetchImpl: async input => {
                                ...
                                if (url.pathname === testCase.ownershipPath) return Response.json(testCase.ownership);
                ...
                expect(await response.json()).toEqual(items.length
                    ? testCase.response
                    : { ...testCase.response, shipments: [] });
            }
        }
    });
...
async function sourcesForFulfillment(): Promise<InMemorySourceRepository> {
    ...
    const delivery = makeSource("delivery", [
        ...
        endpoint("shipmentForExternalOrder", "GET", "/shipmentForExternalOrder", object({
            items: array({
                id: string(), expeditionNumber: string(), status: string(), trackingUrl: string(),
                deliveryRelayLocation: string(), latestEventLabel: string(), latestEventAt: string(),
                carrierAcceptedAt: string(), sellerHandoffDeclaredAt: string(), recipientHandoffAt: string(),
                createdAt: string(), events: array({
                    normalizedStatus: string(), occurredAt: string(), eventLabel: string(),
...
function deliveryPath(path: string): boolean {
    return [
        ...
        "/relaySelection", "/saveRelaySelection", "/createShipment", "/shipments",
        "/shipmentForExternalOrder", "/shipment",
        "/tracking",
