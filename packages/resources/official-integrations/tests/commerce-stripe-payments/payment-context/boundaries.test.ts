...
describe("Commerce Stripe payment workflow boundaries", () => {
    test("keeps authenticated access and validates refresh input before source work", async () => {
        expect((await loadPaymentFunction("getPaymentForOrder")).access)
            .toEqual({ mode: "auth" });
        expect((await loadPaymentFunction("refreshPaymentForOrder")).access)
            .toEqual({ mode: "auth" });

        const invalid = await executePaymentWorkflow(
    ...
    test("rejects an invalid query selector before source work", async () => {
        const { response, calls } = await executePaymentWorkflow(
            "getPaymentForOrder",
            new Request(
                "https://cms.test/functions/getPaymentForOrder?orderId=invalid",
            ),
            successfulResponder,
        );
    ...
    for (const [id, request] of workflows()) {
        ...
        test(`${id} stops when the order belongs to another buyer`, async () => {
            const { response, calls } = await executePaymentWorkflow(
                id,
                request(),
                outgoing => new URL(outgoing.url).pathname.includes("order")
