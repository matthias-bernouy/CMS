...
describe("stripe-connect 1.0.0 source", () => {
    ...
    test("keeps Stripe dispute submission and acceptance locally irreversible", async () => {
        ...
        const adminRole = await sourceRequestWithRole(harness, "operator", "admin", "listProviderExceptions");
        ...
        const resubmission = await sourceJsonWithRole(harness, "finance-1", "finance", "submitStripeDisputeEvidence", {
            disputeId: "dp_submitted",
        ...
        const terminalAcceptance = await sourceJsonWithRole(harness, "finance-1", "finance", "acceptStripeDispute", {
            disputeId: "dp_terminal",
        ...
        expect(stagedBySupport.status).toBe(200);
        ...
        expect(adminAcceptance.status).toBe(403);
        expect(terminalAcceptance.status).toBe(409);
        expect(await jsonBody(terminalAcceptance)).toEqual({ error: "Stripe dispute is already terminal" });
        expect(harness.rest.rows("financial_operations")).toHaveLength(0);
        expect(harness.rest.rows("payment_events")).toContainEqual(expect.objectContaining({
    ...
    test("requires two distinct finance actors above the immutable Commerce threshold", async () => {
        const harness = await createHarness();
        ...
        const firstSubmit = await sourceJsonWithRole(harness, "finance-1", "finance", "submitStripeDisputeEvidence", submitBody);
        const repeatedFirstSubmit = await sourceJsonWithRole(harness, "finance-1", "finance", "submitStripeDisputeEvidence", submitBody);
        const secondSubmit = await sourceJsonWithRole(harness, "finance-2", "finance", "submitStripeDisputeEvidence", submitBody);
        ...
        const firstAccept = await sourceJsonWithRole(harness, "finance-3", "finance", "acceptStripeDispute", acceptBody);
        const secondAccept = await sourceJsonWithRole(harness, "finance-4", "finance", "acceptStripeDispute", acceptBody);

        expect(firstSubmit.status).toBe(202);
        expect(repeatedFirstSubmit.status).toBe(202);
        expect(await jsonBody(firstSubmit)).toMatchObject({ approvalStatus: "pending_second_approval", firstApprovedBy: "finance-1" });
        ...
        expect(harness.rest.rows("irreversible_dispute_action_approvals")).toEqual([
            expect.objectContaining({ first_actor_id: "finance-1", second_actor_id: "finance-2", status: "approved" }),
            expect.objectContaining({ first_actor_id: "finance-3", second_actor_id: "finance-4", status: "approved" }),
        ]);
        expect(harness.rest.rows("payment_events")).toEqual(expect.arrayContaining([
            expect.objectContaining({ event_type: "stripe_dispute_evidence_submitted", actor_kind: "finance", actor_id: "finance-2" }),
            expect.objectContaining({ event_type: "stripe_dispute_accepted", actor_kind: "finance", actor_id: "finance-4" }),
        ]));
    });
    ...
    test("recursively redacts provider secrets from listed financial operations", async () => {
        ...
        const listed = await sourceRequestWithRole(harness, "support-1", "support", "listFinancialOperations");
    ...
    async fetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
        ...
        if (table === "rpc/claim_platform_payout_protection" && method === "POST") {
            ...
            if (revision > Number(control.liability_revision)) {
                ...
                if (!control.decrease_authorization_id && body.p_decrease_authorization_id) {
                    ...
                    return jsonResponse({ message: "forbidden: Finance decrease authorization mismatch" }, 400);
                }
            }
        ...
        if (table === "rpc/claim_financial_operations" && method === "POST") {
            ...
            const claimed = this.tables.financial_operations
                .filter(row => ["payment_intent_cancel", "payout_schedule_update"].includes(String(row.operation_type))
...
async function sourceRequestWithUser(harness: Harness, userId: string, endpoint: string, params: Record<string, string> = {}): Promise<Response> {
    return await sourceRequestWithRole(harness, userId, "finance", endpoint, params);
}
...
async function sourceJsonWithUser(harness: Harness, userId: string, endpoint: string, body: unknown, params: Record<string, string> = {}): Promise<Response> {
    return await sourceJsonWithRole(harness, userId, "finance", endpoint, body, params);
}
