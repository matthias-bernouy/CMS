...
export function registerProviderExceptionResolutionContracts(
    ...
    describe("stripe-connect provider reconciliation exception resolution", () => {
        test("resolves only active known exceptions with one filtered write each", async () => {
            ...
            const result = await successfulJson(await harness.run(
                "provider-exception-resolution-contract",
                1,
            ));
            ...
            expect(resolutionCalls.map(request => request.method)).toEqual([
                "PATCH", "PATCH", "PATCH",
            ]);
            expect(resolutionCalls.map(request => Object.fromEntries(request.searchParams)))
                .toEqual([
