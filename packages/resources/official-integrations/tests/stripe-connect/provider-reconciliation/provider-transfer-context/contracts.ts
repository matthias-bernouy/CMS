...
export function registerProviderTransferContextContracts(createHarness: CreateProviderReconciliationHarness): void {
    describe("stripe-connect provider transfer reconciliation contracts", () => {
        test("preserves a tracked transfer projection and provider order", async () => {
            const fixture = await createTrackedProviderTransferFixture(createHarness, "provider-transfer-context");

            const result = await successfulJson(
                await fixture.submit("system-transfer-context", "reconcileProviderPayment", {
        ...
        test("preserves transfer reversal mismatch quarantine before the transfer update", async () => {
            ...
            expect(
                fixture.rest.postgrestRequests.slice(2, 6).map((request) => [request.method, request.table]),
            ).toEqual([
                ["POST", "rpc/read_provider_transfer_reconciliation_context"],
        ...
        test("keeps an untracked provider transfer quarantined without a local transfer update", async () => {
            const fixture = await createPaymentLedgerFixture(createHarness, "provider-transfer-untracked");
            ...
            expect(
                fixture.rest.postgrestRequests.filter(
                    (request) => request.method === "PATCH" && request.table === "transfers",
                ),
            ).toEqual([]);
        });
    });
