...
export function registerTerminalOperationRecoveryContracts(
    ...
    describe("stripe-connect terminal financial operation recovery contracts", () => {
        for (const recoveryCase of cases) {
            test(`recovers a terminal ${recoveryCase.kind} without a money provider call`, async () => {
                ...
                const seed = harness.rest.seedTerminalOperationRecovery(
                    recoveryCase.kind as OperationRecoveryKind,
                );
                harness.rest.clearPostgrestRequests();
                harness.rest.clearStripeRequests();

                const result = await successfulJson(await harness.run(
                    `terminal-${recoveryCase.kind}-operation-recovery`,
                    1,
                ));
                ...
                expect(harness.rest.rows("financial_operations").find(row => (
                    row.id === seed.operationId
                ))).toMatchObject({
                    status: recoveryCase.operationStatus,
                    stripe_object_id: seed.providerObjectId,
                    response: { id: seed.providerObjectId, status: recoveryCase.operationStatus === "processing"
                        ? "pending" : "succeeded" },
                    last_error: null,
                    attempt_count: 2,
                    completed_at: recoveryCase.operationStatus === "processing"
                        ? null : expect.stringMatching(/^\d{4}-\d{2}-\d{2}T/),
                });
                expect(harness.rest.rows(recoveryCase.table).find(row => (
                    row.id === seed.artifactId
                ))).toMatchObject({
                    ...(recoveryCase.kind === "transfer"
                        ? { stripe_transfer_id: seed.providerObjectId } : {}),
                    ...(recoveryCase.kind === "reversal"
                        ? { stripe_transfer_reversal_id: seed.providerObjectId } : {}),
                    ...(recoveryCase.kind === "refund"
                        ? { stripe_refund_id: seed.providerObjectId } : {}),
                    status: recoveryCase.kind === "refund" ? "pending" : "succeeded",
                });
                expect(harness.rest.stripeRequests.map(request => [request.method, request.pathname]))
                    .toEqual([["GET", "/v1/balance_settings"]]);
                ...
                expect(harness.rest.postgrestRequests.slice(claimIndex).map(request => (
                    [request.method, request.table]
