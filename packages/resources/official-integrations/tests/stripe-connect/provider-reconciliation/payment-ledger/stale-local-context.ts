...
export function registerStalePaymentLocalContextContracts(
    createHarness: CreateProviderReconciliationHarness,
): void {
    describe("stripe-connect stale payment local context contracts", () => {
        test("preserves an uncaptured payment snapshot without charged-provider reads", async () => {
            const fixture = await createUncapturedPayment(
                createHarness,
                "stale-local-context-uncaptured",
            );
            ...
            expect(fixture.rest.stripeRequests.map(request => [request.method, request.pathname]))
                .toEqual([["GET", `/v1/payment_intents/${fixture.paymentIntentId}`]]);
        ...
        test("keeps a fresh individual provider read before ledgering a nonterminal refund", async () => {
            const fixture = await createUncapturedPayment(
                createHarness,
                "stale-local-context-pending-refund",
            );
            fixture.rest.setPaymentIntentSucceeded(fixture.paymentIntentId);
            await successfulJson(await fixture.submit(
                "system-stale-context",
                "reconcileProviderPayment",
                { paymentId: fixture.paymentId },
            ));
            fixture.rest.setNextRefundStatus("pending");
            await successfulJson(await fixture.submit("system-stale-context", "requestProtectedRefund", {
                paymentId: fixture.paymentId,
            ...
            const databaseCalls = fixture.rest.postgrestRequests.map(request => (
                [request.method, request.table] as const
            ));
