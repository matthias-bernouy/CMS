...
begin
    if target is null then
        raise exception 'payment projection: RPC is missing';
    end if;
    if exists (
        select 1
        ...
        raise exception 'payment projection: RPC security changed';
    end if;
    if pg_catalog.has_function_privilege('anon', target, 'execute')
       ...
       or not pg_catalog.has_function_privilege('service_role', target, 'execute') then
        raise exception 'payment projection: RPC grants changed';
    end if;
end;
$security$;
