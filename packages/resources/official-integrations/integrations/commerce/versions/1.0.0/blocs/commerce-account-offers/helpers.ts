export const statusCodes = ["all", "draft", "action_required", "under_review", "online", "paused", "rejected", "archived"];
...
export function formatMoney(amount, currency, locale) {
    if (amount === null || amount === undefined || String(amount).trim() === "") return "Prix à définir";
    const value = Number(amount);
    if (!Number.isSafeInteger(value)) return "Prix à définir";
    try {
        return new Intl.NumberFormat(locale, { style: "currency", currency: String(currency || "EUR").toUpperCase() }).format(value / 100);
    } catch {
