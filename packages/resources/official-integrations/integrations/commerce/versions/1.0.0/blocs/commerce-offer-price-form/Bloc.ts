...
export class CommerceOfferPriceForm extends Composition {
    static observedAttributes = [
        ...
        "unavailable-message",
        "unavailable-title",
        ...colorGroups.flatMap(group => ["text", "background", "border", "accent"].map(color => `${group}-${color}-color`)),
    ];
    ...
    connectedCallback() {
        ...
        for (const retry of this.querySelectorAll("[data-retry]")) retry.addEventListener("click", this.onRetry);
    ...
    disconnectedCallback() {
        ...
        for (const retry of this.querySelectorAll("[data-retry]")) retry.removeEventListener("click", this.onRetry);
    }

    attributeChangedCallback(name) {
        if (!this.isConnected) return;
        if (name === "stripe-source-id" || name === "source-prefix") this.clientConfigPromise = null;
        if (!this.templateReady) return;
        this.syncPresentation();
        if (this.offer && name === "locale") this.renderOffer();
        if (["offer-id", "offer-param", "source-id", "enrollment-function-id"].includes(name)) this.load();
    }
    ...
    async load() {
        ...
        if (!id) {
            this.showTechnical(this.text("technical-message", "Impossible de retrouver cette annonce. Retourne à tes annonces puis réessaie."));
            return;
        }
        try {
            ...
            if (!this.validRule) {
                this.showTechnical(this.text("technical-message", "La plage de prix de cette annonce est momentanément indisponible. Réessaie dans quelques instants."));
                return;
            }
            ...
            this.termsConsentRequired = this.enrollmentRequired || connect?.marketplaceTermsCurrentVersionAccepted !== true;
            this.activationRequired = this.enrollmentRequired || this.termsConsentRequired;
            if (this.enrollmentRequired) await this.loadActivationProfile();
            ...
            this.showTechnical(this.text("technical-message", "Impossible de charger cette annonce pour le moment. Réessaie dans quelques instants."));
        }
    }
    ...
    renderActivationState() {
        ...
        this.activationCopy.textContent = termsOnly
            ? this.text("terms-update-copy", "Lis et accepte la version actuelle des conditions vendeur pour envoyer ton prix.")
            : this.text("activation-copy", "Ces informations sont nécessaires pour publier ton annonce et recevoir le produit de tes ventes plus tard. Aucun compte bancaire n’est demandé ici.");
    }
    ...
    renderOffer() {
        ...
        if (!String(this.amount.value || "").trim()) {
            ...
            if (draft) this.amount.value = draft;
        }
    }
    ...
    onSubmit = async event => {
        ...
        if (this.pending || !this.offer) return;
        ...
        try {
            ...
            this.show("success");
            this.dispatchEvent(new CustomEvent("commerce-offer-price:submitted", {
                bubbles: true,
                composed: true,
                detail: { offerId: this.offer.id, amount },
            }));
        } catch (error) {
            this.showInlineError(error instanceof PublicError
                ? error.message
                : this.text("submit-error-message", "Impossible d’envoyer ton prix pour le moment. Vérifie tes informations puis réessaie."));
        } finally {
    ...
    activationProfile() {
        ...
        for (const field of profileFields) {
            const control = this.profileControl(field);
            if (typeof control?.reportValidity === "function" && !control.reportValidity()) valid = false;
            if (!textValue(control?.value)) valid = false;
        }
        if (!valid) {
            this.profileValidation.textContent = this.text("field-required-message", "Complète tous les champs obligatoires pour continuer.");
            return null;
        }

        const profile = Object.fromEntries(profileFields.map(field => [field, textValue(this.profileControl(field)?.value)]));
        profile.email = textValue(this.profile?.email);
        profile.countryCode = profile.countryCode.toUpperCase();
        if (profile.countryCode !== "FR") {
            this.profileValidation.textContent = this.text("profile-error-message", "L’activation est actuellement réservée aux vendeurs domiciliés en France.");
            return null;
        }
        if (!validEmail(profile.email)) {
            this.profileValidation.textContent = this.text("profile-error-message", "Indique une adresse e-mail valide.");
        ...
        try {
            ...
            this.profileValidation.textContent = error instanceof PublicError
                ? error.message
    ...
    async updateProfileIfNecessary(profile) {
        ...
        const changed = Object.entries(update).some(([field, value]) => comparableProfileValue(field, this.profile?.[field]) !== comparableProfileValue(field, value));
        if (!changed) return;
    ...
    priceError(required = false) {
        ...
        if (!raw) return required ? this.text("required-message", "Indique un prix.") : "";
        const amount = majorToMinor(raw);
        const rule = this.offer?.priceRule;
        if (!Number.isSafeInteger(amount) || amount < 0) return this.text("invalid-message", "Indique un montant valide.");
        if (rule && (amount < rule.minimumAmount || amount > rule.maximumAmount)) {
            return this.text("range-error-message", `Choisis un prix entre ${formatMoney(rule.minimumAmount, rule.currency, this.locale)} et ${formatMoney(rule.maximumAmount, rule.currency, this.locale)}.`);
        }
        return "";
    }
    ...
    setPending(pending) {
        ...
        for (const control of this.profileControls) control.toggleAttribute("disabled", pending);
        this.submit.textContent = pending ? this.text("submitting-label", "Envoi…") : this.text("submit-label", "Envoyer mon prix");
    }
    ...
    syncPresentation() {
        ...
        this.descriptionElement.textContent = this.text("description", "Choisis ton prix de vente dans la plage proposée.");
        ...
        if (!this.activationRequired) {
            ...
            this.activationCopy.textContent = this.text("activation-copy", "Ces informations sont nécessaires pour publier ton annonce et recevoir le produit de tes ventes plus tard. Aucun compte bancaire n’est demandé ici.");
        }
        this.profileSummary.textContent = this.text("profile-summary", "Seules les informations manquantes sont demandées ici. Les autres sont reprises depuis ton profil.");
        ...
        this.stripeTermsLink.setAttribute("href", this.getAttribute("stripe-terms-url") || "https://stripe.com/connect-account/legal");
        this.privacyNotice.textContent = this.text("privacy-notice", "Les informations renseignées sont traitées pour activer ton compte vendeur et sécuriser les paiements.");
        ...
        this.submit.textContent = this.pending ? this.text("submitting-label", "Envoi…") : this.text("submit-label", "Envoyer mon prix");
        this.unavailableTitle.textContent = this.text("unavailable-title", "Cette action n’est pas disponible");
        this.unavailableMessage.textContent = this.text("unavailable-message", "Cette annonce ne nécessite plus de proposition de prix.");
        this.technicalTitle.textContent = this.text("technical-title", "Impossible de charger le prix");
        this.technicalMessage.textContent = this.text("technical-message", "Impossible de charger cette annonce pour le moment. Réessaie dans quelques instants.");
        ...
        this.successMessage.textContent = this.text("success-message", "Ta proposition a bien été transmise et va maintenant être vérifiée.");
        ...
        for (const element of this.querySelectorAll("[data-card-surface]")) copyColors(this, element, "card");
        for (const element of [this.amount, ...this.profileControls]) copyColors(this, element, "field");
        for (const element of [this.submit, this.profileLink, this.back, this.successLink, this.technicalRetry]) copyColors(this, element, "button");
    }
    ...
    connectValidationMessages() {
        ...
        for (const control of this.profileControls) control.setAttribute("aria-describedby", this.profileValidation.id);
    ...
    writePriceDraft(value) {
        try {
            ...
            if (draft) sessionStorage.setItem(this.priceDraftKey, draft);
            else sessionStorage.removeItem(this.priceDraftKey);
        } catch {
    ...
    async request(endpoint, init = {}) {
        ...
        if (!response.ok) throw new Error(body?.error || body?.message || this.text("error-message", "Une erreur est survenue."));
        if (!body || typeof body !== "object" || Array.isArray(body)) throw new Error(this.text("error-message", "Réponse invalide."));
        return body;
    }

    async requestSource(sourceId, endpoint, init = {}) {
        const response = await fetch(`${this.sourcePrefix}/${encodeURIComponent(sourceId)}/${encodeURIComponent(endpoint)}`, {
            credentials: "include",
            ...init,
            headers: {
                accept: "application/json",
    ...
    profileControl(field) { return this.querySelector(`[data-profile-control="${field}"]`); }
    get offerId() { return this.getAttribute("offer-id")?.trim() || new URL(location.href).searchParams.get(this.getAttribute("offer-param") || "id") || ""; }
    get sourcePrefix() { return (this.getAttribute("source-prefix") || "/.cms/sources").replace(/\/+$/, ""); }
    get sourceBase() { return `${this.sourcePrefix}/${encodeURIComponent(this.getAttribute("source-id") || "commerce")}`; }
    get accountSourceId() { return this.getAttribute("account-source-id")?.trim() || "user-account"; }
    get authSourceId() { return this.getAttribute("auth-source-id")?.trim() || "system-auth"; }
    get stripeSourceId() { return this.getAttribute("stripe-source-id")?.trim() || "stripe-connect"; }
    get enrollmentFunctionId() { return this.getAttribute("enrollment-function-id")?.trim() || "getSellerSaleEnrollment"; }
    get submitFunctionId() { return this.getAttribute("submit-function-id")?.trim() || "submitSellerOfferPrice"; }
    get locale() { return this.getAttribute("locale") || "fr-FR"; }
    get validRule() { const rule = this.offer?.priceRule; return Number.isSafeInteger(rule?.minimumAmount) && Number.isSafeInteger(rule?.maximumAmount) && rule.maximumAmount >= rule.minimumAmount; }
    get priceDraftKey() { return `commerce-offer-price:${this.offerId}`; }
    get templateReady() { return Boolean(this.loading && this.card && this.unavailable && this.technical && this.success && this.form); }
    get loading() { return this.querySelector("[data-loading]"); }
    get card() { return this.querySelector("[data-card]"); }
    get unavailable() { return this.querySelector("[data-unavailable]"); }
    get technical() { return this.querySelector("[data-technical]"); }
    get success() { return this.querySelector("[data-success]"); }
    get error() { return this.querySelector("[data-error]"); }
    get form() { return this.querySelector("[data-form]"); }
    get amount() { return this.querySelector("[data-amount]"); }
    get submit() { return this.querySelector("[data-submit]"); }
    get validation() { return this.querySelector("[data-validation]"); }
    get activationPanel() { return this.querySelector("[data-activation-panel]"); }
    get profilePanel() { return this.querySelector("[data-profile-panel]"); }
    get consentPanel() { return this.querySelector("[data-consent-panel]"); }
    get activationTitle() { return this.querySelector("[data-activation-title]"); }
    get activationCopy() { return this.querySelector("[data-activation-copy]"); }
    get profileSummary() { return this.querySelector("[data-profile-summary]"); }
    get profileLink() { return this.querySelector("[data-profile-link]"); }
    get profileValidation() { return this.querySelector("[data-profile-validation]"); }
    get profileControls() { return [...this.querySelectorAll("[data-profile-control]")]; }
    get consent() { return this.querySelector("[data-consent]"); }
    get consentValidation() { return this.querySelector("[data-consent-validation]"); }
    get consentPrefix() { return this.querySelector("[data-consent-prefix]"); }
    get sellerTermsLink() { return this.querySelector("[data-seller-terms-link]"); }
    get stripeConsentFragment() { return this.querySelector("[data-stripe-consent-fragment]"); }
    get stripeConsentPrefix() { return this.querySelector("[data-stripe-consent-prefix]"); }
    get stripeTermsLink() { return this.querySelector("[data-stripe-terms-link]"); }
    get privacyNotice() { return this.querySelector("[data-privacy-notice]"); }
    get privacyLink() { return this.querySelector("[data-privacy-link]"); }
    get titleElement() { return this.querySelector("[data-title]"); }
    get descriptionElement() { return this.querySelector("[data-description]"); }
    get offerLabel() { return this.querySelector("[data-offer-label]"); }
    get offerTitle() { return this.querySelector("[data-offer-title]"); }
    get rangeLabel() { return this.querySelector("[data-range-label]"); }
    get range() { return this.querySelector("[data-range]"); }
    get rangeMessage() { return this.querySelector("[data-range-message]"); }
    get unavailableTitle() { return this.querySelector("[data-unavailable-title]"); }
    get unavailableMessage() { return this.querySelector("[data-unavailable-message]"); }
    get technicalTitle() { return this.querySelector("[data-technical-title]"); }
    get technicalMessage() { return this.querySelector("[data-technical-message]"); }
    get technicalRetry() { return this.querySelector("[data-technical-retry]"); }
    get successTitle() { return this.querySelector("[data-success-title]"); }
    get successMessage() { return this.querySelector("[data-success-message]"); }
    get back() { return this.querySelector("[data-back]"); }
