...
export function withVariantMatrix(body: JsonRecord): JsonRecord {
    ...
    if (!("variantAxes" in body)) return safeBody;
...
function normalizeAxes(value: unknown): Axis[] {
    if (!Array.isArray(value)) throw new HttpError(422, "variantAxes must be an array");
    if (value.length > 4) throw new HttpError(422, "a product can have at most four variant axes");
    const axes = value.map((entry, position) => normalizeAxis(entry, position));
    assertUnique(axes.map(axis => axis.key), "variant axis keys");
    return axes;
}

function normalizeAxis(value: unknown, position: number): Axis {
    if (!isRecord(value)) throw new HttpError(422, `variantAxes[${position}] must be an object`);
    ...
    assertUnique(values.map(item => item.key), `value keys for ${label}`);
    return { key, ...(fieldKey ? { fieldKey } : {}), label, position, values };
}

function cartesian(axes: Axis[]): Choice[][] {
    if (!axes.length) return [];
    return axes.reduce<Choice[][]>((sets, axis) => sets.flatMap(set => axis.values.map(value => [
        ...set,
        { axisKey: axis.key, valueKey: value.key, ...(axis.fieldKey ? { fieldKey: axis.fieldKey } : {}), axisLabel: axis.label, valueLabel: value.label },
    ])), [[]]);
}
...
function metadataKey(value: unknown, name: string): string | undefined {
    ...
    if (!/^[A-Za-z][A-Za-z0-9_]{0,63}$/.test(key)) throw new HttpError(422, `${name} is invalid`);
    return key;
}
...
function requiredLabel(value: unknown, name: string, max: number): string {
    ...
    if (!label) throw new HttpError(422, `${name} is required`);
    if (label.length > max) throw new HttpError(422, `${name} is too long`);
    return label;
}
...
function assertUnique(values: string[], name: string): void {
    if (new Set(values).size !== values.length) throw new HttpError(422, `${name} must be unique`);
}
