...
as $$
    with settings_state as materialized (
        ...
        select offer.*
        from page_ids
        join commerce.offers offer on offer.id = page_ids.id
    ), page_variants as (
        ...
        select page.accepted_price_amount,
               page.updated_at,
               page.id,
               (to_jsonb(page) - 'seller_id' - 'inventory_revision')
               || jsonb_build_object(
    ...
    select jsonb_build_object(
        ...
        'items', coalesce((select jsonb_agg(items.value order by
            case when p_sort = 'price-asc' then items.accepted_price_amount end asc nulls last,
            case when p_sort = 'price-desc' then items.accepted_price_amount end desc nulls last,
