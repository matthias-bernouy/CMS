...
export async function listProtectedPayments(request: Request): Promise<Response> {
    ...
    const params = new URLSearchParams({ select: operationSelect, order: "updated_at.desc,order_id.desc", limit: String(limit), offset: String(offset) });
    for (const [query, column] of [
        ["paymentStatus", "payment_status"], ["settlementStatus", "settlement_status"],
        ["fulfillmentStatus", "fulfillment_status"], ["claimStatus", "claim_status"],
    ] as const) {
        const value = text(url.searchParams.get(query));
        if (value) params.set(column, `eq.${value}`);
    }
    const query = text(url.searchParams.get("q"));
    if (query) params.set("or", `(order_number.ilike.*${safeSearch(query)}*,buyer_cms_user_id.ilike.*${safeSearch(query)}*)`);
...
export async function getProtectedPayment(request: Request): Promise<Response> {
    ...
    if (orderId === undefined && !publicId) throw new HttpError(400, "orderId or publicId is required");
    const operation = orderId !== undefined
    ...
    if (!operation) throw new HttpError(404, "protected payment not found");
    const id = String(operation.order_id);
    const [financialTerms, paymentAttempts, fulfillment, settlement, claims, refundRequests, stripeDisputes, auditEvents] = await Promise.all([
        one("order_financial_terms", { order_id: id }),
        rows("order_payment_attempts", id), one("order_fulfillments", { order_id: id }),
        one("order_settlements", { order_id: id }), rows("marketplace_claims", id),
        rows("refund_requests", id), rows("stripe_dispute_projections", id),
        rows("audit_events", id, "created_at.asc,id.asc"),
    ]);
    return json(camelize({ ...operation, financialTerms, paymentAttempts, fulfillment, settlement, claims, refundRequests, stripeDisputes, auditEvents }));
}
...
export async function listCommerceExceptions(request: Request): Promise<Response> {
    ...
    const limit = Math.min(Math.max(integer(url.searchParams.get("limit"), "limit") ?? 50, 1), 100);
    const offset = Math.max(integer(url.searchParams.get("offset"), "offset") ?? 0, 0);
    const params = new URLSearchParams({ select: "*", order: "detected_at.desc,id.desc", limit: String(limit), offset: String(offset) });
    for (const [query, column] of [["kind", "kind"], ["status", "status"], ["orderId", "order_id"]] as const) {
        const value = text(url.searchParams.get(query));
        if (value) params.set(column, `eq.${value}`);
    }
