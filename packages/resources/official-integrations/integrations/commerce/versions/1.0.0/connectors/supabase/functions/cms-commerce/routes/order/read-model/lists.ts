...
export const listMyOrders = (request: Request): Promise<Response> =>
    listOrderReadModel(request, "buyer");
export const listMySales = (request: Request): Promise<Response> =>
    listOrderReadModel(request, "seller");
export const listAdminOrders = (request: Request): Promise<Response> =>
    listOrderReadModel(request, "admin");
...
async function listOrderReadModel(request: Request, scope: ListScope): Promise<Response> {
    ...
    const sellerId = scope === "seller"
        ? null
        : integer(url.searchParams.get("sellerId"), "sellerId") ?? null;
    const envelope = readEnvelope(await rpc("list_order_read_model", {
        p_scope: scope,
        p_cms_user_id: actor,
        p_status: status,
    ...
    const operationByOrder = new Map(envelope.operations.map(operation => [
        String(operation.order_id), operation,
    ]));
    const items = scope === "seller"
        ? envelope.orders.map(row => projectSale(row, envelope.definitions))
        : envelope.orders.map(row => projectOrderListItem(
            row,
...
function readEnvelope(value: unknown, scope: ListScope): ListEnvelope {
    ...
    if (!orders || !operations || !definitions
        || !Number.isSafeInteger(total) || (total as number) < 0) invalidEnvelope();
    ...
    if (value.state !== "ok" || (scope === "seller" && operations.length)
        || (scope === "admin" && definitions.length)) invalidEnvelope();
    return {
        state: "ok",
        orders,
        operations,
