...
const lineFields = [
    "id", "orderId", "offerId", "productId", "variantId", "acceptedProposalId",
    "title", "sku", "quantity", "unitAmount", "totalAmount", "productSnapshot",
    "variantSnapshot", "offerSnapshot", "sellerSnapshot", "createdAt",
] as const;
const sellerLineFields = lineFields.filter(field => field !== "sellerSnapshot");
const publicEventFields = [
    "id", "orderId", "eventType", "previousStatus", "nextStatus", "createdAt",
] as const;
const adminEventFields = [
    "id", "orderId", "eventType", "actorKind", "actorId", "previousStatus",
    "nextStatus", "message", "data", "createdAt",
] as const;
const sellerFields = ["id", "kind", "slug", "displayName"] as const;
const operationFields = [
    "orderId", "orderPublicId", "orderNumber", "buyerCmsUserId", "sellerId",
    "currency", "buyerTotalAmount", "sellerProceedsAmount", "platformRetainedAmount",
    "financialTermsHash", "paymentStatus", "fulfillmentStatus", "settlementStatus",
    "claimStatus", "totalRefundRequestedAmount", "releaseEligibleAt",
    "recipientHandoffAt", "recipientHandoffFirstObservedAt", "claimWindowStartedAt",
    "claimByAt", "updatedAt",
] as const;
const sellerOperationFields = [
    "orderId", "orderPublicId", "orderNumber", "currency", "paymentStatus",
    "fulfillmentStatus", "settlementStatus", "claimStatus", "recipientHandoffAt",
    "recipientHandoffFirstObservedAt", "claimWindowStartedAt", "claimByAt",
    "releaseEligibleAt", "updatedAt",
] as const;
const financialFields = [
    "orderId", "deliveryQuoteId", "merchandiseSubtotalAmount", "shippingAmount",
    "buyerProtectionFeeAmount", "sellerCommissionAmount", "buyerTotalAmount",
    "sellerProceedsAmount", "platformRetainedAmount", "currency", "financialTermsHash",
...
const fulfillmentFields = [
    "orderId", "status", "sellerHandoffDeadline", "scanGraceDeadline",
    "carrierAcceptedAt", "arrivedAtPickupPointAt", "availableForPickupAt",
    "recipientHandoffAt", "recipientHandoffFirstObservedAt", "claimWindowStartedAt",
    "claimByAt", "releaseEligibleAt", "blockingReason", "version",
] as const;
const sellerFulfillmentFields = [
    "orderId", "status", "sellerHandoffDeadline", "scanGraceDeadline",
    "sellerHandoffDeclaredAt", "carrierAcceptedAt", "recipientHandoffAt",
    "recipientHandoffFirstObservedAt", "claimWindowStartedAt", "claimByAt",
    "releaseEligibleAt", "blockingReason", "version",
] as const;
const settlementFields = [
    "orderId", "status", "authorizedSellerAmount", "totalTransferredAmount",
    "totalReversedAmount", "totalRefundedAmount",
    "sellerReserveLiabilityRemainingAmount", "version",
] as const;
const sellerSettlementFields = settlementFields.filter(field => field !== "totalRefundedAmount");
const claimFields = [
    "id", "publicId", "reason", "status", "sellerResponseByAt", "returnShipByAt",
    "resolvedAt", "version", "createdAt",
] as const;
const authorizationFields = [
    "allowed", "reason", "orderId", "orderPublicId", "sellerId", "currency",
...
export function projectOrderDetail(bundle: DetailBundle, publicMetadata: boolean): JsonRecord {
    const detail = {
        ...
        events: bundle.events.map(event => safeRecord(
            event,
            publicMetadata ? publicEventFields : adminEventFields,
        )),
    ...
    return publicMetadata
        ? withPublicOrderMetadata(detail, bundle.definitions)
        : detail;
}
