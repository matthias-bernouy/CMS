...
export async function registerMySeller(request: Request): Promise<Response> {
    ...
    if (body.displayName !== undefined && !text(body.displayName)) throw new HttpError(400, "displayName must be text");
    if (body.metadata !== undefined && !isRecord(body.metadata)) throw new HttpError(400, "metadata must be an object");
...
export async function listSellers(request: Request): Promise<Response> {
    ...
    if (status) params.set("verification_status", `eq.${status}`);
    if (kind) params.set("kind", `eq.${kind}`);
    if (query) params.set("or", `(display_name.ilike.*${query}*,slug.ilike.*${query}*,cms_user_id.ilike.*${query}*)`);
