...
export class CommerceNegotiationForm extends Composition {
    static observedAttributes = [
        "amount-hint", "amount-label", "appearance", "button-accent-color",
        "button-background-color", "button-border-color", "button-label", "button-text-color",
        "card-background-color", "card-border-color", "card-muted-text-color", "card-text-color",
        "copy", "current-label", "density", "error-message", "existing-message", "field-accent-color",
        "field-background-color", "field-border-color", "field-text-color", "locale",
        "message-hint", "message-label", "message-placeholder", "offer-id", "range-label",
        "own-offer-message",
        "skeleton-base-color", "skeleton-highlight-color", "source-id", "source-prefix",
        "success-message", "title", "toast-background-color", "toast-border-color",
        "toast-error-background-color", "toast-error-border-color", "toast-error-text-color",
        "toast-text-color", "unavailable-message",
    ];
    ...
    async loadPolicy() {
        ...
        try {
            ...
            if (!policy.enabled) throw new Error("Les propositions sont temporairement désactivées.");
            if (policy.canPropose === false) {
                const message = policy.ineligibilityReason === "own_offer"
                    ? this.getAttribute("own-offer-message") || "Vous ne pouvez pas faire une offre sur votre propre annonce."
            ...
            if (controller.signal.aborted) return;
            this.showUnavailable();
            this.showToast(errorMessage(error, this.getAttribute("error-message") || "Impossible de charger les conditions de cette offre."), true);
        }
    }
    ...
    syncPresentation() {
        ...
        setText(this.querySelector("[data-copy]"), this.getAttribute("copy") || "Propose un prix au vendeur. Ton offre doit rester dans la fourchette autorisée.");
        ...
        setText(this.querySelector("[data-unavailable]"), this.getAttribute("unavailable-message") || "Cette annonce n’est pas disponible pour une proposition.");
        this.syncExistingProposal();
        ...
        setAttribute(amount, "hint", this.getAttribute("amount-hint") || "Saisis un montant compris dans la fourchette indiquée.");
        const message = this.messageInput;
        setAttribute(message, "label", this.getAttribute("message-label") || "Message au vendeur (facultatif)");
        setAttribute(message, "hint", this.getAttribute("message-hint") || "Tu peux préciser les raisons de ta proposition.");
        setAttribute(message, "placeholder", this.getAttribute("message-placeholder") || "Bonjour, accepterais-tu mon offre ?");
        for (const field of [amount, message]) copyColors(this, field, "field", ["accent-color", "text-color", "background-color", "border-color"]);

        setText(this.submitButton, this.getAttribute("button-label") || "Envoyer mon offre");
        copyColors(this, this.submitButton, "button", ["accent-color", "text-color", "background-color", "border-color"]);
        ...
        if (this.policy) this.syncPolicy();
    }
    ...
    syncPolicy() {
        ...
        setText(this.querySelector("[data-current-price]"), formatMoney(policy.referenceAmount, policy.currency, locale));
        setText(this.querySelector("[data-range]"), `${formatMoney(policy.minimumAmount, policy.currency, locale)} – ${formatMoney(policy.maximumAmount, policy.currency, locale)}`);
    ...
    async submitProposal() {
        if (!this.policy || !this.form?.reportValidity()) return;
        ...
        try {
            ...
            this.dispatchEvent(new CustomEvent("commerce-negotiation:created", {
                bubbles: true,
                composed: true,
                detail: proposal,
            }));
        } catch (error) {
            this.showToast(errorMessage(error, this.getAttribute("error-message") || "Impossible d’envoyer ton offre."), true);
        } finally {
    ...
    sourceUrl(endpoint) {
        ...
        const sourceId = encodeURIComponent(orchestratedEndpoints.has(endpoint)
            ? "system-functions"
    ...
    syncExistingProposal() {
        if (!this.existingProposal) return;
        const amount = formatMoney(
            this.existingProposal.proposedAmount,
            this.existingProposal.currency,
            this.getAttribute("locale") || "fr-FR",
        );
        const message = this.getAttribute("existing-message")
            || "Vous avez déjà fait une offre de {amount} pour cette annonce.";
        setText(this.querySelector("[data-existing]"), message.replaceAll("{amount}", amount));
    }

    showUnavailable(message) {
        if (message) setText(this.querySelector("[data-unavailable]"), message);
        setHidden(this.querySelector("[data-loading]"), true);
        setHidden(this.querySelector("[data-content]"), true);
        setHidden(this.querySelector("[data-existing]"), true);
    ...
    get amountInput() { return this.querySelector("[data-amount]"); }
    get messageInput() { return this.querySelector("[data-message]"); }
    get submitButton() { return this.querySelector("[data-submit]"); }
}
...
function validPolicy(value) {
    const policy = {
        ...
        currency: typeof value.currency === "string" && /^[a-z]{3}$/i.test(value.currency) ? value.currency.toLowerCase() : null,
    };
    if (!policy.offerId || policy.referenceAmount === null || policy.minimumAmount === null
        || policy.maximumAmount === null || !policy.currency || policy.minimumAmount > policy.maximumAmount) {
...
function validExistingProposal(value, offerId) {
    ...
    if (!proposal || positiveInteger(proposal.offerId) !== offerId || proposal.status !== "pending") return null;
    const proposedAmount = safeAmount(proposal.proposedAmount);
    const currency = typeof proposal.currency === "string" && /^[a-z]{3}$/i.test(proposal.currency)
