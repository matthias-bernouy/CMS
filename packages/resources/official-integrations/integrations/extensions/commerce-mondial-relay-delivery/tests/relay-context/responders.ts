...
export type FailurePoint = "context" | "account" | "save" | "resolve" | "lock" | "public";
...
export function successfulResponder(overrides: Overrides = {}): (request: Request) => Response {
    return (request) => {
        ...
        if (overrides.failAt === point) {
            return privateFailure(point);
        }
        if (path === "/delivery-setup-context") {
            ...
            return Response.json({
                order: contextOrder,
                authorization:
                    contextOrder.status !== "awaiting_quote" || overrides.authorization === null
                        ? null
                        : {
                              ...authorization,
        ...
        if (path === "/delivery-selection-context") {
            ...
            const financialTerms = contextOrder.financialTerms as Record<string, unknown> | null;
            return Response.json({
                publicId: contextOrder.publicId,
                buyerCmsUserId: contextOrder.buyerCmsUserId,
                deliveryQuoteId: financialTerms === null ? null : financialTerms.deliveryQuoteId,
            });
        }
        if (path === "/account") {
...
export function successfulGetResponder(overrides: Overrides = {}): (request: Request) => Response {
    return successfulResponder(overrides);
}

export function selectorResponder(request: Request): Response {
    const url = new URL(request.url);
    if (url.pathname === "/delivery-setup-context" || url.pathname === "/delivery-selection-context") {
        const id = url.searchParams.get("orderId");
        if (!id || !/^-?\d+$/.test(id) || !Number.isSafeInteger(Number(id))) {
            return Response.json({ error: "id or publicId is required" }, { status: 400 });
        }
        if (id !== "42") {
