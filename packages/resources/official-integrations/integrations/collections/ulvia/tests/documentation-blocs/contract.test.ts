...
import { decodeDefaultContent, loadDefinition, loadDefinitions } from "./source";
...
describe("documentation-blocs 4.0.1 catalogue", () => {
    test("hydrates the complete official documentation catalog", async () => {
        ...
        expect(definition.version).toBe("4.0.1");
    ...
    test("validates and builds every runtime and editor source", async () => {
        ...
        for (const artifact of documentationBlocs(definition.artifacts)) {
    ...
    test("force-refreshes every owned bloc when the installation is re-run", async () => {
        ...
        const ownedCalls = calls.filter(({ tag }) => EXPECTED_TAGS.includes(tag));
        expect(ownedCalls.slice(0, EXPECTED_TAGS.length).every((call) => !call.force)).toBe(true);
        expect(ownedCalls.slice(EXPECTED_TAGS.length).every((call) => call.force)).toBe(true);
        expect(rerun.artifacts).toHaveLength(definition.artifacts.length);
        expect(rerun.artifacts.every((artifact) => artifact.action === "updated")).toBe(true);
        expect(rerun.installation.runCount).toBe(2);
    });
});

function documentationBlocs(artifacts: Awaited<ReturnType<typeof loadDefinition>>["artifacts"]) {
    return artifacts.filter(
        (artifact) =>
            artifact.type === "bloc" && artifact.bloc.path?.startsWith("blocs/foundation/documentation-blocs/"),
    );
}
