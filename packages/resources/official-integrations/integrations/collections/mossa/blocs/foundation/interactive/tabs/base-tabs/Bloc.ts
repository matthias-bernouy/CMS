...
export class Bloc extends Component {
    ...
    override connectedCallback(): void {
        ...
        if (this._triggers) {
            this._resizeObs.observe(this._triggers);
        }
    ...
    private _rebuild = () => {
        ...
        this._triggers.replaceChildren(
            ...items.map((item, i) => {
    ...
    private _onKey = (e: KeyboardEvent) => {
        ...
        if (e.key === "ArrowRight") {
            e.preventDefault();
            this._activate((current + 1) % triggers.length);
        } else if (e.key === "ArrowLeft") {
            e.preventDefault();
            this._activate((current - 1 + triggers.length) % triggers.length);
        } else if (e.key === "Home") {
            e.preventDefault();
            this._activate(0);
        } else if (e.key === "End") {
            e.preventDefault();
            this._activate(triggers.length - 1);
        }
    };

    private _moveIndicator() {
        if (!this._indicator || !this._triggers) {
            return;
        }
