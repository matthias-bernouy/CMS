...
const editorCSS = `
.item.is-open .header { color: var(--_mossa-item-accent); }
.item.is-open .indicator { transform: rotate(-180deg); }
.item.is-open .panel { grid-template-rows: 1fr; }
.item.is-open .content {
    padding: 0.25rem var(--_mossa-item-padding-x) var(--_mossa-item-padding-y);
    opacity: 1;
        ...
        return [
            {
                ...
                label: "Pin open",
                isActive: () => {
                    const target = this._previewStateTarget(".item");
                    if (!target) {
                        return false;
                    }
                    return "class" === "class"
