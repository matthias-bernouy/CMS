...
export function profileFieldReady(field, value) {
    ...
    if (field === "countryCode") {
        return /^[A-Z]{2}$/.test(normalized.toUpperCase());
    }
    if (field === "birthDate") {
        try {
...
export function parseDate(value) {
    ...
    if (!match) {
        throw new PublicError("The birth date must use the YYYY-MM-DD format.");
    }
    ...
    if (date.getUTCFullYear() !== year || date.getUTCMonth() !== month - 1 || date.getUTCDate() !== day) {
        throw new PublicError("Enter a valid birth date.");
    }
