...
export function renderSale(host, order) {
    host.setText("[data-order-number]", order.orderNumber || order.publicId || `Sale ${order.id}`);
    host.setText(
        "[data-order-date]",
        `${host.text("date-prefix", "Sold on")} ${formatDate(order.createdAt, host.locale)}`,
    );
...
function sellerShippingValue(host, order, currency) {
    ...
    if (platformShare === total) {
        return host.text("platform-shipping-label", "Covered by the platform");
    }
    return "—";
}
...
function renderLine(host, line, currency) {
    ...
    title.textContent = line.title || line.offerSnapshot?.title || host.text("fallback-article-label", "Item");
    const details = [
        variantLabel(line.variantSnapshot),
        line.quantity > 1 ? `${host.text("quantity-label", "Quantity")} : ${line.quantity}` : "",
        String(line.offerSnapshot?.conditionLabel || "").trim() || conditionLabel(line.offerSnapshot?.conditionCode),
    ].filter(Boolean);
