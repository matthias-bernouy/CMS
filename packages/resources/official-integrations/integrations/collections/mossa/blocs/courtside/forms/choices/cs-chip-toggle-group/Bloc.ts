...
export class Bloc extends Component {
    ...
    private _onChipToggle = (e: CustomEvent<{ value: string }>) => {
        ...
        if (this.getAttribute("mode") !== "multi") {
            ...
            const chips = this.querySelectorAll(":scope > cs-chip-toggle, :scope > [slot] cs-chip-toggle");
            chips.forEach((c) => {
                if (c !== target) {
    ...
    private _emitChange() {
        ...
        requestAnimationFrame(() => {
            const selected = Array.from(this.querySelectorAll("cs-chip-toggle[selected]")).map(
