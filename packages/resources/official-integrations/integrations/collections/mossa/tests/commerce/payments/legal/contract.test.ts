        ...
        await withFetch(
            ...
            async () => {
                ...
                try {
                    ...
                    expect(payment.root.querySelector("[data-status]")?.textContent).toContain("terms changed");
                } finally {
                    payment.remove();
                }
            },
...
function requirements(versionId: string, includeSecondDocument = false): Record<string, unknown> {
    ...
    if (includeSecondDocument) {
        documents.push({
            ...
            consentText: "I have read the privacy policy.",
            pageUrl: "/privacy",
