    ...
    test("requires both initial consents before tokenization and submits their exact evidence", async () => {
        ...
        await withFetch(
            ...
            async () => {
                const onboarding = await mountStripeConnectOnboarding();
                try {
                    onboarding.clientConfig = async () => ({ publishableKey: "pk_test_example" });
                    ...
                    if (!marketplaceConsent || !paymentConsent || !email || !iban) {
                    ...
                    email.value = "vendeuse@example.test";
                    ...
                    expect(accountTokenCalls[0]?.publishableKey).toBe("pk_test_example");
        ...
        await withFetch(
            ...
            async () => {
                ...
                try {
                    ...
                    expect(onboarding.root.querySelector("[data-status]")?.textContent).toContain("terms changed");
                } finally {
                    onboarding.remove();
                }
            },
