...
describe("consent field", () => {
    test("is binding-native and exposes one stable authoring contract", async () => {
        ...
        expect(view).toContain("/.cms/sources/consent/getRequirements");
        expect(view).not.toContain("source-prefix");
        expect(editor).not.toContain('attribute: "source-prefix"');
    ...
    test.each(["binding-disabled", "bind-stop"])(
        ...
        async (boundary) => {
            ...
            wrapper.innerHTML = await read("default.html");
            core.append(wrapper);
    ...
    test("renders loading, error and ready states through Binding Core and retries globally", async () => {
        ...
        expect(core.textContent).toContain("Required terms");
        expect(core.textContent).toContain("opens in a new tab");
        ...
        expect(firstCopy.getAttribute("data-consent-text")).toBe(
            "J’accepte les Conditions générales de vente du service.",
        );
        ...
        expect(firstCopy.querySelector("[data-consent-suffix]")?.textContent).toBe(" du service.");
...
async function mountRuntime(): Promise<HTMLElement> {
    const core = document.createElement(BINDING_CORE_TAG);
    core.innerHTML = await read("default.html");
    document.body.append(core);
    return core;
}
...
function requirements() {
    return {
        ...
        documents: ["a", "b"].map((prefix, index) => ({
            ...
            consentText:
                index === 0 ? "J’accepte les Conditions générales de vente du service." : "J’accepte Document 2",
            consentPrefix: index === 0 ? "J’accepte les " : "J’accepte ",
            consentSuffix: index === 0 ? " du service." : "",
