...
describe("commerce seller fulfillment Source contexts", () => {
    test("declares three exact service-only actor-scoped projections", async () => {
        ...
        for (const contract of contracts) {
            const endpoint = endpoints.find((item) => item.endpointId === contract.id);
            ...
            expect(Object.keys(body?.properties ?? {})).toEqual(contract.fields);
            expect(body?.required).toEqual(contract.required);
        }

        const label = endpoints.find((item) => item.endpointId === "getOrderLabelSellerContext");
        expect(label?.output?.[0]?.body?.properties?.sellerCmsUserId?.semantic).toEqual({
            kind: "user-id",
            authority: "cms",
        });
...
async function sourceEndpoints(): Promise<Endpoint[]> {
    const path = resolve(import.meta.dir, "../../../definition.json");
