...
create or replace function commerce.get_protected_seller_context(
    p_scope text,
    p_offer_ids bigint[],
    p_order_id bigint,
...
begin
    ...
    if p_scope = 'checkout' then
        ...
        if p_offer_ids is not null or p_order_id is null or p_order_id < 1 then
            return pg_catalog.jsonb_build_object('state', 'invalid_request');
        end if;
        if v_buyer_cms_user_id is null then
            return pg_catalog.jsonb_build_object('state', 'identity_required');
