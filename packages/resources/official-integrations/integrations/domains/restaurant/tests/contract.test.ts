...
describe("restaurant 1.0.0 bloc catalogue", () => {
    test("hydrates and builds every editable restaurant bloc", async () => {
        ...
        expect(definition.kind).toBe("restaurant");
        expect(definition.inputs).toEqual([]);
        expect(definition.dependencies).toEqual([{ name: "basicBlocs", kind: "basic-blocs", versionRange: "^1.0.0" }]);
        expect(definition.artifacts).toHaveLength(restaurantTags.length);
        const foundations = definition.theme?.categories.find((category) => category.id === "foundations");
        expect(foundations?.tokens.find((token) => token.id === "accent")?.defaults).toEqual({
            light: "#39ad65",
            dark: "#39ad65",
        });
        expect(foundations?.tokens.find((token) => token.id === "rating")?.defaults).toEqual({
            light: "#e4b45d",
            dark: "#e4b45d",
        });
    ...
    test("keeps the header reusable and gives every hero a distinct media structure", async () => {
        ...
        expect(headerBaseCss).toContain("--_color: var(--integration-restaurant-text, #fffaf0)");
        expect(headerBaseCss).toContain("--integration-basic-blocs-action-min-height: 2.75rem");
        expect(headerBaseCss).toContain(
            "--integration-basic-blocs-surface-text: var(--integration-restaurant-text, #fffaf0)",
        );
