...
describe("restaurant menu and contact blocs", () => {
    ...
    test("composes contact details from restaurant and Basic Blocs responsibilities", async () => {
        ...
        expect(cardCss).toContain("--integration-basic-blocs-table-font-size: 1rem");
        expect(cardCss).toContain("--integration-basic-blocs-surface-border: var(--_border)");
    ...
    test("provides editable light and dark editorial theme tokens", async () => {
        ...
        const foundations = definition.theme?.categories.find((category) => category.id === "foundations");
        const tokens = new Map(foundations?.tokens.map((token) => [token.id, token.defaults]));

        expect(tokens.get("content-background")).toEqual({ light: "#f5f1e9", dark: "#15171a" });
        expect(tokens.get("content-surface")).toEqual({ light: "#fffaf2", dark: "#1d2124" });
        expect(tokens.get("content-text")).toEqual({ light: "#202421", dark: "#fffaf0" });
        expect(tokens.get("content-muted-text")).toEqual({ light: "#60665f", dark: "#d0cbc1" });
        expect(tokens.get("content-border")).toEqual({ light: "#ddd8cf", dark: "#4c4e4f" });
    });
});
