...
const campaignSelect =
    "id,status,template_key,shared_data,rate_per_minute,scheduled_at,started_at,finished_at,total_count,sent_count,failed_count,skipped_count,created_at,updated_at";
...
export async function getCampaign(request: Request): Promise<Response> {
    ...
    const rows = await restJson<CampaignRow[]>(
        `campaigns?${qs({ select: campaignSelect, id: `eq.${id}`, limit: 1 })}`,
        { method: "GET" },
    );
    if (!rows[0]) {
...
export async function retryFailed(request: Request): Promise<Response> {
    ...
    if (rows[0] && retried) {
        await patchCampaign(id, { failed_count: Math.max(0, rows[0].failed_count - retried) });
    }
