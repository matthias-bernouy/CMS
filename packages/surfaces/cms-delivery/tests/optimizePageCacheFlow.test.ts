...
import { InMemoryCmsFilesMetadata, InMemoryCmsFilesBlob, sha256Hex, P9R_CACHE, type CacheEntry } from "@bernouy/cms-shared";
...
describe("optimizePage — PROD cache flow (the pickup that dev-mode + the integration test skip)", () => {
    test("a cached page is invalidated once its images are optimized → re-renders with the srcset", async () => {
        ...
        const png = new Uint8Array(await sharp({ create: { width: 800, height: 500, channels: 3, background: { r: 50, g: 80, b: 120 } } }).png().toBuffer());
        const hash = sha256Hex(png);
        const file = await files.createFile({ name: "a.png", parentId: null, size: png.byteLength, mimeType: "image/png", contentHash: hash });
        await sourceBlob.put(file.id, png);

        const delivery = new DeliveryCms({ repository: stubRepo, cache, filesMetadata: files, filesBlob: sourceBlob, variantStore });
