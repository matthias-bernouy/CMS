...
describe("repository administration error states", () => {
    for (const fixture of [
        {
            status: 409,
            body: { code: "integration_version_exists", existingDigest: "a".repeat(64) },
            text: "already exists",
        },
        { status: 413, body: { code: "repository_request_too_large" }, text: "larger than the allowed" },
        {
            status: 422,
            body: { code: "integration_compatibility_rejected" },
            text: "Compatibility validation rejected",
        },
        ...
        test(`renders an explicit ${fixture.status} publication error`, async () => {
            installRepositoryFetch((call) => {
                if (call.url.pathname.endsWith("/publications")) {
            ...
            expect(required(console, "[data-upload-feedback]").getAttribute("role")).toBe("alert");
        });
    }
    ...
    test("identifies stale report conflicts and requires a fresh exact confirmation", async () => {
        installRepositoryFetch((call) => {
            if (call.url.pathname.endsWith("/stable-promotions")) {
                return Response.json(
                    { code: "stale_compatibility_report", currentReportRevisionId: "revision-2" },
                    { status: 409 },
                );
            }
            return defaultRepositoryResponse(call);
        ...
        required<HTMLInputElement>(form, '[name="confirmationReportRevisionId"]').value = "revision-1";
        submit(form);
        await waitFor(() => console.textContent?.includes("report is stale") === true);
        expect(console.textContent).toContain("revision-2");
