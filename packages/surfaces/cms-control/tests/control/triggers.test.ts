...
describe("triggers API", () => {
    test("creates a trigger only when its endpoint and function exist", async () => {
        ...
        const response = await createTrigger(
            new Request("http://localhost/cms/api/triggers/create", {
                method: "POST",
    ...
    test("rejects response references in request-phase triggers", async () => {
        ...
        ).rejects.toThrow("request-phase triggers cannot reference $response");
    });
    ...
    test("lists triggers and toggles enabled state", async () => {
        ...
        const toggled = await setTriggerEnabled(
            new Request("http://localhost/cms/api/triggers/enabled", {
                method: "POST",
                headers: { "content-type": "application/json" },
        ...
        expect(await listed.json()).toEqual([expect.objectContaining({ id: "notify-on-order", enabled: true })]);
