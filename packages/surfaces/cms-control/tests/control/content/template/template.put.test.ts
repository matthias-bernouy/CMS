...
import putTemplate from "cms-control/api/_content/template/template.put";
...
describe("PUT /api/template (update)", () => {
    test("throws when id is missing", async () => {
        ...
        await expect(putTemplate(makeRequest({ name: "n" }), cms)).rejects.toThrow(/Missing param id/);
    });

    test("throws when name is missing", async () => {
        const { cms } = makeSystem();
        await expect(putTemplate(makeRequest({ id: "tpl-1" }), cms)).rejects.toThrow(/Missing param name/);
    });

    test("throws when target template does not exist", async () => {
        const { cms, updateCalls } = makeSystem({ updatedTemplate: null });
        await expect(putTemplate(makeRequest({ id: "missing", name: "n" }), cms)).rejects.toThrow(
            /Unknown template id/,
        );
