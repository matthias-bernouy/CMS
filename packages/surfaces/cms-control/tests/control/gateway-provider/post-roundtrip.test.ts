...
describe("POST /api/gateway-provider round-trips", () => {
    test("query-param description round-trips", async () => {
        ...
        expect((await gateway.getProvider("urn:shop"))?.endpoints[0]!.input?.params?.[0]).toEqual(
    ...
    test("endpoint meta round-trips verbatim", async () => {
        ...
        expect((await gateway.getProvider("urn:shop"))?.endpoints[0]!.meta).toEqual(meta as any);
    });
    ...
    test("name-less endpoint meta is dropped, not fatal", async () => {
        ...
        expect((await gateway.getProvider("urn:shop"))?.endpoints[0]!.meta).toBeUndefined();
    });
