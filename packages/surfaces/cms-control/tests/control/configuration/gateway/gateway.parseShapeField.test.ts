...
import { makeNode } from "cms-control/components/admin/EndpointsInput/editors/bodyNode";
import { makeResponseRow } from "cms-control/components/admin/EndpointsInput/rows/responseRow";
...
describe("endpoint DataShape editor", () => {
    test("preserves nullable modifiers while reconstructing a seeded shape", () => {
        ...
        const node = makeNode(shape, () => undefined);
        document.body.append(node.typeEl, node.childrenEl);
        expect(node.read()).toEqual(shape);
    });
