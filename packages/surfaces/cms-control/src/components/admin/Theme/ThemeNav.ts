...
export class CmsThemeNav extends Component {
    ...
    private render(): void {
        ...
        for (const source of this.sources) {
            ...
            if (source.id !== this.selection.sourceId) continue;
    ...
    private selectionFromUrl(): ThemeSelection {
        ...
        const source = explicitSource ?? this.sources.find((item) => item.categories.some((category) => category.id === categoryId)) ?? this.sources[0];
        const category = source?.categories.find((item) => item.id === categoryId) ?? source?.categories[0];
    ...
    private onClick = (event: Event): void => {
        ...
        if (source?.dataset.source) this.select(source.dataset.source);
    };
    ...
    private onCategoryAdded = (event: CustomEvent<ThemeCategoryAdded>): void => {
        ...
        const source = this.sources.find((item) => item.id === sourceId);
        if (!source || !category || source.categories.some((item) => item.id === category.id)) return;
    ...
    private onCategoryUpdated = (event: CustomEvent<ThemeCategoryAdded>): void => {
        const { sourceId, category } = event.detail ?? {};
        const current = this.sources.find((item) => item.id === sourceId)
...
if (!customElements.get("cms-theme-nav")) customElements.define("cms-theme-nav", CmsThemeNav);
...
function sourceIconPaths(sourceId: string): string {
    if (sourceId === "colors") return '<circle cx="13.5" cy="6.5" r=".5" fill="currentColor"/><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="8.5" cy="7" r=".5" fill="currentColor"/><path d="M12 2a10 10 0 0 0 0 20c.9 0 1.7-.13 2.45-.35 1.1-.33 1.5-1.65.9-2.65-.58-.98-1.35-1.45-1.35-2.4 0-1.1.9-2 2-2h4c1.1 0 2-.9 2-2A10 10 0 0 0 12 2Z"/>';
    if (sourceId === "typography") return '<path d="M4 5V3h16v2"/><path d="M12 3v18"/><path d="M8 21h8"/>';
    if (sourceId === "spacing") return '<path d="M4 6h16"/><path d="M4 12h10"/><path d="M4 18h16"/><path d="M17 9v6"/>';
