import type { FunctionCatalogSource, FunctionDetail, FunctionExecutionResult } from "./contracts";

export type {
    FunctionCatalogEndpoint,
...
export async function fetchSourceEndpoint(
    ...
    const url = new URL(
        route(`/.cms/sources/${encodeURIComponent(source)}/${encodeURIComponent(endpoint)}`),
