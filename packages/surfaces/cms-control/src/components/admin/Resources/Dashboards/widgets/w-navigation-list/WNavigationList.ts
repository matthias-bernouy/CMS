...
export class DashboardWNavigationList extends Component {
    ...
    static get observedAttributes(): string[] { return ["data-config-json"]; }
    attributeChangedCallback(): void { this.syncConfig(); if (this.isConnected) this.render(); }
    ...
    private syncConfig(): void {
        ...
        if (widget?.widget === "w-navigation-list") this.value = widget;
    }
    ...
    private render(): void {
        ...
        this.query<HTMLElement>("[data-actions]").replaceChildren(...this.visibleActions().map(action => {
            const button = document.createElement("p9r-button");
            button.dataset.action = action.id;
            button.dataset.widget = widget.id;
            if (action.selection?.opens) button.dataset.target = action.selection.opens;
    ...
    private onDragStart = (event: DragEvent): void => {
        ...
        if (!item || !this.value?.reorderable) return;
        this.dragging = item;
    ...
    private onDrop = (event: DragEvent): void => {
        ...
        if (!target || !dragging || target === dragging || !this.value?.reorderable) return;
        event.preventDefault();
        const movesDown = Boolean(dragging.compareDocumentPosition(target) & Node.DOCUMENT_POSITION_FOLLOWING);
        if (movesDown) this.insertBefore(dragging, target.nextSibling);
        else this.insertBefore(dragging, target);
    ...
    private clearDragState(): void {
        this.dragging = null;
        this.items().forEach(item => { item.removeAttribute("data-dragging"); item.removeAttribute("data-drop-target"); });
    }
    private items(): DashboardWNavigationItem[] { return Array.from(this.querySelectorAll<DashboardWNavigationItem>("cms-dashboard-w-navigation-item")); }
    private query<T extends Element>(selector: string): T { return this.shadowRoot!.querySelector(selector) as T; }
}

if (!customElements.get("cms-dashboard-w-navigation-list")) customElements.define("cms-dashboard-w-navigation-list", DashboardWNavigationList);

function dragItem(event: Event): DashboardWNavigationItem | null {
    const fromPath = event.composedPath().find((target): target is DashboardWNavigationItem =>
        target instanceof HTMLElement && target.matches("cms-dashboard-w-navigation-item"));
    if (fromPath) return fromPath;
    const target = event.target;
    return target instanceof Element ? target.closest<DashboardWNavigationItem>("cms-dashboard-w-navigation-item") : null;
}

function parseJson<T>(value: string): T | null { try { return value ? JSON.parse(value) as T : null; } catch { return null; } }
