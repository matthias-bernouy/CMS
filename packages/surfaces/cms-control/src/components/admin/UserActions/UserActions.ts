...
export class CmsUserActions extends Component {
    ...
    private async run(action: Action): Promise<void> {
        ...
        if (!sub) return;
        if (action === "delete" && !confirm("Delete this user? Their account, local password and access tokens are removed. This cannot be undone.")) return;
        ...
        if (action === "delete") window.location.href = `${this.basePath}/admin/users`;
        else document.dispatchEvent(new BubblesEvent(this.getAttribute("emit") ?? "user:updated"));
    }

    private request(action: Action, sub: string): RequestInit {
        if (action === "delete") return { method: "DELETE" };
        return {
            method: "POST",
    ...
    private url(action: Action): string {
        ...
        if (action !== "delete") return endpoint;
...
async function errorMessage(res: Response | null): Promise<string> {
    ...
    try { return JSON.parse(text)?.error ?? (text || `HTTP ${res.status}`); }
...
if (!customElements.get("cms-user-actions")) customElements.define("cms-user-actions", CmsUserActions);
