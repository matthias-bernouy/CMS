import { makeHeaderRow, type HeaderRowHandle } from "../rows/headerRow";
...
export function makeHeadersPanel(endpointIdx: number, seed: EndpointSeed, opts?: { api?: string }): HTMLElement {
    ...
    const readRows = () => handles.map((h) => h.read()).filter((r): r is NonNullable<typeof r> => r !== null);
    const sync = () => field.sync(readRows);

    const addRow = (h: Partial<EndpointHeader>) => {
        const handle = makeHeaderRow(
            h,
            sync,
            () => {
