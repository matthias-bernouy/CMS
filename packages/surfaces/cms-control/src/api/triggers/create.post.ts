...
import { createTriggerDefinition } from "cms-control/core/control/workflows/triggerCreation";

export default async function createTrigger(req: Request, cms: ControlCms): Promise<Response> {
    if (!cms.triggers) return new Response("triggers not configured", { status: 501 });
