...
export function parseSourceDto(body: Record<string, unknown>): SourceDto {
    ...
    if (!id) {
        throw new InvalidParam("id", "cannot derive an id");
    }
    if (isSystemSourceId(id)) {
        throw new InvalidParam("id", `reserved prefix "${SYSTEM_SOURCE_ID_PREFIX}"`);
    }

    // Group flat scalar keys by row index; collect per-endpoint JSON blobs separately.
    const rows = new Map<
        number,
        Partial<Record<"endpointId" | "method" | "targetUrl" | "timeoutMs" | "responseKind" | "mediaType", string>>
    >();
    ...
    for (const idx of [...rows.keys()].sort((a, b) => a - b)) {
        ...
        const queryParams = parseParamsBlob(
            blobs?.params,
            new Set(pathParams.map((p) => p.name)),
            `endpoints.${idx}.params`,
        );
        const body = parseShapeField(blobs?.body, `endpoints.${idx}.body`);
        const output = parseResponsesBlob(
            typeof blobs?.output === "string" ? blobs.output : undefined,
            `endpoints.${idx}.output`,
        );
