...
export function pathParamsFromUrl(targetUrl: string): EndpointParamDto[] {
    return extractPathParamNames(targetUrl).map((name) => ({
        name,
        in: "path" as const,
...
export function parseParamsBlob(raw: unknown, reserved: ReadonlySet<string>, path: string): EndpointParamDto[] {
    ...
    if (!Array.isArray(parsed)) {
        throw new InvalidParam(path, "expected an array");
    }
    const out: EndpointParamDto[] = [];
    const seen = new Set<string>(reserved);
    parsed.forEach((el, i) => {
        if (typeof el !== "object" || el === null) {
            throw new InvalidParam(`${path}.${i}`, "expected an object");
        }
        const p = el as Record<string, unknown>;
        const name = (typeof p.name === "string" ? p.name : "").trim();
        if (!name) {
            return; // unfilled row → skip
        }
        if (seen.has(name)) {
            throw new InvalidParam(`${path}.${i}.name`, "duplicate param name");
        }
        seen.add(name);
...
function parseParamSource(raw: unknown, path: string): ParamValueSource {
    ...
    if (typeof raw !== "object" || Array.isArray(raw)) {
        throw new InvalidParam(path, "expected an object.");
    }
    ...
    if (source.from !== "computed") {
        throw new InvalidParam(`${path}.from`, "must be request|computed.");
    }
...
export function parseMetaField(raw: unknown): SourceMeta | undefined {
    ...
    if (!name) {
        return undefined; // SourceMeta requires a name → drop
    }
