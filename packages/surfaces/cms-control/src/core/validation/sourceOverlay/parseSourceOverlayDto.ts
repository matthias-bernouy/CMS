...
export function parseSourceOverlayDto(body: Record<string, unknown>): SourceOverlay {
    ...
    if (body.input !== undefined) overlay.input = parseTargets(body.input, "input");
    if (body.output !== undefined) overlay.output = parseTargets(body.output, "output");
...
function parseFieldSourceMap(value: unknown): SourceOverlayFieldSourceMap {
    if (!isRecord(value)) throw new InvalidParam("fieldSource.map", "must be an object.");
    return Object.fromEntries(Object.entries(value)
        .filter(([, path]) => path !== undefined)
        .map(([key, path]) => [key, fieldPath(path, `fieldSource.map.${key}`)])) as SourceOverlayFieldSourceMap;
}

function parseFields(value: unknown): SourceOverlayField[] {
    if (!Array.isArray(value)) throw new InvalidParam("fields", "must be an array.");
    const seen = new Set<string>();
    return value.map((entry, index) => {
        if (!isRecord(entry)) throw new InvalidParam(`fields.${index}`, "must be an object.");
        const id = requiredId(entry.id, `fields.${index}.id`);
        if (seen.has(id)) throw new InvalidParam(`fields.${index}.id`, "duplicate field id.");
        seen.add(id);
        return {
            id,
...
function parseSections(value: unknown): SourceOverlaySection[] {
    if (!Array.isArray(value)) throw new InvalidParam("sections", "must be an array.");
    const seen = new Set<string>();
    return value.map((entry, index) => {
        if (!isRecord(entry)) throw new InvalidParam(`sections.${index}`, "must be an object.");
        const id = requiredId(entry.id, `sections.${index}.id`);
        if (seen.has(id)) throw new InvalidParam(`sections.${index}.id`, "duplicate section id.");
        seen.add(id);
        return {
            id,
            label: requiredText(entry.label, `sections.${index}.label`),
            ...(entry.description !== undefined ? { description: requiredText(entry.description, `sections.${index}.description`) } : {}),
        };
    });
}
...
function parseTargets(value: unknown, name: string): SourceOverlayEndpointTarget[] {
    ...
    return value.map((entry, index) => {
        ...
        return {
            ...
            ...(entry.editable !== undefined ? { editable: editableScope(entry.editable, `${name}.${index}.editable`) } : {}),
        };
    });
}
...
function requiredId(value: unknown, name: string): string {
    ...
    if (!SIMPLE_ID.test(result)) throw new InvalidParam(name, "must be a simple id.");
    return result;
}
...
function fieldType(value: unknown, name: string): SourceOverlayFieldType {
    if ((SOURCE_OVERLAY_FIELD_TYPES as readonly unknown[]).includes(value)) return value as SourceOverlayFieldType;
...
function editableScope(value: unknown, name: string): SourceOverlayEditableScope {
    if ((SOURCE_OVERLAY_EDITABLE_SCOPES as readonly unknown[]).includes(value)) return value as SourceOverlayEditableScope;
...
function fieldPath(value: unknown, name: string): string {
    const result = requiredText(value, name);
    if (!FIELD_PATH.test(result)) throw new InvalidParam(name, "must be a dotted object path.");
    return result;
}
...
function targetPath(value: unknown, name: string): string {
    if (value === "") return "";
    const result = requiredText(value, name);
    if (!SAFE_PATH.test(result)) throw new InvalidParam(name, "must be a dotted shape path.");
    return result;
}
