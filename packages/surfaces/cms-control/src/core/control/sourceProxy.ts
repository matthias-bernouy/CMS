...
export function mountControlSourceProxy(
    ...
    const authorizeEndpoint = async (endpoint: SourceEndpoint, req: Request) => {
        ...
        if (!sourceEndpointAccessAllows(sourceEndpointAccessMode(endpoint), controlCallerAccessMode(subject.role))) return false;
    ...
    const overlaySources = state.sources && state.sourceOverlays
        ...
        : state.sources;
    const proxiedSources = overlaySources && state.functions ? withFunctionsSource(overlaySources, state.functions) : overlaySources;
    ...
    const executeSystemEndpoint = async (endpoint: SourceEndpoint, req: Request) => {
        if (endpoint.urn.startsWith(`${SYSTEM_FUNCTIONS_SOURCE_URN}:`)) {
            if (!state.functions || !state.sources) return new Response("function executor not configured", { status: 501 });
            const subject = await state.auth.getSubject(req).catch(() => null);
    ...
    runner.group(CMS_SOURCES_ROUTE, (proxyRunner) => {
        ...
        for (const method of SOURCE_PROXY_METHODS) {
            proxyRunner.setDefaultEndpoint(method, (req) => handleSourceRequest(proxiedSources, req, {
                prefix,
                deps: { resolveSecret, resolveContext, executeSystemEndpoint, authorizeEndpoint, ...(interceptEndpoint ? { interceptEndpoint } : {}) },
            }));
        }
    }, [authGuard]);
}
