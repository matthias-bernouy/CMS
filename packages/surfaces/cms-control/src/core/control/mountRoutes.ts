...
export function mountControlCmsRoutes(
    ...
    if (controlPublicAuth) {
        runner.group(PUBLIC_AUTH_ROUTES.base, (authRunner) => registerPublicAuthRoutes(authRunner, controlPublicAuth));
    }
    ...
    mountControlSourceProxy(cms, state, authGuard, controlPublicAuth);
    ...
    runner.group("/", (staticRunner) => {
        serveStaticFolder(staticRunner, { cache: state.cache, cspExtras: () => cms.getCspExtras() });
    }, [authGuard]);
