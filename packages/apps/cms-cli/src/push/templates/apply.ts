...
export async function applyPushTemplates(
    ...
    for (const e of entries.filter(e => e.status === "new")) {
        try { await postTemplate(adminBase, token, e.template); }
        catch (err) { result.failed.push({ identifier: e.template.identifier, error: msg(err) }); }
    }

    const idByIdentifier = new Map(
        (await fetchRemoteTemplateList(adminBase, token)).map(t => [t.identifier, t.id]),
    );

    for (const e of writes) {
        if (result.failed.some(f => f.identifier === e.template.identifier)) continue;
