...
- Custom domain aliases per bucket — admin UI + cascade-on-bucket-delete,
  ...
  certs can be issued via `lego` straight from the admin UI (HTTP-01 or
  DNS-01 — `provider.config.aliasIssuer`); a `POST /admin/api/aliases/renew`
  endpoint drives `lego renew` across the bucket on a cron.
...
- **[docs/tests/local-scenario.md](./docs/tests/local-scenario.md)** —
  ...
  meaningful change.
- **[docker/impl-prod/](./docker/impl-prod/)** — single-container reference
  image bundling Nginx + lego + Bun. `docker compose up`, env-driven, with
  first-boot wildcard cert provisioning via DNS-01. See its
  [README](./docker/impl-prod/README.md).
