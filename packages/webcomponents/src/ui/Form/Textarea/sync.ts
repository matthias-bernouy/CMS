...
export const syncHint = (
    host: HTMLElement, hint: HTMLElement | null, counter: HTMLElement | null, meta: HTMLElement | null,
) => {
    if (!hint) return;
...
export const syncInvalid = (host: HTMLElement, textarea: HTMLTextAreaElement | null) => {
    if (!textarea) return;
    if (host.hasAttribute('invalid')) textarea.setAttribute('aria-invalid', 'true');
    else textarea.removeAttribute('aria-invalid');
};

export const syncMaxCount = (
    host: HTMLElement,
    counter: HTMLElement | null, max: HTMLElement | null,
    hint: HTMLElement | null, meta: HTMLElement | null,
) => {
    if (!counter || !max) return;
    const value = parseMaxCount(host);
    if (value === null) counter.hidden = true;
    else { counter.hidden = false; max.textContent = String(value); }
    refreshMetaVisibility(hint, counter, meta);
};

export const syncAll = (
    host: HTMLElement,
    textarea: HTMLTextAreaElement | null, label: HTMLLabelElement | null,
    hint: HTMLElement | null, meta: HTMLElement | null,
    counter: HTMLElement | null, max: HTMLElement | null,
) => {
