(()=>{class a extends HTMLElement{constructor(e){super();let t=this.attachShadow({mode:"open"});if(e){let r=document.createElement("style");r.textContent=e.css,t.appendChild(r);let n=document.createElement("template");n.innerHTML=e.template,t.appendChild(n.content.cloneNode(!0))}}connectedCallback(){}}var y=`:host { display: block; padding: 1.75rem 2rem; max-width: 1280px; }
...
`;class g extends a{constructor(){super({css:y,template:`
                <header part="header">
...
`;class b extends a{static get observedAttributes(){return["label"]}constructor(){super({css:A,template:'<h2 part="title" hidden></h2><div part="body"><slot></slot></div>'})}connectedCallback(){this._sync()}attributeChangedCallback(){this._sync()}_sync(){let e=this.shadowRoot?.querySelector("[part='title']");if(!e)return;let t=this.getAttribute("label")??"";e.textContent=t,e.hidden=t===""}}if(!customElements.get("hub-section"))customElements.define("hub-section",b);var h=`:host { display: block; }
...
`;class k extends a{constructor(){super({css:h,template:`
                <div part="grid"><slot></slot></div>
                <div part="empty"><slot name="empty"></slot></div>`})}connectedCallback(){let e=this.shadowRoot.querySelector("slot:not([name])");e.addEventListener("slotchange",()=>this._sync(e)),this._sync(e)}_sync(e){let t=e.assignedElements().filter((r)=>r.tagName!=="W13C-FETCH"&&r.tagName!=="TEMPLATE").length;this.shadowRoot.querySelector("[part='empty']").hidden=t>0}}if(!customElements.get("hub-grid"))customElements.define("hub-grid",k);var E=`:host { display: block; }
...
`;class M extends a{static get observedAttributes(){return["label","sub","href"]}constructor(){super({css:v,template:`
                <a part="tile">
                    <span class="label"></span>
                    <span class="value"><slot></slot></span>
                    <span class="sub"></span>
                </a>`})}connectedCallback(){this._sync()}attributeChangedCallback(){this._sync()}_sync(){let e=this.shadowRoot;if(!e)return;let t=e.querySelector("a"),r=this.getAttribute("href");if(r)t.setAttribute("href",r);else t.removeAttribute("href");e.querySelector(".label").textContent=this.getAttribute("label")??"";let n=e.querySelector(".sub");n.textContent=this.getAttribute("sub")??"",n.hidden=!this.getAttribute("sub")}}if(!customElements.get("hub-stat"))customElements.define("hub-stat",M);var z=`:host { display: inline; }
...
`;class B extends a{constructor(){super({css:R,template:"<slot></slot>"})}}if(!customElements.get("hub-hint"))customElements.define("hub-hint",B);var U=`<pre part="code"></pre>
`;var $=`:host {
    display: block;
}
...
`;class V extends a{_issuers=[];constructor(){super({css:P,template:"<div part='root'></div>"})}connectedCallback(){this._load()}get _tenantId(){return this.getAttribute("tenant-id")||m("tenantId")}get _root(){return this.shadowRoot.querySelector("[part='root']")}async _load(){let e=await u(`/api/namespaces/tenants?tenantId=${encodeURIComponent(this._tenantId)}`);this._issuers=e?.issuers?[...e.issuers]:[],this._render()}_render(){let e=this._root;e.innerHTML="";let t=document.createElement("div");t.className="list",this._issuers.forEach((l,d)=>t.appendChild(this._row(l,d))),e.appendChild(t);let r=document.createElement("div");r.className="actions";let n=document.createElement("p9r-button");n.setAttribute("variant","ghost"),n.textContent="+ Add an issuer",n.addEventListener("click",()=>{this._issuers.push(""),this._render()});let o=document.createElement("p9r-button");o.setAttribute("color","primary"),o.textContent="Save trust-list",o.addEventListener("click",()=>void this._save()),r.append(n,o),e.appendChild(r);let i=document.createElement("div");i.className="msg",e.appendChild(i)}_row(e,t){let r=document.createElement("div");r.className="row";let n=document.createElement("p9r-input");n.setAttribute("label",`Issuer ${t+1}`),n.setAttribute("placeholder","https://keycloak.example/realms/acme"),n.setAttribute("value",e),n.addEventListener("input",()=>{this._issuers[t]=n.value??n.getAttribute("value")??""});let o=document.createElement("p9r-button");return o.setAttribute("variant","ghost"),o.setAttribute("color","danger"),o.textContent="✕",o.addEventListener("click",()=>{this._issuers.splice(t,1),this._render()}),r.append(n,o),r}async _save(){let e=this.shadowRoot.querySelector(".msg");e.innerHTML="";let t=this._issuers.map((o)=>o.trim()).filter(Boolean),r=await x("PATCH",`/api/namespaces/tenants?tenantId=${encodeURIComponent(this._tenantId)}`,{issuers:t}),n=document.createElement("p9r-alert");if(r.ok)n.setAttribute("type","success"),n.textContent="Trust-list saved.",this._issuers=t,document.dispatchEvent(new CustomEvent("tenant:updated",{bubbles:!0}));else n.setAttribute("type","error"),n.textContent=`Failed: ${r.message}`;e.appendChild(n)}}if(!customElements.get("hub-tenant-issuers"))customElements.define("hub-tenant-issuers",V);var F=`:host { display: block; }

form { display: flex; flex-direction: column; gap: 1rem; align-items: flex-start; }

.msg:empty { display: none; }
`;class T extends a{constructor(){super({css:F,template:"<div part='root'></div>"})}connectedCallback(){this._load()}get _tenantId(){return this.getAttribute("tenant-id")||m("tenantId")}get _root(){return this.shadowRoot.querySelector("[part='root']")}_info(e,t){let r=document.createElement("p9r-alert");if(e)r.setAttribute("type",e);r.textContent=t,this._root.replaceChildren(r)}async _load(){this._root.textContent="Loading…";let e=this._tenantId,t=await u(`/api/namespaces/tenants?tenantId=${encodeURIComponent(e)}`);if(!t){this._info("error","Unknown tenant.");return}let n=(await u(`/api/providers?providerId=${encodeURIComponent(t.providerId)}`))?.schemas?.tenantConfig??null;if(!n){this._info("","This data-provider declares no tenantConfig — nothing to edit here.");return}let o=await u(`/api/namespaces/tenants/config?tenantId=${encodeURIComponent(e)}`);this._renderForm(n,o?.config??{})}_renderForm(e,t){this._root.innerHTML="";let r=document.createElement("form"),n=document.createElement("hub-config-form");n.setAttribute("mode","control-plane"),n.setAttribute("prefix","providerConfig"),n.setAttribute("schema",JSON.stringify(e)),n.setAttribute("value",JSON.stringify(t)),r.appendChild(n);let o=document.createElement("p9r-button");o.setAttribute("color","primary"),o.setAttribute("type","submit"),o.textContent="Save config",r.appendChild(o);let i=document.createElement("div");i.className="msg",r.appendChild(i),r.addEventListener("submit",async(l)=>{l.preventDefault(),i.innerHTML="";let d=await x("PATCH",`/api/namespaces/tenants?tenantId=${encodeURIComponent(this._tenantId)}`,{providerConfig:n.getValues()}),c=document.createElement("p9r-alert");if(d.ok)c.setAttribute("type","success"),c.textContent="Config saved.",document.dispatchEvent(new CustomEvent("tenant:updated",{bubbles:!0}));else c.setAttribute("type","error"),c.textContent=`Failed: ${d.message}`;i.appendChild(c)}),this._root.appendChild(r)}}if(!customElements.get("hub-tenant-config-form"))customElements.define("hub-tenant-config-form",T);var O=`:host { display: block; }
...
`;class I extends a{_items=[];_cursor=void 0;_kind="";_level="";_onTenantEvent=()=>{this._load(!0)};constructor(){super({css:q,template:"<div part='root'></div>"})}connectedCallback(){this._renderShell(),this._load(!0);for(let e of["tenant:created","tenant:updated","tenant:removed"])document.addEventListener(e,this._onTenantEvent)}disconnectedCallback(){for(let e of["tenant:created","tenant:updated","tenant:removed"])document.removeEventListener(e,this._onTenantEvent)}get _src(){return this.getAttribute("src")||""}get _root(){return this.shadowRoot.querySelector("[part='root']")}_scopeParam(){for(let e of["tenantId","providerId","namespaceId"]){let t=m(e);if(t)return`${e}=${encodeURIComponent(t)}`}return""}_renderShell(){let e=this._root;e.innerHTML="";let t=document.createElement("div");t.className="bar",t.append(this._select("kind","Kind",["all","security","audit","request"],(i)=>{this._kind=i,this._load(!0)}),this._select("level","Level",["all","debug","info","warn","error"],(i)=>{this._level=i,this._load(!0)}));let r=document.createElement("div");r.className="rows";let n=document.createElement("p9r-button");n.setAttribute("variant","ghost"),n.className="more",n.textContent="Load more",n.addEventListener("click",()=>void this._load(!1));let o=document.createElement("p9r-alert");o.setAttribute("type","info"),o.className="empty",o.textContent="No log entries.",e.append(t,r,o,n)}_select(e,t,r,n){let o=document.createElement("p9r-select");o.setAttribute("name",e),o.setAttribute("label",t);for(let i of r){let l=document.createElement("option");if(l.value=i,l.textContent=i==="all"?"All":i,i==="all")l.setAttribute("selected","");o.appendChild(l)}return o.addEventListener("change",()=>{let i=o.value??o.getAttribute("value")??"all";n(i==="all"?"":i)}),o}async _load(e){if(e)this._items=[],this._cursor=void 0;let t=this._scopeParam();if(!this._src||!t)return;let r=new URLSearchParams(t);if(this._kind)r.set("kind",this._kind);if(this._level)r.set("level",this._level);if(r.set("limit","50"),this._cursor)r.set("cursor",this._cursor);let o=await u(`${this._src}?${r.toString()}`)??{items:[],nextCursor:void 0};this._items=e?o.items:[...this._items,...o.items],this._cursor=o.nextCursor,this._renderRows()}_renderRows(){let e=this.shadowRoot.querySelector(".rows"),t=this.shadowRoot.querySelector(".empty"),r=this.shadowRoot.querySelector(".more");e.innerHTML="",t.hidden=this._items.length>0,r.hidden=!this._cursor;for(let n of this._items){let o=document.createElement("div");o.className=`row level-${n.level}`,o.innerHTML=`
                ...
                <span class="meta">${this._esc(n.outcome??"")}${n.tenantId?" · "+this._esc(n.tenantId):""}</span>`,e.appendChild(o)}}_esc(e){return String(e).replace(/[&<>"]/g,(t)=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[t])}}if(!customElements.get("hub-logs"))customElements.define("hub-logs",I);function ae(e){if(!e)return"An error occurred.";let t=e.body;if(t&&typeof t==="object"&&t.error){let r=t.error;return`${r.code||"error"}: ${r.message||""}`.trim()}return`HTTP ${e.status??"?"}`}function ce(e){let t=e.querySelector(":scope > p9r-alert.hub-form-error");if(!t)t=document.createElement("p9r-alert"),t.setAttribute("type","error"),t.classList.add("hub-form-error"),e.appendChild(t);return t}document.addEventListener("form:failed",(e)=>{let t=e.target;if(!(t instanceof Element))return;let r=t.closest("p9r-modal")||t.closest("dialog")||t.parentElement||document.body;ce(r).textContent=ae(e.detail)});document.addEventListener("form:success",(e)=>{let t=e.target;if(!(t instanceof Element))return;(t.closest("p9r-modal")||t.parentElement)?.querySelector(":scope > p9r-alert.hub-form-error")?.remove()});})();
