...
const DP_URL  = "https://delivery.acme.example";
...
function makeFakeDpFetch(opts: {
    ...
    const issBase         = opts.issBaseUrl      ?? DP_URL;
    const jwksUri         = opts.jwksUri         ?? `${DP_URL}/jwks.json`;

    return (async (input: RequestInfo | URL, init?: RequestInit) => {
        const url    = typeof input === "string" ? input : input.toString();
        const method = init?.method ?? "GET";

        if (url === `${DP_URL}/.well-known/data-provider-info`) {
            return new Response(JSON.stringify({
                providerId, providerKind: "delivery", contractVersion,
                defaultDeprovisionPolicy: { mode: "immediate" },
            }), { status: 200 });
        }
        if (url === `${DP_URL}/.well-known/oauth-authorization-server`) {
            return new Response(JSON.stringify({
                issuer: issBase, jwks_uri: jwksUri,
                signing_alg_values_supported: ["ES256"],
                data_provider_contract_version: contractVersion,
            }), { status: 200 });
        }
        if (url === `${DP_URL}/jwks.json`) {
            return new Response(JSON.stringify({ keys: [] }), { status: 200 });
        }
        if (url === `${DP_URL}/openapi.admin.json`) {
        ...
        if (url === `${DP_URL}/openapi.tenant-config.json`) {
            return new Response(JSON.stringify({ version: "1.0", properties: {} }), { status: 200 });
        }
        if (url.startsWith(`${DP_URL}/admin/tenants`) && method === "POST") {
        ...
        if (url.startsWith(`${DP_URL}/admin/tenants`) && method === "DELETE") {
            return new Response(null, { status: 204 });
        }
        if (url.startsWith(`${DP_URL}/admin/tenants`) && method === "PATCH") {
...
describe("DP registry — happy paths", () => {
    test("importDataProvider fetches + persists", async () => {
        const hub = makeHub(makeFakeDpFetch());
        const dp = await hub.importDataProvider({ url: DP_URL, name: "Acme Delivery" });
        expect(dp.providerId).toBe("delivery-acme");
        expect(dp.providerKind).toBe("delivery");
        expect(dp.iss).toBe(DP_URL);
    ...
    test("duplicate import → duplicate_provider_id", async () => {
        ...
        try { await hub.importDataProvider({ url: DP_URL }); throw new Error("nope"); }
    ...
    test("unimport removes the row", async () => {
        const hub = makeHub(makeFakeDpFetch());
        await hub.importDataProvider({ url: DP_URL });
        await hub.unimportDataProvider("delivery-acme");
        expect(await hub.getDataProvider("delivery-acme")).toBeNull();
    });
});
...
describe("Namespace registry + tenants", () => {
    ...
    test("multiple tenants of the same DP per namespace", async () => {
        const hub = makeHub(makeFakeDpFetch());
        await hub.importDataProvider({ url: DP_URL });
        await hub.createNamespace({ namespaceId: "acme", displayName: "ACME Corp" });
    ...
    test("create on unknown DP → provider_not_found", async () => {
        const hub = makeHub(makeFakeDpFetch());
...
describe("DP registry — error paths", () => {
    test("DP refuses admin → provider_does_not_trust_hub", async () => {
    ...
    test("activate DP returns non-2xx → provider_rejected_provisioning", async () => {
