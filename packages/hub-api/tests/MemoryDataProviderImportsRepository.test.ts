...
function fixture(over: Partial<DataProviderImport> = {}): DataProviderImport {
    return {
...
describe("MemoryDataProviderImportsRepository", () => {
    test("insert + getByProviderId round-trip", async () => {
        const r = new MemoryDataProviderImportsRepository();
        const doc = fixture();
        await r.insert(doc);
    ...
    test("updateSchemas updates fields, preserves _id-like data", async () => {
        const r = new MemoryDataProviderImportsRepository();
        await r.insert(fixture());
        const newSchemas: DataProviderImport["schemas"] = {
            info:         { x: 1 },
            metadoc:      { y: 2 },
            jwks:         { keys: [{ kty: "EC" }] },
    ...
    test("updateSchemas on unknown providerId returns null", async () => {
        const r = new MemoryDataProviderImportsRepository();
        expect(await r.updateSchemas("ghost", {} as DataProviderImport["schemas"], new Date())).toBeNull();
    });
