import {
    metadocPath, TENANT_PROVISIONER_CONTRACT_VERSION,
...
/** Per-fetch timeout for discovery — a hanging TP must not stall the import
...
/**
 * Fetch + validate the 4 discovery documents of a tenant-provisioner, given its
 * base URL. Returns the data the hub needs to persist.
 *
 * Order: tenant-provisioner-info → RFC 8414 metadoc → JWKS → openapi.admin (CP)
...
export async function discoverEndpoints(opts: {
    ...
    const info = await fetchJson(f, `${url}/.well-known/tenant-provisioner-info`, withTimeout());
    ...
    if (!providerId) {
        throw new HubError("provider_metadoc_invalid", `tenant-provisioner-info at ${url} missing providerId`);
    }
    if (contractMajor !== TENANT_PROVISIONER_CONTRACT_VERSION.split(".")[0]) {
        throw new HubError("provider_metadoc_invalid",
            `tenant-provisioner at ${url} declares contractVersion=${info["contractVersion"]}, hub speaks ${TENANT_PROVISIONER_CONTRACT_VERSION} — major mismatch`);
    }
    ...
    // Anti-SSRF: `jwks_uri` is remote-controlled (it comes from the TP metadoc),
    // so it must stay same-origin as the TP — otherwise a hostile/typo'd TP
    ...
    const onAuth = (status: number) => (status === 401 || status === 403)
        ? new HubError("provider_does_not_trust_hub",
            `${url} refused the hub's CP token. Ensure the TP's allowlist contains { iss: <hub.iss>, role: "control-plane" }.`)
        : null;
    ...
    // twice against the same TP. `mintCp` is expected to return a fresh token
    // each call here (importTenantProvisioner/refreshTenantProvisioner pass an uncached
