...
const IssuerInfoSchema = z.object({
    ...
    jwksUrl:       z.string(),
    metadocUrl:    z.string(),
    /** Snippet to paste into a data-provider's `allowlist` config so it
     *  trusts the hub as control-plane. Helps the operator with the trust
     *  setup that must be done BEFORE importing the DP (cf. D7). */
...
export const meta: ApiOperationMeta = {
    ...
    description: "Returns the hub's iss URL, JWKS URL, metadoc URL, and a ready-to-copy allowlist entry the operator pastes into each DP's config before importing it.",
