import { encryptAesGcm, decryptAesGcm, type EncryptedBlob } from "./aesGcm";
import type { KekProvider } from "./KekProvider";
import type { SecretCrypto } from "./SecretCrypto";
import type { DekRepository } from "./DekRepository";
...
export class EnvelopeSecretCrypto implements SecretCrypto {
     ...
     * scope. Without it, three parallel encrypts on a fresh scope each
     * see no row, each generate + upsert their own DEK, and the last
     * write silently invalidates the first two ciphertexts.
     */
    ...
    private async _fetchOrCreate(scopeId: string): Promise<Buffer> {
        ...
        this._remember(scopeId, plaintext);
        return plaintext;
    }
