...
The admin UI **+ New alias** flow regenerates the nginx fragments and
reloads. Two paths for the cert:

**Manual path** — leave the "Issue TLS cert via lego" checkbox unchecked.
You must then place `<domain>.crt` / `<domain>.key` at the path returned
by `aliasCertPath(domain)` (default `/etc/lego/certificates/`) **before**
clicking Create — otherwise the nginx reload fails and the alias is
rolled back.

**Automatic path** — wire `config.aliasIssuer` in your bootstrap and check
the box. The provider runs `lego` itself before writing the alias row.

```ts
// /opt/basic-storage/server.ts
config: {
    nginx: { /* … */ },
    publicHost: (b) => `https://${b}.cdn.bernouy.com`,
    aliasIssuer: {
        email:       "ops@example.com",
        challenge:   "dns",                // or "http"
        dnsProvider: "ovh",                // dns-only — full list at https://go-acme.github.io/lego/dns/
        env: {
        ...
        // binary:    "lego",              // default
        // storePath: "/etc/lego",         // default — must be writable by the basic-storage user
    },
},
```

> The dedicated `basic-storage` system user (§3) needs **write** access to
> `--path` (default `/etc/lego`). On a fresh install:
> `sudo chown -R basic-storage:basic-storage /etc/lego`.

> For `challenge: "http"`, lego defaults to spawning its own listener on
> port 80 — **conflicts with Nginx**. Either pass `httpWebroot` so lego
> writes the challenge to a directory served by Nginx (add a `location
> ^~ /.well-known/acme-challenge/` to the existing `cdn.<host>` server
> block pointing at that webroot), or use DNS-01 instead.

### Renewal cron

`POST /admin/api/aliases/renew` runs `lego renew` for every registered
alias and reloads Nginx if any cert was renewed. Drive it from cron:

```bash
sudo tee /etc/cron.d/basic-storage-alias-renew > /dev/null <<'EOF'
23 3,15 * * * basic-storage curl -sf -X POST -H 'Cookie: <admin-session>' https://cdn.bernouy.com/admin/api/aliases/renew >/dev/null
EOF
```

Authenticating cron against the admin guard is awkward — for production
you'll typically prefer calling `renewAllAliases(provider)` directly from
a scheduled task in the same Bun process (e.g. with `setInterval` or a
sibling unit), bypassing HTTP entirely.
