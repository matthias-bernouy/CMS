...
export class TokenInput extends Component {
    ...
    private renderTokens(): void {
        this.tokensEl?.replaceChildren(
            ...tokenLabels(this.selected, this.options).map((item) => tokenElement(item, this.removeValue)),
        );
    }
    ...
    private renderList(query: string): void {
        ...
        this.listbox.replaceChildren(
            ...this.items.map((item, index) => renderComboItem(item, index, this.activeIndex, "", this.selectItem)),
        );
    ...
    private emitChange(created: boolean): void {
        this.dispatchEvent(
            new CustomEvent("change", {
                bubbles: true,
                composed: true,
                detail: { value: this.value, values: this.values, created },
            }),
        );
    }
    ...
    private onCreate = (event: MouseEvent): void => {
        ...
        if (value) {
            this.selectItem({ kind: "create", value, label: value, disabled: false });
        } else {
            this.input?.focus();
            this.renderList("");
        }
