...
export const getRadios = (slot: HTMLSlotElement | null): HTMLElement[] => {
    ...
    return slot.assignedElements({ flatten: true }).filter((el) => el.tagName === "P9R-RADIO") as HTMLElement[];
};

let _uid = 0;
export const nextRadioGroupName = () => `radiogroup-${_uid++}`;

export const applyValue = (radios: HTMLElement[], value: string | null, internals: ElementInternals) => {
    radios.forEach((r) => {
