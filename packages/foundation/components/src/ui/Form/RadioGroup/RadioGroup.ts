...
import { upgradeProperty, getRadios, applyValue, syncLabel, syncDisabled } from "./compute";
...
export class RadioGroup extends Component {
    ...
    override connectedCallback() {
        ...
        for (const prop of ["value", "name", "disabled"]) {
    ...
    attributeChangedCallback(name: string, _oldVal: string | null, newVal: string | null) {
        if (name === "value") {
            applyValue(getRadios(this._slot), newVal, this._internals);
        } else if (name === "label") {
            syncLabel(this._label, newVal);
        } else if (name === "disabled") {
            syncDisabled(getRadios(this._slot), this.hasAttribute("disabled"));
        }
    }
