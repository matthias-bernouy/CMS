import { comboItemsFor } from "../../Selection/Combobox/list";
...
export function tokenLabels(values: string[], options: ComboOption[]): ComboOption[] {
    return values.map(
        (value) => options.find((option) => option.value === value) ?? { value, label: value, disabled: false },
    );
}

export function tokenItemsFor(
    options: ComboOption[],
    selected: string[],
    query: string,
    creatable: boolean,
    ...
    if (items.length > 0 || !query || !creatable || selected.includes(query)) {
        return items;
    }
