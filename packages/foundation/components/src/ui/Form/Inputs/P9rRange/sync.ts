...
export const syncBounds = (
    ...
    minEl: HTMLElement | null,
    maxEl: HTMLElement | null,
) => {
    if (!slider || !input || !minEl || !maxEl) {
        return;
    }
...
export const syncDisabled = (host: HTMLElement, slider: HTMLInputElement | null, input: HTMLInputElement | null) => {
    if (!slider || !input) {
        return;
    }
