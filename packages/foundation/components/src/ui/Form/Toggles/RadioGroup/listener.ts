...
export const syncRadios = (host: HTMLElement, slot: HTMLSlotElement | null, internals: ElementInternals) => {
...
export const handleKeydown = (host: HTMLElement, slot: HTMLSlotElement | null, e: KeyboardEvent) => {
    ...
    switch (e.key) {
        case "ArrowLeft":
        case "ArrowUp":
            nextIdx = (fallback - 1 + radios.length) % radios.length;
            break;
        case "ArrowRight":
        case "ArrowDown":
            nextIdx = (fallback + 1) % radios.length;
            break;
        case "Home":
            nextIdx = 0;
            break;
