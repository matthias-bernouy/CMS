...
export class FormDialog extends Component {
    ...
    attributeChangedCallback(name: string, _oldVal: string | null, newVal: string | null) {
        ...
        if (name === "action") {
            this._form.action = newVal ?? "";
        }
        if (name === "method") {
            this._form.method = newVal ?? "get";
        }
        if (name === "enctype") {
            this._form.enctype = newVal ?? "application/x-www-form-urlencoded";
        }
    }
    ...
    set action(v: string) {
        v ? this.setAttribute("action", v) : this.removeAttribute("action");
    }
    ...
    set method(v: string) {
        v ? this.setAttribute("method", v) : this.removeAttribute("method");
    }
    ...
    set enctype(v: string) {
        v ? this.setAttribute("enctype", v) : this.removeAttribute("enctype");
    }
}
