...
export function ensureCompositionStyle(root: Node): void {
    ...
    if (scope.querySelector(`style[${COMPOSITION_STYLE_ATTRIBUTE}]`)) {
        return;
    }

    const document = scope.nodeType === Node.DOCUMENT_NODE ? (scope as Document) : scope.ownerDocument!;
...
function styleScope(root: Node): Document | ShadowRoot {
    ...
    return scope.nodeType === Node.DOCUMENT_NODE ? (scope as Document) : (scope as ShadowRoot);
}
