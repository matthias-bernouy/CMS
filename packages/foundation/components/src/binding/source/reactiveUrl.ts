...
export function listenReactiveUrlChanges(
    template: string,
    doc: Document,
    onChange: () => void,
): () => void {
    ...
    if (listensState) doc.addEventListener(STATE_CHANGE_EVENT, onChange);
    ...
    return () => {
        ...
        if (listensState) doc.removeEventListener(STATE_CHANGE_EVENT, onChange);
    };
}
