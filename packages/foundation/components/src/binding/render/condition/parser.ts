...
class Parser {
    ...
    private parseOr(): ConditionNode {
        ...
        while (this.match("||")) node = { kind: "or", left: node, right: this.parseAnd() };
        return node;
    }

    private parseAnd(): ConditionNode {
        let node = this.parseComparison();
        while (this.match("&&")) node = { kind: "and", left: node, right: this.parseComparison() };
        return node;
    }
    ...
    private parseUnary(): ConditionNode {
        if (this.match("!")) return { kind: "not", node: this.parseUnary() };
    ...
    private parsePrimary(): ConditionNode {
        ...
        if (token.kind === "path") return { kind: "path", path: token.value };
        if (token.kind === "end") throw new Error("unexpected end of expression");
