...
export class RepositoryCatalogLoader {
    ...
    private async loadVersion(
        kind: string,
        indexVersion: IntegrationDefinitionVersion,
        work: BoundedCatalogWork,
        ...
        assertReleaseMatchesIndex(indexVersion, release);
        return {
            summary: {
                version,
                ...(packageMetadata
...
function assertReleaseMatchesIndex(
    index: IntegrationDefinitionVersion,
    release: PublicRepositoryRelease | undefined,
): void {
    if (!release) {
        if (index.status !== undefined || index.verificationDigest !== undefined) {
            throw new IntegrationRepositoryContractError();
        }
        return;
    }
    if (release.status !== (index.status ?? "installable") || release.verificationDigest !== index.verificationDigest) {
        throw new IntegrationRepositoryContractError();
    }
}
