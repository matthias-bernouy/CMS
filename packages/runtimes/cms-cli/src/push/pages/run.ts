 ...
 */
export async function runPages(adminBase: URL, token: string, flags: RunPagesFlags): Promise<number> {
    ...
    const entries = await classifyPages(local, remote, state, (id) => fetchRemotePageHash(adminBase, token, id), force);

    renderRecap(
        entries.map((e) => ({ path: e.page.path, status: e.status })),
        "page",
    );
    ...
    if (flags.dryRun) {
        console.log(`\n→ Dry-run — would push ${writes.length} page(s).`);
        return 0;
    }
