...
export async function scanIntegrations(siteDir: string): Promise<LocalIntegration[]> {
    ...
    for (const file of files.sort()) {
        ...
        try { request = normalizeIntegrationImport(JSON.parse(raw), file.slice(0, -".json".length)); }
        catch (e) { throw new Error(`Invalid integration in ${INTEGRATIONS_SUBDIR}/${file}: ${e instanceof Error ? e.message : e}`); }
