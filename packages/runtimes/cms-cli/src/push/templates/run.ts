...
export async function runTemplates(adminBase: URL, token: string, flags: RunTemplatesFlags): Promise<number> {
    ...
    if (!printValidation(report, force)) {
        return 1;
    }
    ...
    renderRecap(
        entries.map((e) => ({ path: e.template.identifier, status: e.status })),
        "template",
    );
    ...
    if (flags.dryRun) {
        console.log(`\n→ Dry-run — would push ${writes.length} template(s).`);
        return 0;
    }
    ...
    for (const { identifier, error } of result.failed) {
        console.error(`    ✗ ${identifier}: ${error}`);
    }
    for (const { identifier } of result.pushed) {
