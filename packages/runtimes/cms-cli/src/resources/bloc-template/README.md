...
These are declared in `@bernouy/components` (`src/assets/default.css`, served at `<basePath>/resources/css/cms-blocs.css`) and are available everywhere (they pierce the Shadow DOM because custom properties are inherited).
