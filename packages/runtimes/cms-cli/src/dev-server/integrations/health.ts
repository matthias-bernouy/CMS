...
export async function detectMissingGeneratedIntegrationArtifacts(
    siteDir: string,
): Promise<MissingGeneratedIntegrationsWarning | null> {
    ...
    if (existsSync(generatedInstallationsFile)) {
        return null;
    }
...
export async function warnMissingGeneratedIntegrationArtifacts(siteDir: string): Promise<void> {
    ...
    console.warn(
        `! Found ${count} local integration import${count === 1 ? "" : "s"} under ${SITE_INTEGRATIONS_DIR}/, but ${GENERATED_INTEGRATION_INSTALLATIONS_FILE} is missing.`,
    );
    console.warn(
        "  p9r dev does not rebuild integration artifacts automatically because imports can deploy connectors or write secrets.",
    );
    console.warn(
        "  Restore .p9r/generated, run `p9r pull --type=integrations`, or import the integration again from Admin > Integrations.",
    );
}
