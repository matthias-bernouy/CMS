...
const PORTS: DevPorts = Object.freeze({ control: 5100, delivery: 5101, repository: 5102, mongo: 27019 });
...
async function runDev(
    ...
    await startLocalSupabase(paths.supabase);
    ...
    try {
        const config = await loadOrCreateDevRuntimeConfig(paths.dev);
        const cms = await startLocalCms(paths, config, mongo, bridge.url, PORTS);
        log("");
