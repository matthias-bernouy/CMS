...
export async function prepareOfficialSourceInstallation(input: {
    ...
    const dependency = input.release.dependencyPackage;
    await input.installations.create({
        id: dependency.kind,
        label: dependency.definition.label,
        definitionVersion: dependency.version,
        definitionSnapshot: dependency.definition,
