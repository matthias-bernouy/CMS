    ...
    return Object.freeze({
        async listClaimable(limit: number) {
            const value = await request(
                repositoryManagementAbsolutePath(`${REPOSITORY_VERIFICATION_JOBS_PATH}?limit=${limit}`),
                "GET",
                worker,
            );
        ...
        async claim(candidate: CandidateStatusProjection) {
            const value = await request(
                repositoryManagementAbsolutePath(REPOSITORY_VERIFICATION_JOB_CLAIMS_PATH),
                "POST",
                worker,
                {
        ...
        async renew(candidate: ClaimedVerificationJob["candidate"]) {
            const value = await request(
                repositoryManagementAbsolutePath(REPOSITORY_VERIFICATION_JOB_LEASE_PATH),
                "POST",
                worker,
                {
                    candidateId: candidate.candidateId,
        ...
        async seal(candidate: ClaimedVerificationJob["candidate"], resultDigest: string) {
            const value = await request(
                repositoryManagementAbsolutePath(REPOSITORY_VERIFICATION_JOB_RESULT_CAPABILITIES_PATH),
                "POST",
                worker,
                {
                    candidateId: candidate.candidateId,
        ...
        async submit(
            ...
            const value = await request(
                repositoryManagementAbsolutePath(REPOSITORY_VERIFICATION_JOB_RESULT_PATH),
                "POST",
