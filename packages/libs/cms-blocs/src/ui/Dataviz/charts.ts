...
export function formatValue(n: number, format: string): string {
    ...
    if (format === "pct") return `${(n * 100).toFixed(1).replace(".", ",")} %`;
...
export function emptyHtml(title: string, hint?: string): string {
    return `<div class="empty"><svg class="empty-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">`
...
export function lineSvg(points: Point[]): string {
    ...
    const W = 320, H = 140, ML = 32, MR = 8, MT = 10, MB = 22;
    const pw = W - ML - MR, ph = H - MT - MB, baseY = MT + ph;
    const max = Math.max(...points.map((p) => p.value), 1);
    const n = points.length;
    const xy = points.map((p, i) => [ML + (n === 1 ? pw / 2 : (i / (n - 1)) * pw), MT + (1 - p.value / max) * ph] as const);
    ...
    return `<svg class="line" viewBox="0 0 ${W} ${H}" role="img">`
...
export function barsHtml(points: Point[], showCount: boolean): string {
    ...
    return points.map((p) => {
        ...
        return `<div class="bar"><span class="bar-key" title="${esc(p.label)}">${esc(p.label)}</span>`
