 ...
 * `BlocEditor.ts` (currently `getFields` for OpenAPI response-shape
 * autocomplete; future helpers around mock generation, schema lookup,
 * etc. land here too).
 *
 ...
 * Plain functions like `getFields` should be bundled inline per bloc —
 * cheaper than the global plumbing, and keeps the editor entry's
 * shimmable surface small. This entry is NOT intercepted; bun bundles
 * each call site normally.
 */
