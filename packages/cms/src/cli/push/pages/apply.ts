 ...
 */
export async function applyPushPages(
    adminBase: URL,
    token:     string,
    entries:   ClassifiedPage[],
): Promise<ApplyResult> {
    ...
    for (const e of entries.filter(e => e.status === "new")) {
        try { await postPage(adminBase, token, e.page); }
        catch (err) { result.failed.push({ path: e.page.path, error: msg(err) }); }
    }

    const idByPath = new Map((await fetchRemoteList(adminBase, token)).map(r => [r.path, r.id]));

    for (const e of writes) {
        if (result.failed.some(f => f.path === e.page.path)) continue;
