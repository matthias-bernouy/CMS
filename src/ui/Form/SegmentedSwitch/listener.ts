import {
    getOptions, updateSliderPosition, updateSlottedSelections,
} from './compute';
...
export const handleKeydown = (host: SwitchHost, slot: HTMLSlotElement | null, e: KeyboardEvent) => {
    ...
    switch (e.key) {
        case 'ArrowLeft':
        case 'ArrowUp':   nextIndex = (fallback - 1 + options.length) % options.length; break;
        case 'ArrowRight':
        case 'ArrowDown': nextIndex = (fallback + 1) % options.length; break;
