...
export class TagSuggest extends Component {
    ...
    static get observedAttributes() { return ['placeholder', 'mode', 'resource', 'api', 'disabled', 'value']; }
    ...
    constructor() {
        ...
        if (this._suggestionsEl) this._suggestionsEl.id = `${this._uid}-listbox`;
        if (this._input) this._input.setAttribute('aria-controls', `${this._uid}-listbox`);
    }
    override connectedCallback() {
        if (!this._defaultsCaptured) { this._defaultValue = this.getAttribute('value') ?? ''; this._defaultsCaptured = true; }
        ['placeholder', 'mode', 'resource', 'api', 'disabled', 'value'].forEach(p => upgradeProperty(this, p));
        this._input?.addEventListener('input', this._onInput);
        this._input?.addEventListener('keydown', this._onKeyDown);
        this._input?.addEventListener('focus', this._onFocus);
        this._input?.addEventListener('blur', this._onBlur);
        loadSuggestions(this).then(data => { if (data) this._allSuggestions = data; });
    }
    ...
    formResetCallback() {
        this._silent = true;
        try { this.value = this._defaultValue; } finally { this._silent = false; }
    }
    ...
    attributeChangedCallback(name: string, _oldVal: string | null, newVal: string | null) {
        ...
        if (name === 'placeholder') this._input.placeholder = newVal ?? '';
            ...
            loadSuggestions(this).then(data => { if (data) this._allSuggestions = data; });
        } else if (name === 'mode') renderTagList(this);
        else if (name === 'value') this.value = newVal ?? '';
    }
    ...
    get value(): string { return computeValueString(this.getAttribute('mode') || 'multiple', this._tags); }
    set value(v: string) {
        const mode = this.getAttribute('mode') || 'multiple';
        this._tags = parseValueString(mode, v);
        if (this._input) this._input.value = mode === 'multiple' ? '' : (this._tags[0] ?? '');
        update(this);
    }
    ...
    get mode() { return this.getAttribute('mode') || 'multiple'; }
    set mode(v: string) { v ? this.setAttribute('mode', v) : this.removeAttribute('mode'); }
    get resource() { return this.getAttribute('resource') || ''; }
    set resource(v: string) { v ? this.setAttribute('resource', v) : this.removeAttribute('resource'); }
    get api() { return this.getAttribute('api') || ''; }
    set api(v: string) { v ? this.setAttribute('api', v) : this.removeAttribute('api'); }
    get disabled() { return this.hasAttribute('disabled'); }
    set disabled(v: boolean) { v ? this.setAttribute('disabled', '') : this.removeAttribute('disabled'); }
    get tags() { return [...this._tags]; }
}

if (!customElements.get('p9r-tag-suggest')) customElements.define('p9r-tag-suggest', TagSuggest);
