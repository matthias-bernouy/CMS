...
test("documents every Source image switch as dark by default", () => {
    expect(instanceEnvExampleSource).toContain("CMS_SOURCE_IMAGE_TRANSFORMS_ENABLED=false");
    expect(instanceEnvExampleSource).toContain("CMS_RESPONSIVE_PUBLIC_SOURCE_IMAGES_ENABLED=false");
    expect(instanceEnvExampleSource).toContain("CMS_RESPONSIVE_PRIVATE_SOURCE_IMAGES_ENABLED=false");
});
