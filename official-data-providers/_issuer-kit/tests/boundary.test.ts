...
// `@bernouy/data-provider-contract`. Drift here would break the
// sign↔verify decoupling the architecture relies on.
const FORBIDDEN = [
    "@bernouy/data-provider-sdk",
    "@bernouy/cms",
...
test("detector flags a forbidden import (self-test of the guard)", () => {
    expect(findForbidden(`import x from "@bernouy/data-provider-sdk";`)).toContain("@bernouy/data-provider-sdk");
    expect(findForbidden(`import x from "@bernouy/cms";`)).toContain("@bernouy/cms");
    expect(findForbidden(`import x from "@bernouy/data-provider-contract";`)).toHaveLength(0);
