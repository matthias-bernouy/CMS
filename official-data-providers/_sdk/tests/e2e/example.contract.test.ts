...
import { examplePackageRoot } from "@bernouy/data-provider-example/constants";
...
test("example imports ONLY @bernouy/data-provider-sdk (decoupling proof)", async () => {
    ...
    for await (const f of new Bun.Glob("**/*.ts").scan({ cwd: exampleSrc })) {
        ...
        for (const m of txt.matchAll(/from "([^"]+)"/g)) {
            const spec = m[1]!;
            if (spec.startsWith("@bernouy/") && spec !== "@bernouy/data-provider-sdk") {
