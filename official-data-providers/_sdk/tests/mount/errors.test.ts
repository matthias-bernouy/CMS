...
test("each family → right status / slug / title / content-type", () => {
    ...
    expect(toProblem(new PlaneError()).body.type).toBe("urn:data-provider:error/plane");
    expect(toProblem(new SuspendedError()).body.type).toBe("urn:data-provider:error/suspended");
...
test("400/404/409 keep the actionable message (hub/config facing)", () => {
    ...
    expect(c.body.type).toBe("urn:data-provider:error/provisioning");
});
...
test("ConfigError and unknown errors → 500, opaque, no leak", () => {
    ...
    expect(plain.body.type).toBe("urn:data-provider:error/internal");
});
