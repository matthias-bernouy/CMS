...
Ce document est **normatif**. Tout official data-provider DOIT le respecter
...
Un data-provider est une API externe importée dans le CMS via son schéma
OpenAPI. **Aucun client n'appelle jamais le data-provider en direct.** Tout
...
| `/.well-known/data-provider-info`       | GET     | non  | §2.0 — métadonnées discovery (providerId + kind opt-in) |
...
`/health`, `/.well-known/data-provider-info`, `/openapi.json` et
...
### 2.0 — `/.well-known/data-provider-info`
...
1. Le central résout `providerId` AVANT de pouvoir mint un token CP avec
   ...
   gated).
2. Un UI de gestion groupe les data-providers par `providerKind` (opt-in)
   pour l'affichage.
...
- **non réversible** par le data-provider (le provider ne peut pas remonter
  à l'identité réelle).
...
- `{iss}/.well-known/oauth-authorization-server` — métadonnées, contenant au
  ...
  un champ **`data_provider_contract_version`** (version du présent contrat de fil, pour
...
- DOIT rejeter un métadoc dont `data_provider_contract_version` est incompatible avec la
...
- **Métadoc RFC 8414 conforme** : `issuer` == `iss` des tokens, `jwks_uri`
  same-origin, `data_provider_contract_version` renseignée.
...
Un official data-provider DOIT être un **déploiement partagé unique** :
...
Chaque official data-provider vit dans un dossier dédié :
`official-data-providers/<name>/`, avec toute sa structure interne à
l'intérieur.
...
| --- | --- | --- | --- |
| `/openapi.json` | 3 — consommation | racine | l'importer data-provider du CMS |
...
Le hub fetch ce schéma à l'import du data-provider, le cache, le re-fetch
...
- **Versionnement du contrat** : `data_provider_contract_version` dans le métadoc RFC
