 ...
 * dependencies (keycloak / composite-auth). Admin OIDC is platform-level
 * (shared realm + client) and configured on the runtime itself, not per-tenant.
 */
export interface CmsTenantRuntime {
    /** Mount the tenant's CMS admin + seed the initial admin. Idempotent. */
    provision(input: { tenantId: string; displayName?: string; initialAdminEmail: string }): Promise<void>;
    /** Re-mount the tenant (the auth chain is captured at mount). */
    reprovision(input: { tenantId: string; displayName?: string; initialAdminEmail: string }): Promise<void>;
...
export function makeCmsHooks(opts: CmsHooksOptions): ProviderHooks {
    return {
        onProvision: async ({ tenantId, displayName, providerConfig }) => {
            ...
            await opts.runtime.provision({ tenantId, displayName, initialAdminEmail: cfg.initialAdminEmail });
        },
        ...
        onUpdate: async ({ tenantId, patch }) => {
            ...
            await opts.runtime.reprovision({ tenantId, initialAdminEmail: cfg.initialAdminEmail });
        },
