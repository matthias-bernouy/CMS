...
import {
    TENANT_PROVISIONER_CONTRACT_VERSION, METADOC_VERSION_FIELD, metadocPath,
...
test("buildMetadoc satisfies every _sdk discovery invariant by construction", () => {
    ...
    expect(m[METADOC_VERSION_FIELD]).toBe(TENANT_PROVISIONER_CONTRACT_VERSION);
    expect(m["signing_alg_values_supported"]).toEqual(["ES256"]);
    // the literal field name is the contract one (locks sign↔verify)
    expect(METADOC_VERSION_FIELD).toBe("tenant_provisioner_contract_version");
