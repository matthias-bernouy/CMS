...
import { defaultClock } from "@bernouy/tenant-provisioner-contract";
...
export interface CreateProviderOptions {
     ...
     *  omitted. Default: `<cwd>/tenant-provisioner-logs`. */
    logDir?: string;
    ...
    /** Optional: TP-computed read-only info about a tenant (e.g. a realm
...
export function createProvider(opts: CreateProviderOptions): Provider {
    ...
    const store    = opts.store ?? new FileLogStore(
        opts.logDir ?? `${process.cwd()}/tenant-provisioner-logs`,
    );
