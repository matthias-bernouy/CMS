...
test("public endpoints need no auth", async () => {
    ...
    try {
        ...
        // §2.0 — tenant-provisioner-info is also public (no token).
        const info = await (await s.request("GET", "/.well-known/tenant-provisioner-info")).json();
...
test("§2.0 + D13: providerKind opt-in is exposed when declared", async () => {
    ...
    try {
        ...
        const info = await (await srv.request("GET", "/.well-known/tenant-provisioner-info")).json();
...
test("/openapi.admin.json is control-plane-gated, then generated from meta", async () => {
    ...
    try {
        ...
        expect(spec["x-tenant-provisioner"].contractVersion).toBe("1.0");
    } finally { s.stop(); }
});
