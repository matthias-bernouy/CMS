...
#    + cacheControl + bucket-notfound fragments end up.
...
chown -R cdn:cdn "$DATA/buckets" "$DATA/lego" "$DATA/nginx-generated" "$DATA/ssh" "$DATA/lsyncd"
...
# 6. Daily admin cert renew loop. Mode follows the issuance mode:
#    - DNS-01: same as boot, talks to the DNS provider API.
#    - HTTP-01: webroot mode (lego writes the challenge file to
#      $DATA/lego/webroot, nginx serves it). nginx is up at this point,
#      its port-80 server has the `location ^~ /.well-known/acme-challenge/`
#      block pointing at that webroot.
RENEW_PID=
(
    while true; do
        sleep 86400
...
RENEW_PID=$!
echo "[origin] admin cert renew loop started (pid ${RENEW_PID})"
