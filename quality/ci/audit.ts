...
const ADVISORY_PATTERN = /^(?:CVE-\d{4}-\d{4,}|GHSA-[23456789cfghjmpqrvwx]{4}-[23456789cfghjmpqrvwx]{4}-[23456789cfghjmpqrvwx]{4})$/i;
...
export function validateAuditExceptions(value: unknown, today = new Date()): AuditException[] {
    if (!value || typeof value !== "object") throw new Error("Audit exceptions must be an object");
    const file = value as Partial<AuditExceptionsFile>;
    if (file.schemaVersion !== 1) throw new Error("Unsupported audit-exception schema");
    if (!Array.isArray(file.exceptions)) throw new Error("Audit exceptions must be an array");
    ...
    for (const [index, exception] of file.exceptions.entries()) {
        ...
        if (createdAt > todayTimestamp) throw new Error(`Audit exception ${exception.advisory} starts in the future`);
        if (expiresAt < todayTimestamp) throw new Error(`Audit exception ${exception.advisory} expired`);
