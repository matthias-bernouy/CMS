...
export function parseRemovedOrRenamedPaths(nameStatus: string): Set<string> {
    ...
    for (const line of nameStatus.split(/\r?\n/)) {
        ...
        if (firstPath && (status === "D" || status?.startsWith("R"))) {
            removed.add(normalizePath(firstPath));
        }
    }
    return removed;
}
...
export function readReferenceBaseline(
    ...
    if (commit.exitCode !== 0) {
        throw new Error(`Coverage baseline reference is not a commit: ${reference}`);
    }
    ...
    for (const repositoryPath of repositoryPaths) {
        ...
        if (show.exitCode === 0) {
            return parseBaseline(show.stdout, `Coverage baseline at ${reference}`);
        }
    ...
    if (tree.exitCode !== 0) {
        throw new Error(tree.stderr.trim() || `Cannot inspect coverage baseline at ${reference}`);
    }
    if (tree.stdout.trim().length === 0) {
        return undefined;
    }
