...
const POLICY_ENTRY_PATH = "quality/ci/file-size-ratchet.ts";
...
function policyExists(reference: string, repositoryRoot: string): boolean {
    return git(["cat-file", "-e", `${reference}:${POLICY_ENTRY_PATH}`], repositoryRoot).exitCode === 0;
}
...
export function resolveFileSizeReference(
    ...
    const introduction = requireGit(
        ["log", "--reverse", "--format=%H", `${candidate}..HEAD`, "--", POLICY_ENTRY_PATH],
